...
function use_firework(nItemIndex, nType)
	...
	if nCurDate ~= nDate then
		...
		VNG_SetTaskPos(FIREWORK_CNDSK_USE, 0, 3, 3) 	--Reset gi¸ trÞ x¸c ®Þnh x6 kinh nghiÖm tèn 79 vµng
	end
	
	local nCheck_kichhoat = VNG_GetTaskPos(FIREWORK_CNDSK_USE, 3, 3)		
	if nType == 1 then
		VNG_SetTaskPos(FIREWORK_CNDSK_USE, 1, 3, 3) 	--Set gi¸ trÞ x¸c ®Þnh kh«ng x2 kinh nghiÖm 
	elseif nType == 2 then
		if nCheck_kichhoat ~= 2 then
--			if nDate >= 150720 and nDate <= 150810 then
				if GetCash() < 890000 then
					Talk(1,"","§¹i hiÖp kh«ng cã ®ñ 89 vµng ®Ó thùc hiÖn x6 ®iÓm kinh nghiÖm")
					return
				end
				if Pay(890000) ~= 1 then
					return
				end
--			end
			VNG_SetTaskPos(FIREWORK_CNDSK_USE, 2, 3, 3) 	--Set gi¸ trÞ x¸c ®Þnh x6 kinh nghiÖm tèn 79 vµng
			Msg2Player("§¹i hiÖp tiªu hao 89 vµng ®Ó ®­îc x6 ®iÓm kinh nghiÖm khi ®èt ph¸o hoa")
			gf_WriteLogEx(szLogTitle, "thµnh c«ng", 1, "Tiªu 89 vµng ®­îc x6 kinh nghiÖm")	
		end	
	elseif nType == 3 then	
		if nCheck_kichhoat ~= 3 then
--			if nDate >= 150720 and nDate <= 150810 then
				if GetCash() < 1990000 then
					Talk(1,"","§¹i hiÖp kh«ng cã ®ñ 199 vµng ®Ó thùc hiÖn x12 ®iÓm kinh nghiÖm")
					return
				end
				if Pay(1990000) ~= 1 then
					return
				end
--			end
			VNG_SetTaskPos(FIREWORK_CNDSK_USE, 3, 3, 3) 	--Set gi¸ trÞ x¸c ®Þnh x12 kinh nghiÖm tèn 199 vµng
			Msg2Player("§¹i hiÖp tiªu hao 199 vµng ®Ó ®­îc x12 ®iÓm kinh nghiÖm khi ®èt ph¸o hoa")
			gf_WriteLogEx(szLogTitle, "thµnh c«ng", 1, "Tiªu 199 vµng ®­îc x12 kinh nghiÖm")	
		end	
	end			
...
--	end
	...
	if nCount >= nMaxDay then
		Talk(1, "", "H«m nay thiÕu hiÖp ®· dïng 60 ph¸o hoa råi, ch¼ng cßn ph¸o cho thiÕu hiÖp n÷a ®©u. H·y quay l¹i vµo ngµy mai!")
		return
	end
...
--	end
	...
	if VNG_GetTaskPos(FIREWORK_CNDSK_USE, 3, 3) == 2 then -- x6 kinh nghiÖm khi kick ho¹t x6
--		if nDate >= 150720 and nDate <= 150810 then
			nExp = nExp * 6
--		end
	elseif VNG_GetTaskPos(FIREWORK_CNDSK_USE, 3, 3) == 3 then -- x12 kinh nghiÖm khi kick ho¹t x12
		nExp = nExp * 12
	end
	ModifyExp(nExp)
...
--	end
	...
	--cd key
	SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "_ck_UseItemFirecraker()");

	FireEvent("event_cost_item", 2,1,30297,1);

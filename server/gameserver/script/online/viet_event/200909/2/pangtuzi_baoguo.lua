...
--	end
	...
	if PKState == 0  then
		Talk(1,"","Tr¹ng th¸i luyÖn c«ng kh«ng thÓ dïng ThŞt N­íng, ®¹i hiÖp chuyÓn sang c¸c tr¹ng th¸i <color=red>PK<color> ®Ó cã thÓ dïng ®­îc ThŞt N­íng !!");
		return 0;
	end;
	...
	-- mçi nh©n vËt thuëng thøc ®óng 100 c¸i / ngµy
	if GetByte(nTaskVal,2) >= 100 then
		Talk(1,"","Mçi ngµy chØ cã thÓ th­ëng thøc 100 ThŞt N­íng.")
		return 0
	end	
	...
	if GetTime() - GetTask(GET_PANGTUZI_TIME_ID) < 45 then
		Talk(1,"","B¹n cßn "..GetTask(GET_PANGTUZI_TIME_ID) + 45 - GetTime().." gi©y míi cã thÓ tiÕp tôc th­ëng thøc ThŞt N­íng.")
		return 0
	end
	...
	--  phÇn th­ëng thŞch thá n­íng
	--	ThoNuong_Award()		--Expired
	
	-- sau b¶o tr× 4-12-2010 sÏ cã thªm tİnh n¨ng nµy
	...
	StartTimeGuage("ThŞt N­íng",45,0,1)	
		
	if GetTask(TSK_THONUONG_ACCEPT) == 1 then
		set_task_thonuong(BYTE_COUNT_COLLECT, get_task_thonuong(BYTE_COUNT_COLLECT) + 1)
		Msg2Player("Thu thËp ThŞt N­íng "..get_task_thonuong(BYTE_COUNT_COLLECT).."/20")
...
--			BachTienSinh_Award();
			...
			TaskTip("Hoµn thµnh nhiÖm vô thu thËp ThŞt N­íng. Mau vÒ b¸o cho B¹ch Tiªn Sinh")
		end	
	end		
end
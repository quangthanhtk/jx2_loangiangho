...
--	end
	...
	if GetLevel() < 1 then
		Talk(1, "", "VÉn ch­a ®¹t ®Õn cÊp 1 kh«ng thÓ trång c©y");
		return 0;
	end
	if checkPlaceMainCity() == 0 then
		Talk(1, "", "§Þa ®iÓm trång c©y kh«ng thÝch hîp");
		return 0;
	end
-----ÖÖÊ÷µÄÈÕÆÚ¼ì²é£¬Èç¹û½ñÌìµÚÒ»´ÎÖÖÊ÷£¬±íÊ¾ÏÖÔÚ¿ÉÒÔÖÖÊ÷ÁË£¬ÔòÇå¿ÕÈÎÎñ±äÁ¿-----
-----½«ÖÖÊ÷µÄÊýÁ¿ÉèÎª0£¬ÖÖÊ÷ÈÕÆÚÉèÎª½ñÌì£¬ÖÖÏÂÊ÷µÄÊ±¼äºÍ½½Ë®Ê±¼äÉèÎªÏÖÔÚ--------
	local nDate = tonumber(date("%y%m%d"));
--Èç¹û½ñÌì»¹Ã»ÓÐÖÖÊ÷£¬Ôò±íÊ¾¿ÉÒÔÖÖÊ÷
...
-----¼ì²éÊÇ·ñÒÑ¾­ÖÖÏÂÒ»¿ÃÊ÷ÁË----------------------------------------------------
		...
		if GetTask(TASK_PLANT_TREE_TYPE) == 2 and GetTime() - GetTask(TASK_PLANT_TREE_TIME) < 210 then--Èç¹ûÖÖÏÂ²»ËÀÊ÷£¬ÔòÔÚÃ»²¦Ö®Ç°£¬¶¼²»ÄÜÖÖ±ðµÄÊ÷
				Talk(1, "", "§¹i hiÖp ®· trång 1 c©y råi");
				return 0;
		end
-----¼ì²éÀëÉÏ´ÎÖÖÊ÷ÊÇ·ñÒÑ¾­¹ýÁËPLANT_TREE_INTERVALÕâÃ´³¤Ê±¼ä---------------------
...
-----´Ë´¦ÔËÓÃÊ÷³öÏÖµÄ¸ÅÂÊÀ´µÃµ½ÖÖÏÂÊ÷µÄÀàÐÍ-----------------------------------------------------
		local nRand = 0;
		local nTreeType = 0;
		local nMin = 0;
		...
		local nShowUsed = GetTask(TASK_PLANT_TREE_COUNT) + 1
		local npcTreeIndex = CreateNpc(TREE_TYPE[nTreeType][1], GetName().." trång "..TREE_TYPE[nTreeType][2], nMapID, nMapX, nMapY);
		Msg2Player("Sè l­îng h¹t gièng trång trong ngµy: "..nShowUsed.."/"..PLANT_TREE_LIMIT)

...
function SelectSay()	
	local tbSayDialog = {};
	local nSaySize = 0;
	local szSayHead = "Th«ng tin chi tiÕt c¸c ho¹t ®éng ®ång ®¹o cã thÓ xem t¹i trang chñ."
	...
	--tinsert(tbSayDialog, format("Sù kiÖn %s/monthlyEvent_OnNpcTalk", g_monthlyEvent_Name)) --LongDaiKa: ®ãng event h»ng th¸ng
	----------------------------------
...
--		tinsert(tbSayDialog, "Mua 1000 GiÊy Ngò S¾c (tiªu hao 79 vµng)/#nguyenlieu_thang4(2)")
		tinsert(tbSayDialog, "Event ChÝnh Th¸ng 10/EventMenu")
--		tinsert(tbSayDialog, "Ho¹t §éng ThÎ Th¸ng 9/ac_MainDialog")
...
--		tinsert(tbSayDialog, "Event MÇm C©y/Event_Tree_Thang5")
	...
	--tinsert(tbSayDialog,1,"Vò KhÝ UÈn Linh/WEfMainDialog")

	if mg_CheckOpen() == 1 then
		tinsert(tbSayDialog,"HiÖp Kh¸ch Hµnh/_OpenMonthlyGoals")
	end
...
--	tbSayDialog,szSayHead = VET_201503_AdditionDialog(tbSayDialog, szSayHead);
	...
	-- if gf_GetTaskByte(2731,1) == 1 then
		-- tinsert(tbSayDialog,"NhËn phÇn th­ëng 4000 Nh¸nh §µo,400 Nh¸nh Mai,4000 Nô TÇm Xu©n/#EventT201502_Max4000_Menu(1)")
	-- end
	
	-- if GetTask(2766) == 4000 and gf_GetTaskByte(2731,2) == 0 then
		-- tinsert(tbSayDialog,"NhËn phÇn th­ëng 4000 Nh¸nh §µo/#EventT201502_Max4000_Menu(2)")
	-- end
	-- if GetTask(2767) == 4000 and gf_GetTaskByte(2731,3) == 0 then
		-- tinsert(tbSayDialog,"NhËn phÇn th­ëng 4000 Nh¸nh Mai/#EventT201502_Max4000_Menu(3)")
	-- end
	-- if GetTask(2768) == 4000 and gf_GetTaskByte(2731,4) == 0 then
		-- tinsert(tbSayDialog,"NhËn phÇn th­ëng 4000 Nô TÇm Xu©n/#EventT201502_Max4000_Menu(4)")
	-- end
	tinsert(tbSayDialog,"KÕt thóc ®èi tho¹i/dialog_over"	)
	nSaySize = getn(tbSayDialog);
	Say(szSayHead, nSaySize, tbSayDialog);
...
--     		szSayHead = "Ho¹t ®éng th¸ng 3 diÔn ra trong thêi gian tõ ngµy 25 th¸ng 2 n¨m 2011 ®Õn 24 giê ngµy 27 th¸ng 3 n¨m 2011, néi dung chi tiÕt t¹i trang chñ."
...
--	end
	...
	--if nDate >= 20110820 and nDate <= 20191231 then
		tinsert ( tbSayDialog, "NhËn Ph¸o Hoa ChiÕn Th¾ng/Confirm_VET_201106_Give_Fire" )	--VET_201106_Give_Fire
	--end
...
function NextPage2()
	local tbSayDialog = {};
	local nSaySize = 0;
	local szSayHead = "Th«ng tin chi tiÕt c¸c ho¹t ®éng ®ång ®¹o cã thÓ xem trªn trang chñ."
	local nNation = GetGlbValue(GLB_TSK_SERVER_ID)
	local nDate = tonumber(date("%Y%m%d"))

      if nDate >= 20101210 and nDate <= 20110109 then
    		szSayHead = "<color=yellow>Ho¹t ®éng mõng Gi¸ng Sinh<color> b¾t ®Çu tõ <color=red>ngµy 10/12/2010 ®Õn ngµy 09/1/2011<color>. Néi dung chi tiÕt vui lßng t×m hiÓu t¹i trang chñ."
   	 end
...
function EventThang8_HoTro1()
 ...
 function EventT201502_Max4000_Menu(nNum)
	...
	if nNum == 1 then
		gf_AddItemEx2({2,95,204,2}, "Thiªn Cang LÖnh", "MORONGEVENT", "Nhan",0,1)
		gf_AddItemEx2({2,1,30370,2}, "Thiªn M«n Kim LÖnh", "MORONGEVENT", "Nhan",0,1)
	...
	if nNum == 2 then 
		...
		gf_AddItemEx2({2,1,3356,1}, "Tói Thiªn Th¹ch Tinh Th¹ch", "MORONGEVENT", "Nhan",7 * 24 * 3600,1)
	...
	if nNum == 4 then 
		gf_Modify("Exp",2000000000)		
		gf_AddItemEx2({1,1,15,333}, "B¸nh Bét Vµng", "MORONGEVENT", "Nhan",0,1)
		gf_AddItemEx2({1,1,19,333}, "T©y B¾c Väng", "MORONGEVENT", "Nhan",0,1)
		gf_AddItemEx2({1,1,17,333}, "Minh NguyÖt D¹", "MORONGEVENT", "Nhan",0,1)
		gf_AddItemEx2({2,1,30344,4}, "ThÇn Tµi B¶o R­¬ng", "MORONGEVENT", "Nhan",0,1)

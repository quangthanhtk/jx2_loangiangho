...
function getfruit()
	if GetTask(TASK_TREEGROW_SMALL) == 3 then
		...
		if check_npc_valid(npcTreeIndex, 2) ~= 1 then
			WriteLog("[Lçi ho¹t ®éng trång c©y Th¸i H­]:"..GetName().." NPC is not valid when getfruit of the small tree");
			return 0;
		end
...
--		end
		...
		FireEvent("event_mission_task_award", "planttree", "smalltreeprize", 0)
		--SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "_ck_BZBD_PVP_Set(1)");	 -- cdkey 1ºÅpvpÈÎÎñ
		SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(1301)");	

		SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(2210)");	
		--Give_Fruit_Award();
        --VietSmallFruit_Award();

		WAward:BatNhaNho_ThuHoach()
           --IP Bonus
...
--	    local szFullLog = VIET_0912_STR_EVENT_LOG_TITLE.."-ÖÖÐ¡°ãÈôÊ÷µÃÊ¥µ®ÌÇ¹û"
--	    gf_AddItemEx2({2, 1, 30138, 20}, "Ê¥µ®ÌÇ¹û", szFullLog, 0, 1, 10, 1);
--	end
--end

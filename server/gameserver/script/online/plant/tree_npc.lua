...
function givewater()
	--LongDaiKa: mod yªu cÇu bËt pk khi t­íi c©y
	local nPkFlag1, nPkFlag2 = GetPKFlag();
	if g_WCFG_BATNHALON.bPkReq == WEnv.TRUE and nPkFlag1 == 0 then
		WNpc:Talk("H¹t gièng cÇn thªm s¸t khÝ ®Ó ph¸t triÓn, <sex> cÇn bËt chiÕn ®Êu míi cã thÓ t­ c©y!", 1);
		return WEnv.RETCODE_ERROR;
	end
	--LongDaiKa: mod di chuyÓn t­íi c©y, l­u vÞ trÝ
	local nMapId, nPosX, nPosY = WPlayer:GetWorldPos();
	local nLastPosData = WTask:Get(WTASKID_PLANT_GIVEWATER_LAST_POS);
	local nCurrentPosData = nPosX*1000 + nPosY;
	if nLastPosData == nCurrentPosData then
		WNpc:Talk("Long m¹ch chç nµy ®· c¹n råi, <sex> h·y nhÝch ng­êi qua mét tÝ xem sao!", 1);
		return WEnv.RETCODE_ERROR;
	end
	...
	--LongDaiKa: mod di chuyÓn t­íi c©y, l­u vÞ trÝ
	WTask:Set(WTASKID_PLANT_GIVEWATER_LAST_POS, nCurrentPosData);
end;
...
function getfruit()
	...
	if GetTask(TASK_TREEGROW) == 6 then
		...
		if check_npc_valid(npcTreeIndex, 1) ~= 1 then
			WriteLog("[Lçi ho¹t ®éng trång c©y Th¸i H­]:"..GetName().." NPC is not valid when getfruit of the tree");
			return 0;
		end
...
--		gf_AddItemEx2({tFruitAwardJune2010[nGroup][6][1], tFruitAwardJune2010[nGroup][6][2], tFruitAwardJune2010[nGroup][6][3], 2}, tFruitAwardJune2010[nGroup][7], "Bat Nha Lon", "nhËn")
		...
		--Give_Fruit_Award(npcTreeIndex); --LongDaiKa: bá phÇn th­ëng trång b¸t nh· cò, ®ãng r¬i tói vµng
		SetNpcScript(npcTreeIndex,"");
		SetNpcLifeTime(npcTreeIndex,0);
		...
		----------------------- PhÇn th­ëng cò
		--LongDaiKa: bá phÇn th­ëng trång b¸t nh· lín cò
		--local nRandomNum = 0;
		...
		-- NhiÖm vô ChuyÓn Sinh 4
		 if GetTask(TRANSLIFE_MISSION_ID) == 26 and gf_GetTaskByte(TRANSLIFE_TASK_34_FLAG,TRANSLIFE_BYTE_TASK4) < 16 then
			...
			if gf_GetTaskByte(TRANSLIFE_TASK_34_FLAG,TRANSLIFE_BYTE_TASK4) == 16 then
				TaskTip("Hoµn thµnh yªu cÇu cña B¹ch Tiªn Sinh 16 lÇn trång thµnh c«ng B¸t Nh· Lín")
			end
		end
		
		--gf_EventGiveCustomAward(2,280000,0,"Bat Nha Lon","nhËn")
		...
		--give_viet200908_banruozhengshu(); -- B¸t Nh· Chøng Th­ --LongDaiKa: bá phÇn th­ëng trång b¸t nh· cò
        ...
        --Give_DaBanRuoShu_Award();
          ThuHoachBatNhaLon();
		WAward:BatNhaLon_ThuHoach(); --LongDaiKa: chuyÓn phÇn th­ëng vËt phÈm/vµng/exp vÒ ®©y
        --give_gongcheng_fangshou_award();
        
		FireEvent("event_mission_task_award", "planttree", "bigtreeprize", 0)
		SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(2209)");
		-- SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "_ck_BZBD_PVP_Set()");	 -- cdkey 2ºÅ pvpÈÎÎñ ÏÖÔÚÊÇÖÖ´óÊ÷ÁË
	end;
end;
...
--		gf_SetLogCaption("Ô½ÄÏ09Äê7ÔÂ»î¶¯Ö®Ò»×·Ñ°³¤ÉúÆÕ");
--		gf_AddItemEx({2, 1, 30088, 100}, "³Â¾ÉÃØ±¾");
--		gf_SetLogCaption("");
--	end
--end
--
...
--		gf_SetLogCaption("[Ô½ÄÏ2009Äê8ÔÂ·Ý»î¶¯]£º");
--		gf_AddItemEx({2, 1, 30097, 70}, "Ë®¹û´ü");
--		gf_SetLogCaption("");
--	end
--end
----09Äê8ÔÂµ÷Õû£¬µÃ°ãÈôÖ¤Êé
...
--		gf_SetLogCaption("[Ô½ÄÏ09ÄêÖÐÇï»î¶¯]£º");
--		gf_AddItemEx({2, 1, 30106, 100}, "°×Ãæ°ü");
--		gf_SetLogCaption("");
--	end
--end
--
...
--	    local szFullLog = VIET_0912_STR_EVENT_LOG_TITLE.."-ÖÖ°ãÈôÊ÷µÃÊ¥µ®ÌÇ¹û"
--	    gf_AddItemEx2({2, 1, 30138, 40}, "Ê¥µ®ÌÇ¹û", szFullLog, 0, 1, 10, 1);
--	end
--end

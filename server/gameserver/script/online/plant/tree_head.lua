...
ACT_DATE_START	= 20201124 --½ÚÈÕ¿ªÊ¼ÈÕÆÚ
ACT_DATE_END	= 20210103 --½ÚÈÕ½áÊøÈÕÆÚ

TREELIFETIME = 5*60;	--Ê÷µÄÉúÃü£º5·ÖÖÓ
PLANT_SMALL_TREE_LIMIT = 20;	--Ã¿ÌìÖÖÐ¡Ê÷µÄÊýÁ¿ÏÞÖÆ -- 20101122
PLANT_BIG_TREE_LIMIT = 20;	--Ã¿ÌìÖÖ´óÊ÷µÄÊýÁ¿ÏÞÖÆ
...
function is_in_act_time()
	local today = tonumber(date("%Y%m%d"))
	if today >= ACT_DATE_START and today <= ACT_DATE_END then
		return 1
	end
	return 0
end

--±äÌ¬µÄÐèÇó »î¶¯ÆÚ¼äÕâ¸öÍâ¹ÛÒª²»Ò»Ñù
if is_in_act_time()== 1 then
TREETYPE = {	--Ä£°åÃû£¬NPCÃû×Ö£¬¼ä¸ôÊ±¼ä£¬ËùÐèÂ¶Ë®
		{"C©y Gi¸ng sinh 3d","MÇm c©y Th¸i H­ B¸t Nh· ",90,1},
		{"Xmas_Tree1","MÇm c©y Th¸i H­ B¸t Nh· ",90,1},
		{"C©y Gi¸ng sinh 3c","C©y Th¸i H­ B¸t Nh· nhá ",90,1},
		{"Xmas_Tree3","C©y Th¸i H­ B¸t Nh· lín ",90,1},
		{"C©y Gi¸ng sinh 3b","C©y hoa Th¸i H­ B¸t Nh· ",90,2},
		{"C©y Gi¸ng sinh 3a","C©y Th¸i H­ B¸t Nh· ",90,2},
		};
SMALLTREETYPE = {	--Ä£°åÃû£¬NPCÃû×Ö£¬¼ä¸ôÊ±¼ä£¬ËùÐèÂ¶Ë®
		{"C©y Gi¸ng sinh 2d","MÇm c©y Th¸i H­ nhá ",60,1},
		{"C©y Gi¸ng sinh 2c","MÇm c©y Th¸i H­ nhá ",60,1},
		{"C©y Gi¸ng sinh 2b","C©y Th¸i H­ nhá ",60,1},
		}
else
TREETYPE = {	--Ä£°åÃû£¬NPCÃû×Ö£¬¼ä¸ôÊ±¼ä£¬ËùÐèÂ¶Ë®
		{"MÇm hoa hång","MÇm c©y Th¸i H­ B¸t Nh· ",25,1},
		{"MÇm Hoa hång","MÇm c©y Th¸i H­ B¸t Nh· ",25,1},
		{" nh¸nh hoa hång","C©y Th¸i H­ B¸t Nh· nhá ",25,1},
		{"c©y §a MËt Hoa Qu¶ nhá","C©y Th¸i H­ B¸t Nh· lín ",25,1},
		{"c©y §a MËt Hoa Qu¶ trung","C©y hoa Th¸i H­ B¸t Nh· ",25,2},
		{"c©y §a MËt Hoa Qu¶ lín","C©y Th¸i H­ B¸t Nh· ",25,2},
		};
SMALLTREETYPE = {	--Ä£°åÃû£¬NPCÃû×Ö£¬¼ä¸ôÊ±¼ä£¬ËùÐèÂ¶Ë®
		{"MÇm hoa hång","MÇm c©y Th¸i H­ nhá ",30,1},
		{"MÇm Hoa hång","MÇm c©y Th¸i H­ nhá ",30,1},
		{" nh¸nh hoa hång","C©y Th¸i H­ nhá ",30,1},
		}
end

function check_npc_valid(nNpcIdx, nType)
	local tTreeInfo = ((nType == 1) and TREETYPE) or SMALLTREETYPE;
	local szNpcName = GetNpcName(nNpcIdx);
	local szNpcTemplate = GetNpcTemplate(nNpcIdx);
	local nIsValid = 0;
	for i=1, getn(tTreeInfo) do
		if tTreeInfo[i][1] == szNpcTemplate then
			nIsValid = 1;
			break
		end	
	end

...
function plant()
	if g_WCFG_BATNHALON:IsDateOpen(1) ~= WEnv.TRUE then
		return WEnv.RETCODE_NOTOPEN;
	end
	if g_WCFG_BATNHALON:IsCanDoAtTime(1) ~= WEnv.TRUE then
		return WEnv.RETCODE_OVERTIME;
	end
	if g_WCFG_BATNHALON.nLevelMinReq >= WPlayer:GetLevel() then
		WNpc:Talk(format("§¼ng cÊp yªu cÇu trªn %d míi cã thÓ tham gia trång c©y, h·y quay l¹i sau khi ®· ®¹t ®ñ cÊp nhÐ!", g_WCFG_BATNHALON.nLevelMinReq), 1);
		return WEnv.RETCODE_NEEDLEVEL;
	end
	local nPkFlag1, nPkFlag2 = GetPKFlag();
	if g_WCFG_BATNHALON.bPkReq == WEnv.TRUE and nPkFlag1 == 0 then
		WNpc:Talk("H¹t gièng cÇn thªm s¸t khÝ ®Ó ph¸t triÓn, bËt chiÕn ®Êu míi cã thÓ trång ®­îc c©y!", 1);

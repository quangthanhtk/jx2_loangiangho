...
-----ÉèÖÃÊ÷µÄ´æ»îÊ±¼ä----------------------------------------------------------------------------		
		...
		local nCount = GetTask(TASK_PLANT_TREE_COUNT)
		SetTask(TASK_PLANT_TREE_COUNT, GetTask(TASK_PLANT_TREE_COUNT) + 1);
		SetTask(TASK_PLANT_TREE_TIME, GetTime());
		Say("H«m nay, ®©y lµ lÇn thø <color=yellow>"..nCount.."<color> lÇn trång c©y, h«m nay cßn cã thÓ trång <color=yellow>"..PLANT_TREE_LIMIT - GetTask(TASK_PLANT_TREE_COUNT).."<color> c©y. NÕu nh­ " .. TREE_PARAM[1][2] .. " gi©y kh«ng t­íi n­íc, c©y sÏ tù ®éng chÕt ®i.", 0);
		FireEvent("event_mission_task_award", "planttree", "plantbigtree", nCount)
	end
end
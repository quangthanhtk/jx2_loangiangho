...
--°æ±¾ºÅ
QIANHE_VERSION = 5;
QIANHE_TASK_GROUP_VERSION = 27;
QIANHE_TASK_GROUP = 7;

--»î¶¯Ê±¼ä
QIANHE_START_TIME = 2021012500;
QIANHE_END_TIME = 2021021524;

--»î¶¯¿ªÊ¼Ê¹ÓÃÁÙÊ±ÈÎÎñ±äÁ¿£¬ÔÙ´Î¿ªÆôÒª×¢ÒâÁË
QHT_TASK_GROUP =  TaskManager:Create(QIANHE_TASK_GROUP, QIANHE_TASK_GROUP_VERSION);
...
function qht_get_date(nDate)
	local s = tostring(nDate)
	local y = strsub(s, 1, 4)
	local m = strsub(s, 5, 6)
	local d = strsub(s, 7, 8)
	return format("%s-%s-%s", y, m, d)
end

--»î¶¯²ÎÓëÌõ¼þ
function qht_check_condition(bTag)
	return gf_CheckBaseCondition(59,bTag);
end

--ÒªÇåÀíµÄ´ú±ÒÉÌµêÈÎÎñ±äÁ¿
tIBShopTasks = {
--    3391,
--    3392,
--    3393,
...
function clearIbShopTasks()
	local szLog = format("%s clear qianxuntower IBSHop task data", GetName())
	--print(szLog)
	WriteGsLog(6, szLog)
	for i=1,getn(tIBShopTasks) do
		SetTask(tIBShopTasks[i], 0);
	end
end

function fixIbShopTasks()
	local curDate = tonumber(date("%Y%m%d"))
	local fixDate = 20170226
	if curDate >= fixDate and curDate <= 20170308 and GetTask(2913) < fixDate then--ÐÞ¸´bug
		clearIbShopTasks()
	end
end
...
function qht_DailyTaskReSet()
	local bIsOpen = qht_activity_isopen();
	local nCurrVersion = GetTask(QHT_TASK_VERSION);
	if bIsOpen == 0 and nCurrVersion == QIANHE_VERSION then
		BigDelItem(2, 1, 30435, BigGetItemCount(2, 1, 30435))
		--BigDelItem(2, 1, 30438, BigGetItemCount(2, 1, 30438))
		for i = QHT_TASK_IBSHOP_BEGIN, QHT_TASK_IBSHOP_END do
			SetTask(i, 0);
		end
		clearIbShopTasks()
		SetTask(QHT_TASK_VERSION, 0);
		return 0;
	end
	if bIsOpen ~= 0 then
		if nCurrVersion ~= QIANHE_VERSION then
			BigDelItem(2, 1, 30435, BigGetItemCount(2, 1, 30435))
			--BigDelItem(2, 1, 30438, BigGetItemCount(2, 1, 30438))
			for i = QHT_TASK_IBSHOP_BEGIN, QHT_TASK_IBSHOP_END do
				SetTask(i, 0);
...
function qht_award_six(targetNpcIdx)
	...
	gf_TeamOperateEX(function ()
		...
		if gf_Judge_Room_Weight(2,1," ") == 1 and qht_map_allow(QHT_MAP_TABLE,nMapID) == 1 then
			qht_add_exp(6000000);
			gf_AddItemEx2({2,1,30435,5}, "M¶nh Hæ phï", "C«ng chiÕn Thiªn TÇm Th¸p", "", 0, 1);
			qht_add_zhenqi(200);
			qht_add_xiuwei(100);
			qht_add_shengwang(50);
			qht_add_shimen(50);
			gf_AddItemEx2({2,1,30368,30}, "Cæ Linh Th¹ch", "C«ng chiÕn Thiªn TÇm Th¸p", "", 0, 1);
			gf_AddItemEx2({2,1,30369,5}, "Cæ Linh Ngäc", "C«ng chiÕn Thiªn TÇm Th¸p", "", 0, 1);
			--¸ø½ðÎÆ»¢·û
			qht_award_jinwenhufu(1,100,100);
		end
	end)
...
function qht_award_seven(targetNpcIdx)
	...
	gf_TeamOperateEX(function ()
		...
		if gf_Judge_Room_Weight(2,1," ") == 1 and qht_map_allow(QHT_MAP_TABLE,nMapID) == 1 then
			...
			qht_add_shimen(50);
			gf_AddItemEx2({2,1,30368,30}, "Cæ Linh Th¹ch", "C«ng chiÕn Thiªn TÇm Th¸p", "", 0, 1);
			gf_AddItemEx2({2,1,30369,20}, "Cæ Linh Ngäc", "C«ng chiÕn Thiªn TÇm Th¸p", "", 0, 1);
			--¸ø½ðÎÆ»¢·û
			qht_award_jinwenhufu(1,100,100);
		end
	end)
...
--ºÚ±ªÍ¶µã¿ØÖÆ
function qht_award_heibao( targetNpcIdx )
	local nCount = GetMapTaskTemp(6023, QHT_MAPTEMPTASK_HEIBAO_MAX);
	qht_itmedice_roll({0, 105, 30029, 1, 3, -1, -1, -1, -1, -1, -1}, QHT_THROWDICE_CBFILE, "qht_throwdice_callback", 100, 60, qht_get_teamsize(),targetNpcIdx);
	SetMapTaskTemp(6023,QHT_MAPTEMPTASK_HEIBAO_MAX, nCount + 1);
end
--»Æ±ªÍ¶µã¿ØÖÆ
function qht_award_huangbao( targetNpcIdx )
	local nCount = GetMapTaskTemp(6023, QHT_MAPTEMPTASK_HUANGBAO_MAX);
	qht_itmedice_roll({0, 105, 30030, 1, 3, -1, -1, -1, -1, -1, -1}, QHT_THROWDICE_CBFILE, "qht_throwdice_callback", 100, 60, qht_get_teamsize(),targetNpcIdx);
	SetMapTaskTemp(6023,QHT_MAPTEMPTASK_HUANGBAO_MAX, nCount + 1);
end

--Ìì½¾ÁîÍ¶µã--¿ØÖÆ
function qht_award_tianjiao(targetNpcIdx)
	local nCount = GetMapTaskTemp(6023, QHT_MAPTEMPTASK_TIANJIAO_MAX);
	qht_itmedice_roll({2,97,236,2}, QHT_THROWDICE_CBFILE, "qht_throwdice_callback", 100, 60, qht_get_teamsize(),targetNpcIdx);
	SetMapTaskTemp(6023,QHT_MAPTEMPTASK_TIANJIAO_MAX, nCount + 2);
end
...
function qht_award_resettable(tbTemlate)
	...
	for i = 1, getn(tbTemlate) do
		local bFlag = nil;
		if tbTemlate[i][3] == "qht_award_zuoji(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_YJZUOJI_MAX) >= QHT_YJZUOJI_MAX then
				bFlag = 1;
			end
		end
		if tbTemlate[i][3] == "qht_award_tiangang(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_TIANGANG_MAX) >= QHT_TIANGANG_MAX then
				bFlag = 1;
			end
		end
		if tbTemlate[i][3] == "qht_award_tianmen(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_TIANMEN_MAX) >= QHT_TIANMEN_MAX then
				bFlag = 1;
			end
		end
		if tbTemlate[i][3] == "qht_award_tianjiao(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_TIANJIAO_MAX) >= QHT_TIANJIAO_MAX then
				bFlag = 1;
			end
		end

		if tbTemlate[i][3] == "qht_award_heibao(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_HEIBAO_MAX) >= QHT_HEIBAO_MAX then
				bFlag = 1;
			end
		end

		if tbTemlate[i][3] == "qht_award_huangbao(%d)" then
			if GetMapTaskTemp(6023,QHT_MAPTEMPTASK_HUANGBAO_MAX) >= QHT_HUANGBAO_MAX then
				bFlag = 1;
			end
		end
...
function qht_award_throwdice(tbAward,targetNpcIdx)
	local t = qht_award_resettable(tbAward)
	if not t then return 0; end
	local nRandIndex = gf_GetRandItemByTable(t, gf_SumRandBase(t), 1);
	if t[nRandIndex] then
		if t[nRandIndex][1] == 1 then
				qht_itmedice_roll(t[nRandIndex][4], QHT_THROWDICE_CBFILE, "qht_throwdice_callback", 100, 60, qht_get_teamsize(),targetNpcIdx, tbAward[nRandIndex][5]);
		elseif t[nRandIndex][1] == 31 then
				dostring(format(t[nRandIndex][3],targetNpcIdx));
		end
	else
...
function qht_award_jinwenhufu(nNum, nProbability, nBaseMax, bBind)
	qht_DailyTaskReSet()--ÏÈÅÐ¶ÏÊÇ·ñÐèÒªÖØÖÃ
...
function qht_ds_goto_check()
	...
	for i = 1, gf_GetTeamSize() do
		PlayerIndex = gf_GetTeamMember(i);
		if gf_CheckBaseCondition(59,1) ~= 1 then
			msg = msg.."<color=red>"..GetName().."<color>\n";
			nNum = nNum + 1;
		end
	end
...
function qht_award_tong_fight_flag()
	...
	--gf_AddItemEx2({2,1,30369,1}, "¹ÅÁéÓñ", "¼¤Õ½Ç§Ñ°Ëþ", "", 0, 1);
...
function qht_award_tong_fight_flag_tiexue()
	...
	--gf_AddItemEx2({2,1,30369,2}, "¹ÅÁéÓñ", "¼¤Õ½Ç§Ñ°Ëþ", "", 0, 1);

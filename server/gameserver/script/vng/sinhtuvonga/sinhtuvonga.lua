...
function STVN2_NhanNhiemVu()
	...
	local tRoute = {[2]=1,[3]=1,[4]=1,[6]=1,[8]=1,[9]=1,[11]=1,[12]=1,[14]=1,[15]=1,[17]=1,[18]=1,[20]=1,[21]=1,[23]=1,[25]=1,[26]=1,[27]=1,[29]=1,[30]=1}
...
function MatTichCaoCap70(nSoluong)
	local tbMatTich = {
		...
		[25] = {item={{gdp={0,107,219,nSoluong}, name ="Cöu Thiªn Phong L«i Ch©n QuyÓn"}}},
		[26] = {item={{gdp={0,107,220,nSoluong}, name ="Cöu Thiªn Phong L«i Ch©n QuyÓn"}}},
		[27] = {item={{gdp={0,107,221,nSoluong}, name ="Cöu Thiªn Phong L«i Ch©n QuyÓn"}}},
...
function TrangBiHaoNguyet()
	...
	local tbVuKhiHaoNguyet = {
		...
		[25] = {item={{gdp={0,2,8987,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "H¹o NguyÖt-BÊt §éng Ch©n Tiªn KiÕm"}}},
		[26] = {item={{gdp={0,2,8988,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "H¹o NguyÖt-BÊt §éng Ch©n Tiªn KiÕm"}}},
		[27] = {item={{gdp={0,2,8989,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "H¹o NguyÖt-BÊt §éng Ch©n Tiªn KiÕm"}}},
...
function VukhiDieuDuong()
	local tbVukhiDieuDuong = {
		...
		[25]={item={{gdp={0,2,9007,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "DiÖu D­¬ng-BÊt §éng Ch©n Tiªn KiÕm"}}},
		[26]={item={{gdp={0,2,9008,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "DiÖu D­¬ng-BÊt §éng Ch©n Tiªn KiÕm"}}},
		[27]={item={{gdp={0,2,9009,1,1,-1,-1,-1,-1,-1,-1,-1,0}, name = "DiÖu D­¬ng-BÊt §éng Ch©n Tiªn KiÕm"}}},
...
function MatTichTranPhai()
	local tbMatTich = {
		...
		[25] = {item={{gdp={0,112,193,1}}}},
		[26] = {item={{gdp={0,112,194,1}}}},
		[27] = {item={{gdp={0,112,195,1}}}},
...
--=========Sinh Tö V« Ng· 3
function STVN3_NhanNhiemVu(nType)
	local tCondition = {
					[1] = {599, {2,1,199,5}, " §¹i Ng©n PhiÕu", 500},
					[2] = {696, {2,1,30490,5}, " Hoµng Kim §¹i Ng©n PhiÕu", 5000},
					[3] = {697, {2,1,30490,3}, " Hoµng Kim §¹i Ng©n PhiÕu", 3000},
					[4] = {698, {2,1,30490,2}, " Hoµng Kim §¹i Ng©n PhiÕu", 1500},
					[5] = {699, {2,1,199,1}, " §¹i Ng©n PhiÕu", 500},
				}
	local nChuyenSinh = (GetTranslifeCount() + GetPlayerRebornParam(0)) * 100 + GetLevel()
	if GetTask(TSK_SINHTULAN3A092015) >= nType then
		Talk(1, "", szNpcName.."C¸c h¹ ®· nhËn nhiÖm vô Sinh Tö V« Ng· lÇn 3 råi.")
		return 0
	end
	if nChuyenSinh < tCondition[nType][1] then
		Talk(1, "", szNpcName.."§¼ng cÊp chuyÓn sinh cña c¸c h¹ qu¸ thÊp, kh«ng ®¹t ®ñ yªu cÇu.")
		return 0
	end
	if nChuyenSinh > tCondition[nType][1] then
		Talk(1, "", szNpcName.."§¼ng cÊp chuyÓn sinh cña c¸c h¹ qu¸ cao, kh«ng thÝch hîp víi nhiÖm vô nµy.")
		return 0
	end
	if GetItemCount(tCondition[nType][2][1],tCondition[nType][2][2],tCondition[nType][2][3]) < tCondition[nType][2][4] then
		Talk(1, "", szNpcName.."C¸c h¹ kh«ng mang theo ®ñ "..tCondition[nType][2][4]..tCondition[nType][3]..". Xin h·y kiÓm tra l¹i")
		return 0
	end	
	if GetCash() < tCondition[nType][4]*10000 then
		Talk(1, "", szNpcName.."C¸c h¹ kh«ng mang ®ñ ng©n l­îng.")
		return 0
	end
	if DelItem(tCondition[nType][2][1],tCondition[nType][2][2],tCondition[nType][2][3],tCondition[nType][2][4]) ~= 1 or Pay(tCondition[nType][4]*10000) ~= 1 then
		gf_WriteLogEx(szLogTitle, "xãa thÊt b¹i", 1, "Xãa vËt phÈm thÊt b¹i cÊp "..tCondition[nType][1])
		return 0
	end
	SetTask(TSK_SINHTULAN3A092015, 0)
	SetTask(TSK_SINHTULAN3B092015, 0)
	SetTask(TSK_SINHTULAN3C092015, 0)
	gf_WriteLogEx(szLogTitle, szLogAction, 1, "Sinh Tö 3 cÊp "..tCondition[nType][1])
	VNG_SetTaskPos(TSK_SINHTULAN3A092015, nType, 1, 1)
	
end
function STVN3_LamNhiemVu(nType)
	local nCheckTime = CheckTime_SinhTu3()
	local nCheckStatus = CheckStatus_SinhTu3()
	if nCheckTime ~= 1 then
		return 0
	end
	if nCheckStatus ~= 1 then
		return 0
	end
	local nTypeMission = VNG_GetTaskPos(TSK_SINHTULAN3A092015,1,1) 
	local tbSTVN3config = tbSTVN3config1
	if nTypeMission > 1 then
		tbSTVN3config = tbSTVN3config2
	end
	local nCheckSoLuong = VNG_GetTaskPos(tbSTVN3config[nType][1],tbSTVN3config[nType][4],tbSTVN3config[nType][5]) 
	if nCheckSoLuong < tbSTVN3config[nType][3] then
		VNG_SetTaskPos(tbSTVN3config[nType][1], nCheckSoLuong + 1,tbSTVN3config[nType][4],tbSTVN3config[nType][5]) 
		Msg2Player("NhiÖm vô Sinh Tö V« Ng· 3: Hoµn thµnh 1 lÇn nhiÖm vô "..tbSTVN3config[nType][2])
		TaskTip("NhiÖm vô Sinh Tö V« Ng· 3: Hoµn thµnh 1 lÇn nhiÖm vô "..tbSTVN3config[nType][2])
		gf_WriteLogEx(szLogTitle, "hoµn thµnh", 1, "Hoµn thµnh 1 nhiÖm vô "..tbSTVN3config[nType][2])
	else
		return 0
	end
end
function STVN3_HoanThanhNhiemVu()
	local nCheckStatus = CheckStatus_SinhTu3()
	if nCheckStatus < 1 then
		Talk(1, "", szNpcName.."C¸c h¹ ch­a nhËn nhiÖm vô Sinh Tö V« Ng· lÇn 3.")
		return 0
	end
	if nCheckStatus >= 2 then
		Talk(1, "", szNpcName.."C¸c h¹ ®· hoµn thµnh nhiÖm vô Sinh Tö V« Ng· lÇn 3.")
		return 0
	end
	local nTypeMission = VNG_GetTaskPos(TSK_SINHTULAN3A092015,1,1) 
	local tbSTVN3config = tbSTVN3config1
	local nCheckFinisMission = 0
	local nCheckFinisMission1 = 0
	if nTypeMission > 1 then
		tbSTVN3config = tbSTVN3config2
	end
	for i=1, getn(tbSTVN3config) do
		local nCheckOneMission = VNG_GetTaskPos(tbSTVN3config[i][1],tbSTVN3config[i][4],tbSTVN3config[i][5])
		if nCheckOneMission < tbSTVN3config[i][3] then
			nCheckFinisMission = 1
			break
		end
	end
	if nTypeMission > 1 then	
		for i = 1, 4 do 
			local ncheckii = i + 2
			local nCheckOneMission1 = VNG_GetTaskPos(TSK_SINHTULAN3C092015,ncheckii,ncheckii)
			if nCheckOneMission1 < 1 then	--=1 lµ ®· lµm xong nhiÖm vô
				nCheckFinisMission1 = 1
				break
			end		
		end	
	end
	if nTypeMission > 1 then
		if nCheckFinisMission == 1 or nCheckFinisMission1 == 1 then
			Talk(1, "", szNpcName.."C¸c h¹ ch­a hoµn thµnh hÕt c¸c nhiÖm vô yªu cÇu. H·y kiÓm tra l¹i!!!")
			return 0
		end	
	else
		if nCheckFinisMission == 1 then
			Talk(1, "", szNpcName.."C¸c h¹ ch­a hoµn thµnh hÕt c¸c nhiÖm vô yªu cÇu. H·y kiÓm tra l¹i!!!")
			return 0
		end	
	end
	STVN3_HoanThanhNhiemVu_chonHuong()
end
function STVN3_HoanThanhNhiemVu_chonPhe()
	do return 0 end
	local szSayHead = "Khi luyÖn Sinh Tö V« Ng·, c¸c h¹ sÏ ®o¹n tuyÖt qu¸ khø, trë thµnh ng­êi hoµn toµn míi. H·y chän l¹i phe ph¸i cña m×nh."
	local tbSay = {}
	tinsert(tbSay, "Ta sÏ gia nhËp phe Tèng/#STVN3_HoanThanhNhiemVu_chonHuong(1)")
	tinsert(tbSay, "Ta sÏ gia nhËp phe Liªu/#STVN3_HoanThanhNhiemVu_chonHuong(2)")
	tinsert(tbSay, "Ta muèn nghÜ kü h¬n/gf_DoNothing")
	Say(szSayHead, getn(tbSay), tbSay)
end
function STVN3_HoanThanhNhiemVu_chonHuong()
	if IsPlayerDeath() ~= 0 then 
	...
	local tRoute = {[2]=1,[3]=1,[4]=1,[6]=1,[8]=1,[9]=1,[11]=1,[12]=1,[14]=1,[15]=1,[17]=1,[18]=1,[20]=1,[21]=1,[23]=1,[25]=1,[26]=1,[27]=1,[29]=1,[30]=1}
	if tRoute[nRoute] ~= 1 then
		Talk(1, "", szNpcName.."Ch­a gia nhËp hÖ ph¸i, kh«ng thÓ tham gia ho¹t ®éng nµy.");
		return 0
	end	
	local tSay = 	{
		"Ta muèn tu luyÖn theo h­íng Long Tö!/#SinhTu3_select_translife(1)",
		"Ta muèn tu luyÖn theo h­íng Hæ Tö!/#SinhTu3_select_translife(2)",
		"Ta muèn tu luyÖn theo h­íng ¦ng Tö!/#SinhTu3_select_translife(3)",
		"Ta muèn tu luyÖn theo h­íng Phông Tö!/#SinhTu3_select_translife(4)",
		"Ta suy nghÜ l¹i ®·./gf_DoNothing",
	}
	Say("<color=yellow>Hçn Tôc Hßa Quang<color> hÊp thu linh khÝ ©m d­¬ng, v¹n vËt sinh diÖt ®Òu cã thÓ khèng chÕ, ng­¬i muèn tu luyÖn theo h­íng nµo?",getn(tSay),tSay)
end
function SinhTu3_select_translife(nType)
	if IsPlayerDeath() ~= 0 then 
		Talk(1, "", szNpcName.."HiÖn t¹i kh«ng thÓ nhËn th­ëng!!!")
		return 0
	end
	local nTypeMission = VNG_GetTaskPos(TSK_SINHTULAN3A092015,1,1) 
	local nCheckMission = CheckStatus_SinhTu3()
	local nCheckFinisMission = 0
	if nCheckMission == 0 then
		Talk(1, "", szNpcName.."C¸c h¹ ch­a nhËn nhiÖm vô!!!")
		return 0
	end
	local tbSTVN3config = tbSTVN3config1
	if nTypeMission > 1 then
	...
	gf_WriteLogEx(szLogTitle, "hoµn thµnh", 1, "Hoµn thµnh sinh tö v« ng· lo¹i:  "..nTypeMission)
	VNG_SetTaskPos(TSK_SINHTULAN3A092015, 6, 1, 1) --set gi¸ trÞ x¸c nhËn ®· hoµn thµnh xong nhËn th­ëng, function CheckStatus_SinhTu3() tr¶ vÒ gi¸ trÞ 2
	local tb_translife_seal = {
	...
	if nTypeMission == 1 then
		local nExp = GetExp()
		big_del_exp(nExp) --set ®iÓm kinh nghiÖm vÒ 0
		PlayerReborn(1,nType)	--Phôc sinh 1
		local nType_cs5 = GetTranslifeFaction()	
			local nTransCount = GetTranslifeCount()	
			local nRoute = GetPlayerRoute()
			for i=1, getn(tb_translife_cloth[nType_cs5][nTransCount]) do
			...
			TraoChienCuong(0)
			gf_AddItemEx(tb_translife_seal[nType][2], tb_translife_seal[nType][1])
			Say("Chóc mõng ®¹i hiÖp ®· hoµn thµnh Sinh Tö V« Ng· !!!!! H·y ®¨ng nhËp l¹i.",1,"Tho¸t!/relogin_stvn3")
	else
		...
		local nExp = GetExp()
		big_del_exp(nExp) --set ®iÓm kinh nghiÖm vÒ 0
		PlayerReborn(2,nType) --Phôc sinh 2
		Say("Chóc mõng ®¹i hiÖp ®· hoµn thµnh Sinh Tö V« Ng· !!!!! H·y ®¨ng nhËp l¹i.",1,"Tho¸t!/relogin_stvn3")
	end
	
end
function STVN3_TienDoNhiemVu()
	local nCheckStatus = CheckStatus_SinhTu3()
	if nCheckStatus < 1 then
		Talk(1, "", szNpcName.."C¸c h¹ ch­a nhËn nhiÖm vô Sinh Tö V« Ng· lÇn 3.")
		return 0
	...
	for i=1, getn(tbSTVN3config) do
		szMsg = szMsg .. tbSTVN3config[i][2]..": ®­îc <color=red>(".. VNG_GetTaskPos(tbSTVN3config[i][1],tbSTVN3config[i][4],tbSTVN3config[i][5]) .."/"..tbSTVN3config[i][3]..")<color> lÇn.\n"
	end
	Talk(1,"",szMsg)
end

function STVN_submit_nguyenlieu(nType)
	local tbNguyenlieu = {--vÞ trÝ check ®· nép, 
						[1] = {3, {2,1,1006,1}, "Cöu Thiªn V« Cùc §¬n"},			--Cöu Thiªn V« Cùc §¬n
						[2] = {4, {2,1,30916,20000}, "Bét M×"},	--Bét M×
						[3] = {5, {2,1,30917,20000}, "Nh©n B¸nh"},	--Nh©n B¸nh
						[4] = {6, {2,1,30918,10000}, "Nh©n B¸nh ThËp CÈm"},	--Nh©n B¸nh ThËp CÈm
							}
	local nCheck = VNG_GetTaskPos(TSK_SINHTULAN3C092015,tbNguyenlieu[nType][1],tbNguyenlieu[nType][1]) 						
	if nCheck == 1 then
		Talk(1, "", szNpcName.."C¸c h¹ ®· nép nhiÖm vô nµy råi.")
		return 0
	end			
	if GetItemCount(tbNguyenlieu[nType][2][1],tbNguyenlieu[nType][2][2],tbNguyenlieu[nType][2][3]) < tbNguyenlieu[nType][2][4] then
		Talk(1, "", szNpcName.."C¸c h¹ kh«ng mang theo ®ñ sè l­îng yªu cÇu.")
		return 0
	end
	if DelItem(tbNguyenlieu[nType][2][1],tbNguyenlieu[nType][2][2],tbNguyenlieu[nType][2][3],tbNguyenlieu[nType][2][4]) ~= 1 then
		return 0
	end
	gf_WriteLogEx(szLogTitle, szLogAction, 1, "Sinh Tö 3: xãa thµnh c«ng lo¹i: "..tbNguyenlieu[nType][3])
	VNG_SetTaskPos(TSK_SINHTULAN3C092015, 1, tbNguyenlieu[nType][1],tbNguyenlieu[nType][1]) 		
end

function CheckStatus_SinhTu3()
	local nTypeMission = VNG_GetTaskPos(TSK_SINHTULAN3A092015,1,1) 
	if nTypeMission >= 1 and nTypeMission <= 5 then	--®· nhËn nhiÖm vô Sinh Tö 3
		return 1
	end
	if nTypeMission > 5 then
		return 2
	end
	return 0
end

function CheckTime_SinhTu3()
	local nDate = tonumber(date("%Y%m%d"))

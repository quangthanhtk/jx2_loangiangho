...
--	VBonus_NuOa()
	...
	WPlayerLog:WriteBattleLog("Use_Award_NuOa", "Sö dông N÷ Oa B¶o H¹p", 0, "", nExpBefore, nExp, nHonorBefore, nQuanCong, nPointBefore, 0);
...
function Use_Award_NgoiSao()
	...
	if WTask:Get(WTASKID_USE_NGOISAO_DAILY) >= nMax  or mod(GetTask(TSK_LUCKY_STAR_201011), 100) > nMax then
		WNpc:Talk(format("§· nhËn tèi ®a %d phÇn th­ëng tõ Ng«i Sao May M¾n, kh«ng thÓ nhËn thªm!", nMax));
		return 0;
	end
...
--	AwardGenuineQi(nExpChanKhi)
	...
	WPlayerLog:WriteBattleLog("Use_Award_NgoiSao", "Sö dông Ng«i Sao May M¾n", 0, "", nExpBefore, 0, nHonorBefore, nQuanCong, nPointBefore, 0);
...
function ThuHoachBatNhaLon()
	--LongDaiKa: bá phÇn th­ëng b¸t nh· lín cò
	--local nDate = tonumber(date("%y%m%d"))
	--local nVang = 160000
	...
	FireEvent("event_mission_task_award","planttree","plantbigtree",16)
	--VBonus_BatNhaLon()--LongDaiKa: bá phÇn th­ëng b¸t nh· lín cò
	--============nhiÖm vô Book15

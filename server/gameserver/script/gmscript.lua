...
Include("\\script\\online\\zgc_public_fun.lua") --´å³¤µÄ¹«¹²´¦Àíº¯Êý
...
function _w_r2s_CreateBossTnc()
	WBOSS_TNC:CreateBoss();
end

function _w_event2024_CreateBossEvent2024()
	--CreateBossEvent2024();
end
...
--server¸üÐÂCharm RankÅÅÃû¡£Ã¿ÈÕ0£º05´¥·¢
...
function one_road_battle_alarm()
	...
	local msg2PrepareField = function(curMin)
		local msg = format("ChiÕn tr­êng HiÖp Lé T­¬ng Phïng sÏ më sau %d phót, c¸c hiÖp sÜ h·y chuÈn bÞ tham gia!", 15 - curMin)
		if 15 - curMin == 0 then 
			msg = "ChiÕn tr­êng HiÖp Lé T­¬ng Phïng s¾p b¾t ®Çu, c¸c hiÖp sÜ h·y chuÈn bÞ tham gia!"
		end
	...
	if nCurGs ~= 0 then -- ¿ç·þ
		...
		if min == 0 or min == 5 or min == 10 then
			if SubWorldID2Idx(100) >= 0 then 
				Msg2Global(format("ChiÕn tr­êng HiÖp Lé T­¬ng Phïng sÏ më sau %d phót, c¸c hiÖp sÜ hay mau chãng ®Õn liªn server [HiÖp Lé T­¬ng Phïng Tham T­íng] tham gia!", 15 - min))
			end
		end
	end
end

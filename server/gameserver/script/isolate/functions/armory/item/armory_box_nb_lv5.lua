...
function Confirm(nSel, nItemIndex)
	...
	local szTitle = format("<color=green>[%s]<color>: Dïng sÏ nhËn ®­îc1 V¨n Søc cã ngò hµnh vµ thuéc tÝnh chØ ®Þnh <color=yellow>[%s][+%d][%s]<color>.\n\n", tItem[1], g_tStringTable.tLevel[tItem[5]], tItem[6], g_tStringTable.tQuality[tItem[7]]);
	local tMenu = {
...
function MakeArmory_Armory(nSel, nStep, nValue)
	...
	if not nValue then
		local szMsg = format("<color=green>[%s]<color> Mêi lùa chän vÞ trÝ\n\n", tItem[1]);
		local tMenu = {
			format("\nTrë l¹i	/#MakeArmory(%d, %d)", nSel, nStep - 1),
			"Rêi khái	/nothing",
		};
...
function MakeArmory_Suit(nSel, nStep, nPageIdx, nValue)
	...
	local nLinePerPage = 9;
	if not nValue then
		local szMsg = format("<color=green>[%s]<color>:%s\n\n", tItem[1], "§¹i hiÖp h·y chän bé m×nh muèn: <color=gold>Ph­îng V¨n lµ bé phßng ngù, Long V¨n lµ bé tÊn c«ng.<color>");
		local tMenu = {
			format("\nTrë l¹i	/#MakeArmory(%d, %d)", nSel, nStep - 1),
			"Rêi khái	/nothing",
		};
		...
		if getn(tItem[10]) > (nPageIdx + 1) * nLinePerPage then
			tinsert(tMenu, 1, format("%s/#MakeArmory_Suit(%d, %d, %d)", "Trang kÕ", nSel, nStep, nPageIdx + 1));
		end

		if 0 < nPageIdx then
			tinsert(tMenu, 1, format("%s/#MakeArmory_Suit(%d, %d, %d)", "Trang tr­íc", nSel, nStep, nPageIdx - 1));
		end
		...
		for i = 1, nLinePerPage do
			...
			tinsert(tMenu, 1, format("@ Thuéc tÝnh bé %s :%s/#MakeArmory_Suit(%d, %d, %d, %d)", szSuitName, szSuitAttrDes, nSel, nStep, nPageIdx, nSuitIdx));
		end
...
function MakeArmory_Confirm(nSel, nStep, nValue)
	...
	if not nValue then
		...
		szMsg = szMsg .. format("\t\t<color=gold>Bé - %s<color>\n", g_tStringTable.tSuitName[tData.nSuitIdx]);

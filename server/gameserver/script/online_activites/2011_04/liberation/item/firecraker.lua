...
--	end
	...
	local tbSayDialog = {};
	local nSaySize = 0;
	local szSayHead = "Th«ng tin chi tiÕt ho¹t ®éng ®ång ®¹o cã thÓ xem trªn trang chñ.";
	if nCheck == 0 then
--		if nDate >= 150720 and nDate <= 150810 then
			tinsert(tbSayDialog, "Ta muèn x 12 kinh nghiÖm (tiªu hao 199 vµng)/#use_firework("..nItemIndex..", 3)")
			tinsert(tbSayDialog, "Ta muèn x 6 kinh nghiÖm (tiªu hao 89 vµng)/#use_firework("..nItemIndex..", 2)")
--		end
		tinsert(tbSayDialog, "Ta chØ sö dông b×nh th­êng/#use_firework("..nItemIndex..", 1)")

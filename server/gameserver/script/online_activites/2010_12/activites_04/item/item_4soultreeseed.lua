...
--		end
		...
		local nShowUsed = nUsed + 1 
		Msg2Player("Sè l­îng c©y tø linh trång trong ngµy: "..nShowUsed.."/"..nMax4SouldTree)
	...
	--Say("H«m nay, ®©y lµ lÇn thø <color=yellow>"..GetTask(VET_201012_04_PLANT_4_SOUL_TREE_COUNT).."<color> b¹n trång c©y Tø Linh,  h«m nay b¹n vÉn cã thÓ trång <color=yellow>"..(VET_4_SOUL_MAX_PLANT_TIMES-GetTask(VET_201012_04_PLANT_4_SOUL_TREE_COUNT)).."<color> c©y Tø Linh.",0);		        
        ...
        FireEvent("event_mission_task_award", "planttree", "plant4lingtree", 1)
        SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(1302)");

        SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(2212)");
    end
end

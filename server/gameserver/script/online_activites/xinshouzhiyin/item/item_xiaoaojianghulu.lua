...
--	tinsert(tSay, "Tham gia tÝnh n¨ng Vâ L©m B¶o H¹p/vlbh_active")
	...
	tinsert(tSay,"Mua 10 B×nh Tiªn Lé = 30 Xu/BuyTienLo")
	tinsert(tSay,"Mua 1 C©y Tø Linh = 10 Xu/BuyCayTuLinh")
	tinsert(tSay,"Mua 1 ThÎ Niªn Thó = 20 Xu/BuyNienThuCard")
...
function BuyTienLo()
	...
	local tbConsume = {
		tbItems = {
			{ tbProp = { 2, 1, 30230 }, nCount = 30 }, -- Xu vËt phÈm
		}
	};
	if WLib:PayMaterial(tbConsume) == WEnv.TRUE then
		gf_AddItemEx({2, 1, 30274, 10}, "B×nh tiªn lé")
	else
		return WEnv.RETCODE_CONSUMEERROR;
	end
	SayPage1()
end
function BuyCayTuLinh()
	if gf_Judge_Room_Weight(1, 10, " ") == 0 then
		return
	end
	local tbConsume = {
		tbItems = {
			{ tbProp = { 2, 1, 30230 }, nCount = 10 }, -- Xu vËt phÈm
		}
	};
	if WLib:PayMaterial(tbConsume) == WEnv.TRUE then
		gf_AddItemEx({2, 1, 40134, 1}, "C©y Tø Linh")
	else
		return WEnv.RETCODE_CONSUMEERROR;
	end
	SayPage1()
end
function BuyNienThuCard()
	if gf_Judge_Room_Weight(1, 10, " ") == 0 then
		return
	end
	local tbConsume = {
	...
	if WLib:PayMaterial(tbConsume) == WEnv.TRUE then
		gf_AddItemEx({2, 1, 40146, 1}, "ThÎ Gäi Niªn Thó")
	else
		return WEnv.RETCODE_CONSUMEERROR;
	end
	SayPage1()
...
function get_tinhnhan()
	local tbSayDialog = {};
	local nSaySize = 0;
	local szSayHead = "Th«ng tin chi tiÕt c¸c ho¹t ®éng ®ång ®¹o cã thÓ xem thªm t¹i trang chñ."

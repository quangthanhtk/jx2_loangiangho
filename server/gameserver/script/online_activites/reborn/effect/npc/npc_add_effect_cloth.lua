...
function cloth_effect_add_final(nEquipPos, nLevel, nEffectType)
    ...
    if getn(tbAtrr) == 3 then
        nAddFlag,nItemIndex = AddItem(nGenreId, nGetailId, nParticularId, 1, 1, tbAtrr[1].attr_id_level[nLevel], tbAtrr[1].attr_id[nLevel], tbAtrr[2].attr_id_level[nLevel], tbAtrr[2].attr_id[nLevel], tbAtrr[3].attr_id_level[nLevel], tbAtrr[3].attr_id[nLevel]);
        SetItemExpireTime(nItemIndex, 30*24*3600);
    end
    if nAddFlag == 1 then
...
function del_cloth_effect_final(nEquipPos)
    ...
    local nEquipIndex = GetPlayerEquipIndex(nEquipPos);
    local nGenreId, nGetailId, nParticularId = GetItemGenre(nEquipIndex), GetItemDetail(nEquipIndex), GetItemParticular(nEquipIndex);
    local nExpireTime = GetItemExpireTime(nEquipIndex) or 0
    if DelItemByIndex(nEquipIndex, -1) ~= 1 then
    ...
    --add外装
    local _,idx = AddItem(nGenreId, nGetailId, nParticularId, 1, 1);
    if nExpireTime > 0 and idx > 0 then --保留时限
    	SetItemExpireTime(idx, nExpireTime);
    end

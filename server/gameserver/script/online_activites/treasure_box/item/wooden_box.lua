...
function OnUse(nItem)
	...
	local nMocRuong =  35	--GetItemCount(2,1,30340)	--Méc R­¬ng
	local nXu = 15	--GetItemCount(2,1,30230)	--Xu vËt phÈm
	...
	if GetItemCount(2,1,30340) < nMocRuong then
		...
		Talk(1,"","Sè l­îng Méc R­¬ng trong hµnh trang thiÕu hiÖp kh«ng ®ñ <color=green>"..nMocRuong.."<color>")
		return 0
	end	
	if GetItemCount(2,1,30230) < nXu then
		Talk(1,"","Sè l­îng Xu vËt phÈm trong hµnh trang thiÕu hiÖp kh«ng ®ñ <color=green>"..nXu.."<color>")
		return 0
	end
	local nTitle = "X¸c ®Þnh n©ng cÊp Méc R­¬ng? (N©ng cÊp lªn Tµng R­¬ng cÇn tiªu phÝ <color=green>"..nMocRuong.."<color> Méc R­¬ng vµ <color=green>"..nXu.."<color> Xu vËt phÈm)"
	Say(nTitle,2,"X¸c ®Þnh n©ng cÊp (tèn "..nMocRuong.." Méc R­¬ng + "..nXu.." Xu vËt phÈm)/to_upgrade","Kh«ng cÇn ®©u/do_nothing")
end

local FilePath = "\\script\\pet\\pet_get_booklv7.lua"


function pet_getbooklv7()
    PutinItemBox("§æi Kim Ngäc §iÖp" ,1 , "CÇn dïng 1 MËt TÞch Thó C­ng CÊp 7 bÊt kú vµ 200 vµng  ®æi 100 Kim Ngäc DiÖp, x¸c nhËn ®æi?", %FilePath, 1)
end
...
function OnPutinCheck(param, idx, genre, detail, particular)
    ...
    if genre ~= 2 or detail ~= 150 or particular < 107 or particular > 126 then
        Talk(1, "", "ChØ ®­îc ®Æt vµo MËt TÞch Thó C­ng CÊp 7.");
        return 0;
    end
    return 1;
end
...
function get_item_count(t)
    ...
    for i=1,getn(t) do
        ...
        if t[i][2]==2 and t[i][3]==150 and t[i][4]>=107 and t[i][4]<=126 then
            nCnt = nCnt + 1;
        end
    end
    return nCnt;
...
function OnPutinComplete(param)
    ...
    if nResult == 1 and Pay(2000000) == 1 then
    --Ìí¼ÓÐÂÃØ¼®
        --CustomAwardGiveGroup("5ji_huoban_book", 0)
        AddItem(2,1,30729 ,100)

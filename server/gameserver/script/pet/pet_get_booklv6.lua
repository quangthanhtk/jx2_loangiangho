local FilePath = "\\script\\pet\\pet_get_booklv6.lua"


function pet_getbooklv6()
    PutinItemBox("§æi Kim Ngäc §iÖp" ,1 , "CÇn dïng 1 MËt TÞch Thó C­ng CÊp 6 bÊt kú vµ 100 vµng  ®æi 50 Kim Ngäc DiÖp, x¸c nhËn ®æi?", %FilePath, 1)
end
...
function OnPutinCheck(param, idx, genre, detail, particular)
    ...
    if genre ~= 2 or detail ~= 150 or particular < 87 or particular > 106 then
        Talk(1, "", "ChØ ®­îc ®Æt vµo MËt TÞch Thó C­ng CÊp 6.");
        return 0;
    end
    return 1;
end
...
function get_item_count(t)
    ...
    for i=1,getn(t) do
        ...
        if t[i][2]==2 and t[i][3]==150 and t[i][4]>=87 and t[i][4]<=106 then
            nCnt = nCnt + 1;
        end
    end
    return nCnt;

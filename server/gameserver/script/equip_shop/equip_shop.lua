...
Import("\\script\\ksgvn\\define\\define_shop.lua"); -- import ®Ó sö dông define shop id
...
tEquipShopDate = {
	...
	[3024] = {beginDate = 20210125, endDate = 20210215},	-- Shop Thiªn TÇm Th¸p
	[3069] = {beginDate = 20160423, endDate = 20170430},
	[3036] = {beginDate = 20190101, endDate = 20190101},	-- §ãng shop lÖnh bµi bang héi
	[EQUIP_SHOP_EXCHANGE_SATTINH] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp S¸t Tinh
	[EQUIP_SHOP_EXCHANGE_PHAHON] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp Ph¸ Hån
	[EQUIP_SHOP_EXCHANGE_TONG_UHT] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp Uy Hæ T­íng
	[EQUIP_SHOP_EXCHANGE_LIEU_UHT] = {beginDate = 20190101, endDate = 20290101},	--LongDaiKa: §ãng n©ng cÊp Uy Hæ T­íng
	[EQUIP_SHOP_EXCHANGE_TONG_CTT] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp Chu T­íc T­íng
	[EQUIP_SHOP_EXCHANGE_LIEU_CTT] = {beginDate = 20190101, endDate = 20290101},	--LongDaiKa: §ãng n©ng cÊp Chu T­íc T­íng
	--[EQUIP_SHOP_EXCHANGE_KIMXA7] = {beginDate = 20190101, endDate = 20190101},	--LongDaiKa: §ãng n©ng cÊp Kim Xµ 7
	[EQUIP_SHOP_EXCHANGE_KIMXA8] = {beginDate = 20190101, endDate = 20290101},	--LongDaiKa: §ãng n©ng cÊp Kim Xµ 8
	[EQUIP_SHOP_EXCHANGE_KHACBAN_HOANMY_LV4] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp V¨n Søc 4
	[EQUIP_SHOP_EXCHANGE_KHACBAN_HOANMY_LV5] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng n©ng cÊp V¨n Søc 5
	[EQUIP_SHOP_EXCHANGE_DAQUY] = {beginDate = 20190101, endDate = 20190101}, --LongDaiKa: §ãng ghÐp ®¸ quý
};
-------------------------Check Goods Date-------------------------
...
-- if you dont want date limit with a shop, just ignore this table
m_tbGoodsDate = {
	--[53468] = {beginDate = 20190101, endDate = 20190101}, --LongDaiKa: §ãng Th­¬ng H¶i Ch©u CÊp 7
	[53469] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: §ãng Th­¬ng H¶i Ch©u CÊp 8
	[53454] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: TÈy T©m Th¹ch CÊp 7
	[53455] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: TÈy T©m Th¹ch CÊp 8
	[53460] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: LuyÖn L« ThiÕt CÊp 7
	[53461] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: LuyÖn L« ThiÕt CÊp 8
	[53475] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: Tói §¸ Quý CÊp 7
	[53476] = {beginDate = 20190101, endDate = 20290101}, --LongDaiKa: Tói §¸ Quý CÊp 8
};
----------------------------------------------------------------------
...
function _DoOneExchange(tGoodInfo, tExchangeInfo, uShopId, uGoodID, uExchangeID)
	...
	nNeedWeight = nNeedWeight + tItemTemplate.Weight --¶à¼ÓÒ»¸öµÀ¾ßµÄ¸ºÖØ£¬²»È»additem¿ÉÄÜÊ§°Ü
	-- ¸ºÖØ
	...
	for _, tCostItem in tExchangeInfo.CostItem do
		if 0 < tCostItem[4] then
			local nTotalCount 		= GetItemCount(tCostItem[1], tCostItem[2], tCostItem[3], tCostItem[5] or 0);  -- ²»ÂÛÊÇ·ñ¼ÓËøµÄµÀ¾ß×ÜÊýÁ¿
			local nUnlockedCount	= GetUnlockedItemCount(tCostItem[1], tCostItem[2], tCostItem[3], tCostItem[5] or 0); -- Î´¼ÓËøµÄµÀ¾ßÊýÁ¿
			if tCostItem[4] > nUnlockedCount then
				local szItemName = GetItemName(tCostItem[1], tCostItem[2], tCostItem[3]);
				local szTalk = format("C¸c h¹ mang theo [%s] kh«ng ®ñ %u, kh«ng thÓ ®æi [%s].", szItemName, tCostItem[4], tGoodInfo.Name)
				if nTotalCount ~= nUnlockedCount then
					szTalk = szTalk .. "<color=yellow>(§Ó ®¶m b¶o an toµn tµi s¶n, ®¹o cô ®· khãa sÏ kh«ng thÓ ®æi)<color>"
				end
				Talk(1, "", szTalk);
				return 0
			end
	...
	for _, tCostTask in tExchangeInfo.CostTask do
		if 0 < tCostTask[1] then
			local nTaskValue	= GetTask(tCostTask[1]);
			if tCostTask[1] == TASKID_CHARM_CAN_USED_POINT then  --÷ÈÁ¦Öµ
	...
	-- Íâ×°´¦Àí ¼° ÓÐÄÍ¾Ã¶ÈµÄ´ì±ÈµÀ¾ß¡¾200¡¿
	--------------------------------
	-- bugfixed: 	ÉèÖÃÎïÆ·µÄ¹ýÆÚÊ±¼ä,±ØÐëÔÚadditemÖ®Ç°ÉèÖÃ
	--				·ñÔòÈç¹ûÍæ¼Ò±³°üÀï´æÔÚÒ»¸öÓÀ¾ÃµÀ¾ß£¬ÄÇÃ´ÐÂÔöµÄÓÐÆÚÏÞµÀ¾ß»áÖ±½Ó±»¸½¼Óµ½Ö®Ç°µÄÓÀ¾ÃµÀ¾ßÉÏ¡£
	--				´Ó¶øµ¼ÖÂÐÂµÀ¾ßµÄÓÐÐ§ÆÚ±ä³ÉÎÞÏÞ¡£
...
function OnExchange(uShopId, uGoodID, uExchangeID, uBatchNum)
	...
	if checkShopDate(uShopId) == 0 then
		Talk(1,"","Cöa hµng ®· qu¸ h¹n, h·y cÈn thËn, ®õng thùc hiÖn thao t¸c kh«ng hîp lÖ!")
	...
	--LongDaiKa: kiÓm tra thêi gian më b¸n item  -------------------------------------------
	if checkGoodsDate(uGoodID) == 0 then
		Talk(1,"","VËt phÈm ®· qu¸ h¹n, h·y cÈn thËn, ®õng thùc hiÖn thao t¸c kh«ng hîp lÖ!")
...
function on_weekly_clear()
	...
	SetTask(3383, 0) --ÒøÈ¯ÉÌµêÃ¿ÖÜ¶Ò»»´ÎÊý
	SetTask(3384, 0) --ÒøÈ¯ÉÌµêÍ¬¼ÃÖ®Ô¸Ã¿ÖÜ¶Ò»»´ÎÊý
end
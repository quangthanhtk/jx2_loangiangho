...
function view_gongcheng_info()
	...
	if szShou == "" then
        ...
        szGong = sf_Replace(szGong, "|" ,"!");
        szShou = sf_Replace(szShou, "|" ,"!");
...
function single_enter(nCamp)
	...
	if GetTongDuty() == 1 and GetTask(TNC_TONG_AWARD) ~= 0 then
		Say(g_InfoHeader.."Sau khi nhËn phÇn th­ëng C«ng Thµnh §¹i ChiÕn míi ®­îc tham gia c«ng thµnh!", 0);
		return 0;
	end
	transmit_single_man(nCamp,nBattleMapID);
end;
...
function reinforcement_enter(nCamp)
	...
	if gf_CheckTeam(function ()
			...
			return 1;
		end, g_InfoHeader.."Sau khi nhËn phÇn th­ëng C«ng Thµnh §¹i ChiÕn míi ®­îc tham gia c«ng thµnh!") ~= 1 then
		return 0;
	end
...
function get_award()
	...
	if GetTongDuty() == 1 then
		tinsert(selTab, "Ta muèn nhËn phÇn th­ëng bang héi (Bang chñ nhËn)/get_tong_award");
	end
...
function get_tong_award()
	if GetTongDuty() ~= 1 then
		Say(g_InfoHeader.."ChØ cã bang chñ míi ®­îc nhËn phÇn th­ëng nµy!", 0);
		return 0;
	end
	local nFlag = GetTask(TNC_TONG_AWARD);
	if nFlag == 0 then
		Say(g_InfoHeader.."B¹n kh«ng tham gia C«ng Thµnh §¹i ChiÕn hoÆc ®· nhËn phÇn th­ëng bang héi råi!", 0);
		return 0;
	end
	if nFlag == 1 then
		if gf_Judge_Room_Weight(15, 10, g_InfoHeader) ~= 1 then
			return 0;
		end
		SetTask(TNC_TONG_AWARD, 0);
		Msg2Player("B¹n ®· giµnh th¾ng lîi ë C«ng Thµnh §¹i ChiÕn lÇn tr­íc, phÇn th­ëng nh­ sau:")
		gf_AddItemEx2({2, 1, 1210, 15, 4}, "B¶o r­¬ng Tµi nguyªn", "C«ng Thµnh §¹i ChiÕn", "PhÇn th­ëng bang chñ", 0, 1);
		-- gf_AddItemEx2({2, 1, 30589, 1, 4}, "°ïÅÉÁîÅÆ", "¹¥³Ç´óÕ½", "°ïÖ÷½±Àø", 0, 1);
		gf_AddItemEx2({2, 1, 30844, 5, 4}, "R­¬ng Háa Phông", "C«ng Thµnh §¹i ChiÕn", "PhÇn th­ëng bang chñ", 0, 1);
		-- gf_AddItemEx2({2, 1, 30574, 1, 4}, "ÌúÑª°ïÅÉÕ½Æì", "¹¥³Ç´óÕ½", "°ïÖ÷½±Àø", 0, 1);
		gf_AddItemEx2({2, 1, 30770, 3, 1}, "Háa Phông Tinh Hoa", "C«ng Thµnh §¹i ChiÕn", "PhÇn th­ëng bang chñ", 0, 1);
		
		return 1;
	end
	if nFlag == 2 then
		if gf_Judge_Room_Weight(4, 10, g_InfoHeader) ~= 1 then
			return 0;
		end
		SetTask(TNC_TONG_AWARD, 0);
		Msg2Player("B¹n ®· thÊt b¹i ë C«ng Thµnh §¹i ChiÕn lÇn tr­íc, phÇn th­ëng nh­ sau:")
		gf_AddItemEx2({2, 1, 1210, 5, 4}, "B¶o r­¬ng Tµi nguyªn", "C«ng Thµnh §¹i ChiÕn", "PhÇn th­ëng bang chñ", 0, 1);
		gf_AddItemEx2({2, 1, 30844, 1, 4}, "R­¬ng Háa Phông", "C«ng Thµnh §¹i ChiÕn", "PhÇn th­ëng bang chñ", 0, 1);
		return 1;
	end
end
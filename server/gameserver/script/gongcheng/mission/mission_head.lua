...
function MS_JoinMission(nCamp)
	...
	if nCamp == GONG_ID then
		...
		Say("Phe tÊn c«ng: KÝch ho¹t vµ ph¸ hñy Long Trô míi giµnh ®­îc th¾ng lîi cuèi cïng. Ch­a ph¸ hñy c¬ quan cæng thµnh hoÆc chiÕm lÜnh cæng thµnh sÏ kh«ng thÓ vµo thµnh. Gîi ý: ë chç Qu©n S­ cã thÓ nhËn chiÕn b¸o tiÒn tuyÕn vµ t×m hiÓu t×nh h×nh chiÕn tr­êng.",0);
	elseif nCamp == SHOU_ID then
		if GetTask(TNC_CAMP_TIME) == 0 then
			SetTask(TNC_CAMP_TIME, GetTime());
		end
		Say("Phe phßng thñ: B¶o vÖ tèt Long Trô sÏ quyÕt ®Þnh chiÕn th¾ng cuèi cïng. Khi phe ta trÊn gi÷ cæng thµnh th× kÎ ®Þch sÏ kh«ng thÓ vµo thµnh. Gîi ý: ë chç Qu©n S­ cã thÓ nhËn chiÕn b¸o tiÒn tuyÕn vµ t×m hiÓu t×nh h×nh chiÕn tr­êng.",0);
	end;
end;
--´¦Àí×¼±¸½×¶ÎµÄ¼ÆÊ±Æ÷º¯Êý
...
function MS_ProcessGongChengTimer()
	...
	if nLoop == 0 then
		...
		if GetMissionV(MV_LONGZHU_FLAG) == 2 then	--¹¥³Ç·½Ó®ÁË
			...
			AddRuntimeStat(59, 1, 0, 1) --ÊØ·½Ê¤ÀûÍ³¼Æ

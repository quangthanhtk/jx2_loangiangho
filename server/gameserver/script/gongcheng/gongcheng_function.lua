...
Import("\\script\\gongcheng\\gongcheng_head.lua");
...
--			Say("¾­¹ý¼¤ÁÒµÄÕ½¶·£¬×îÖÕ<color=yellow>"..GetMissionS(MS_GONG_TONG_NAME).."<color>°ï»á³É¹¦Õ¼ÁìÁËÓÉ<color=yellow>"..GetMissionS(MS_SHOU_TONG_NAME).."<color>°ï»áÊØÎÀµÄ<color=yellow>"..GetMissionS(MS_CITY_NAME).."<color>¡£",0);
--			SetFightState(0);
...
--		Msg2Global("¾­¹ý¼¤ÁÒµÄÕ½¶·£¬×îÖÕ"..GetMissionS(MS_GONG_TONG_NAME).."°ï»á³É¹¦Õ¼ÁìÁËÓÉ"..GetMissionS(MS_SHOU_TONG_NAME).."°ï»áÊØÎÀµÄ"..GetMissionS(MS_CITY_NAME));
--		AddGlobalCountNews("¾­¹ý¼¤ÁÒµÄÕ½¶·£¬×îÖÕ<color=yellow>"..GetMissionS(MS_GONG_TONG_NAME).."<color>°ï»á³É¹¦Õ¼ÁìÁËÓÉ<color=red>"..GetMissionS(MS_SHOU_TONG_NAME).."<color>°ï»áÊØÎÀµÄ<color=green>"..GetMissionS(MS_CITY_NAME).."<color>",1);
--		PlayMusic("\\music\\themusicisnotexist.mp3",1);
--		GCZ_PlaySound(GONG_ID,"¹¥³ÇÕ½Ê¤Àû.wav");
--		GCZ_PlaySound(SHOU_ID,"¹¥³ÇÕ½Ê§°Ü.wav");
--		WriteLog("[¹¥³ÇÕ½Ö®¹¥³Ç]:"..GetMissionS(MS_GONG_TONG_NAME).."°ï»áÕ¼ÁìÁË£¨"..nCurFlagCount.."ÃæÆì×Ó£©ÓÉ"..GetMissionS(MS_SHOU_TONG_NAME).."°ï»áÊØÎÀµÄ"..GetMissionS(MS_CITY_NAME).."¡£MSUIDÊÇ"..GetMissionV(MV_MISSION_UID));
--		return 1;
--	else
--		return 0;
--	end;
...
--¼ì²éÊÇ·ñË¢ÁúÖù
function GCZ_CheckCreateLongzhu()
	local msState = GetMissionV(MV_MISSION_STATE)
	if msState ~= MS_STATE_GONGMEN and msState ~= MS_STATE_GONGCHENG then
			return 0;
	end;
	if GetMissionV(MV_LONGZHU_FLAG) ~= 0 then
		return 0;
	end
	if GCZ_CheckAllDoor() ~= 1 then
		Msg2MSAll(MISSION_ID, "Ch­a pha hñy tÊt c¶ Thµnh M«n, Long Trô t¹m ch­a xuÊt hiÖn, c¸c hiÖp sÜ cè g¾ng lªn!");
		return 0
	end
	if GetMissionV(MV_FLAG_OWN_COUNT) < FLAG_COUNT_END_WIN then
		Msg2MSAll(MISSION_ID, format("Sè kú bªn c«ng thµnh ®· chiÕm kh«ng ®ñ %d, Long trô t¹m ch­a xuÊt hiÖn, c¸c hiÖp sÜ cè g¾ng lªn!", FLAG_COUNT_END_WIN));
		return 0;
	end
	if GetMissionV(MV_DESTROY_FLAG1) ~= 1 or GetMissionV(MV_DESTROY_FLAG2) ~= 1 or GetMissionV(MV_DESTROY_FLAG3) ~= 1 then
		Msg2MSAll(MISSION_ID, "Ch­a ph¸ hñy tÊt c¶ §¹i Kú Thµnh M«n, Long Trô t¹m ch­a xuÊt hiÖn, c¸c hiÖp sÜ cè g¾ng lªn!");
		return 0;
	end
	local nBattleMapID = SubWorldIdx2ID(SubWorld);
	local npcIndex = CreateNpc("GC_longzhu", "Long Trô", nBattleMapID, 1293, 2779);
	if npcIndex > 0 then
		SetMissionV(MV_LONGZHU_FLAG, 1);
		SetCampToNpc(npcIndex, tCampNameP[SHOU_ID]);
		SetNpcDeathScript(npcIndex, "\\script\\gongcheng\\npc\\longzhu_death.lua");
		Msg2MSAll(MISSION_ID, "Long Trô ®· xuÊt hiÖn, ®¸nh b¹i Long Trô sÏ nhËn ®­îc th¾ng lîi cuèi cïng!");
	end
end
function nothing()

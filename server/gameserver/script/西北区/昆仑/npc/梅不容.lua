...
function main()
	...
	tinsert(tDiaSel, getn(tDiaSel), "Ta muèn ®æi Vâ Hån HuyÕt./jianzun_buy");
...
-- ½£×ğ¶Ò»»Îä»êÖ®Ñª
function jianzun_buy()
	local szNpcName = GetTargetNpcName()
	if GetPlayerRoute() ~= 32 then
		Talk(1,"",format("<color=green>%s<color>: Ng­¬i kh«ng ph¶i lµ ®Ö tö KiÕm T«n, kh«ng thÓ mua vËt phÈm bİ truyÒn cña bæn m«n.",szNpcName))
		return
	end
	local szTitle = format("<color=green>%s<color>: KiÕm T«n lÊy kiÕm nhËp ®¹o, luyÖn kiÕm tu thÇn, khi Linh KiÕm ®¹i thµnh, dïng Vâ Hån HuyÕt nu«i d­ìng, t¨ng m¹nh linh khİ, gióp chñ nh©n vµ Linh KiÕm t©m ı t­¬ng th«ng.",szNpcName)
	local tbSay = {
		"Dïng 5 Thiªn Cang LÖnh, 5 Thiªn M«n Kim LÖnh ®æi Vâ Hån HuyÕt/jianzun_buy_confirm",
		"§Ó ta nghÜ l¹i/nothing",
	}
	Say(szTitle,getn(tbSay),tbSay)
end
function jianzun_buy_confirm()
	local szNpcName = GetTargetNpcName()
	if GetItemCount(2,95,204) < 5 then
		Talk(1,"",format("<color=green>%s<color>: Thiªn Cang LÖnh kh«ng ®ñ!",szNpcName))
		return
	end
	if GetItemCount(2,1,30370) < 5 then
		Talk(1,"",format("<color=green>%s<color>: Thiªn M«n Kim LÖnh kh«ng ®ñ!",szNpcName))
		return
	end
	if GetFreeItemRoom() < 1 then
		Talk(1, "", format("<color=green>%s<color>: Trªn ng­êi kh«ng ®ñ chç, h·y s¾p xÕp l¹i tr­íc",szNpcName));
		return
	end
	if DelItem(2,95,204,5) == 1 and DelItem(2,1,30370,5) == 1 then
		gf_AddItemEx({2,201,20,1,4}, "Vâ Hån Chi HuyÕt");
	end
end

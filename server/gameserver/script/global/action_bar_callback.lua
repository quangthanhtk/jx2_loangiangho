...
----------------------------------------------------------------------------------------------------
Include("\\script\\warlord\\callbacks\\callback_dowait.lua");
----------------------------------------------------------------------------------------------------
...
function main(deal_suc,deal_fail,deal_flag)
	...
	local deal_action_total = {
			...
			--LongDaiKa: thªm tÝnh n¨ng ®¸nh ng­îc 255 lªn
			[252] = _w_RuongQuyMao_OpenSuccess,
			[253] = _w_RuongQuyMao_OpenFail,
			
			[254] = _w_ItemUpgrade_OnComplete,
			[255] = _w_ItemUpgrade_OnAbort,
...
--					ModifyExp(nLev_GetExp * 8)
			...
			if nPoint < 3000 then
				Msg2Player("TÝch lòy thu thËp: "..nPoint.."/3000")
			else
				Msg2Player("TÝch lòy thu thËp: 3000/3000")
			end
			if nPoint >= 3000 then
...
--								ModifyExp(nLev_GetExp_mem * 8)							
							...
							nPoint = mod(GetTask(TSK_NEWRES_POINT),10000) --GetTask(TSK_NEWRES_POINT)
							if nPoint < 3000 then
								Msg2Player("B¹n ®· thu thËp thµnh c«ng: "..nPoint.."/3000")
							else
								Msg2Player("B¹n ®· thu thËp thµnh c«ng: 3000/3000")
							end
							if nPoint >= 3000 then
								gf_SetTaskByte(TSK_NEWRES_FINISH,BYTE_NEWRES_STATE,2)
								TaskTip("B¹n ®· hoµn thµnh nhiÖm vô thu thËp tµi nguyªn.")
...
function bachbaoruong_OpenFail()
end

function bachbaoruong_OpenSuccess()
	SendScript2VM("\\script\\ksgvn\\event\\bachbaoruong\\bachbaoruong.lua","bachbaoruong_GiveAward()")
end

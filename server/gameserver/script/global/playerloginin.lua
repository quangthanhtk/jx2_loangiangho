...
-- KSGVN
Import("\\script\\ksgvn\\autoexec\\ksg_login_listener.lua")
-- END KSGVN

----------------------------------------------------------------------------------------------------
Import("\\script\\warlord\\autoexecute\\player_login_listener.lua");
----------------------------------------------------------------------------------------------------
...
--	SB_OnPlayerLogin();
	...
	OnKsgPlayerLogin();
	--END KSGVN
	--LongDaiKa: thùc thi hµm khi playerloginin
	_w_OnPlayerLogin();
	--
...
--		Say("Thêi gian x¸c nhËn 5 giê ch¬i ®· hÕt, xin ®¨ng nhËp vµo trang qu¶n lı ®Ó ®¨ng kı th«ng tin.",0);
--		SetTask(443,1);
	end;
	--========³ÇÊĞ³ÆºÅ==============
...
--	end	
	...
	Update_Player_Attr_Point_Addon() --ÏÖÔÚÇ±ÄÜµãÉÏÏŞµ÷ÕûÍ³Ò»·ÅÔÚÕâÀï
	...
	--ÈºĞÛÕ½º£ ³ÆºÅ
	if IsTitleExist(78, 1) ~= 1 then
		AddTitle(78, 1);
		Msg2Player(format("NhËn ®­îc danh hiÖu [%s]", "QuÇn Hïng ChiÕn"));
	end
	---------------
end;
--*****************************É¾³ı¹ıÆÚÎïÆ·*************************
...
function _TellOfflineTime(nBaiJuTime, nLiuShenTime, nSanQingTime, nJuLingTime)
	if nBaiJuTime > 0 or nLiuShenTime > 0 or nSanQingTime > 0 or nJuLingTime > 0 then
		...
		local szMsg = "C¸c h¹ ®· tİch lòy rêi m¹ng ®­îc \n"
    	szMsg = format("%s %s thêi gian B¹ch C©u hoµn, cã thÓ ®æi tèi ®a <color=yellow>%s<color> ®iÓm kinh nghiÖm\n", szMsg, _GetTimeStr(nBaiJuTime), get_large_num(GiveExp))
    	szMsg = format("%s %s thêi gian Lôc ThÇn Hoµn, cã thÓ ®æi tèi ®a <color=yellow>%d<color> ®iÓm danh väng\n", szMsg, _GetTimeStr(nLiuShenTime), floor(nLiuShenTime/2))
    	szMsg = format("%s %s thêi gian Tam Thanh Hoµn, cã thÓ ®æi tèi ®a <color=yellow>%d<color> ®iÓm cèng hiÕn s­ m«n\n", szMsg, _GetTimeStr(nSanQingTime), floor(nSanQingTime * 3 / 20))
    	szMsg = format("%s %s thêi gian Tô Linh Hoµn, cã thÓ ®æi tèi ®a <color=yellow>%d<color> ®iÓm ch©n khİ\n", szMsg, _GetTimeStr(nJuLingTime),3*nJuLingTime)

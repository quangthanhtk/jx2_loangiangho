...
function OnUse()
	Say("B¹n cã muèn lÊy <color=yellow>Hßa ThÞ BÝch<color> ®æi thµnh vµng kh«ng? Cã thÓ ®æi ®­îc <color=yellow>2000<color>.",
		2,
		"§­îc/open_panlongbi",
...
function open_panlongbi()
	...
	if (DelItem(2, 1, 1001, 1) == 1) then
		...
		if nLimitTime == 0 then
			Say("B¹n nhËn ®­îc <color=yellow>2000 vµng<color>!",0);
		else
			Say("B¹n nhËn ®­îc <color=yellow>2000 vµng<color>! H«m nay b¹n ®· dïng Hßa ThÞ BÝch lÇn thø <color=yellow>"..(nCurUseTime+1).."<color>. Dùa vµo ®¼ng cÊp hiÖn t¹i, h«m nay b¹n cã thÓ dïng <color=yellow>"..nLimitTime.."<color> Hßa ThÞ BÝch.", 0)

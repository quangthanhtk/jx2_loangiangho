g_tbItem = {
	--ID1, ID2, ID3,     name,Ôö¼ÓÖµ,ÏÞÖÆÖµ,[7]=CDµ¥Î»Ö¡
	[1] = {2,	1,	1156,	"NghÞch L©n",	1,	10, nil},
	[2] = {2,	1,	1157,	"§Êu hån",	10,	10, 120*18},
}
...
function use(nItemIdx,nIdx)
	...
	if nMapID ~= 606 then
		if DelItemByIndex(nItemIdx,1) == 1 then
			...
			if g_tbItem[nIdx][7] and g_tbItem[nIdx][7] > 0 then
				SetItemUseLapseByGDP(g_tbItem[nIdx][1],g_tbItem[nIdx][2],g_tbItem[nIdx][3],g_tbItem[nIdx][7])
			end
		end;
	else
...
function chk_can_use()
	...
	if nMapID == 606 --LongDaiKa: t¾t sö dông ®Êu hån t¹i map Tµi Nguyªn ChiÕn
			or nMapID == 969 --LongDaiKa: t¾t sö dông ®Êu hån t¹i map Hoa S¬n
			or (nMapID >= 881 and nMapID <= 884) --LongDaiKa: t¾t sö dông ®Êu hån t¹i c¸c map chiÕn tr­êng
	then
		return 0;
	end

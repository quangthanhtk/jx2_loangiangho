...
function OnUse()
	Say("B¹n cã muèn lÊy <color=yellow>Bµn Long bÝch<color> ®æi thµnh vµng kh«ng? Cã thÓ ®æi ®­îc <color=yellow>35<color> tiÒn.",
		2,
		"§­îc/open_panlongbi",
...
function open_panlongbi()
	...
	if (DelItem(2, 1, 1000, 1) == 1) then
		...
		if nLimitTime == 0 then
			Say("B¹n nhËn ®­îc <color=yellow>35 vµng<color>!",0);
		else
			Say("B¹n nhËn ®­îc <color=yellow>35 vµng<color>! H«m nay b¹n ®· dïng Bµn Long bÝch lÇn thø <color=yellow>"..(nCurUseTime+1).."<color>. Dùa vµo ®¼ng cÊp hiÖn t¹i, h«m nay b¹n cã thÓ dïng <color=yellow>"..nLimitTime.."<color> Bµn Long bÝch.", 0)

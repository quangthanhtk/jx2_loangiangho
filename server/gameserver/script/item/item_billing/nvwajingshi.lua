...
function OnUse()
	if IsBookLocked() > 0 then
		Say("MËt tŞch ®ang ë tr¹ng th¸i ®ãng, kh«ng thÓ dïng N÷ Oa tinh th¹ch lÊy bİ kİp ra!", 0)
		return
	end
	
	strTab = {"Sau khi sö dông <color=yellow>N÷ Oa tinh th¹ch<color> cã thÓ lÊy ra 1 quyÓn bİ kİp trong mËt tŞch cña trang bŞ hiÖn cã. B¹n muèn lÊy bİ kİp nµo trong mËt tŞch?",
	...
	if (nAddendarCount == 0) then
		Say("HiÖn t¹i b¹n kh«ng thÓ nhËn bİ kİp!", 0)
	end
	...
	if (nAddendarCount == 1) then
		if (nAddendarID[1] ~= -1) then
			Say(strTab[1],
				2,
				"Ta muèn lÊy bİ kİp "..strAddendar[1].."/get_addendar_1",
				strTab[2])
		end
		if (nAddendarID[2] ~= -1) then
			Say(strTab[1],
				2,
				"Ta muèn lÊy bİ kİp "..strAddendar[2].."/get_addendar_2",
				strTab[2])
		end
		if (nAddendarID[3] ~= -1) then
			Say(strTab[1],
				2,
				"Ta muèn lÊy bİ kİp "..strAddendar[3].."/get_addendar_3",
				strTab[2])
		end
		if (nAddendarID[4] ~= -1) then
			Say(strTab[1],
				2,
				"Ta muèn lÊy bİ kİp "..strAddendar[4].."/get_addendar_4",
				strTab[2])
		end
	end
...
function get_addendar_1()
	...
	Say("B¹n x¸c nhËn muèn lÊy <color=yellow>"..strAddendar.."<color> chø? Xin x¸c nhËn cßn ®ñ søc lùc vµ kho¶ng trèng trong hµnh trang kÎo ®¸nh r¬i bİ kİp!",
		2,
		"LÊy bİ kİp!/confirm_get_addendar_1",
		"Ta kh«ng lÊy--Hñy bá/cancel_dialog")
end;
-- È·ÈÏÈ¡ÏÂ¾öÒª1
function confirm_get_addendar_1()
	...
	if (DelItem(2, 1, 504, 1) == 1) then
		...
		AddItem(ADDENDAR_GENRE, ADDENDAR_DETAIL, nAddendar, 1, 4)
		Say("B¹n ®· hoµn thµnh thao t¸c lÊy bİ kİp <color=yellow>"..strAddendar.."<color>!", 0)
		Msg2Player("B¹n ®· hoµn thµnh thao t¸c lÊy bİ kİp!")
	end
end;
...
function get_addendar_2()
	...
	Say("B¹n x¸c nhËn muèn lÊy <color=yellow>"..strAddendar.."<color> chø? Xin x¸c nhËn cßn ®ñ søc lùc vµ kho¶ng trèng trong hµnh trang kÎo ®¸nh r¬i bİ kİp!",
		2,
		"LÊy bİ kİp!/confirm_get_addendar_2",
		"Ta kh«ng lÊy--Hñy bá/cancel_dialog")
end;
-- È·ÈÏÈ¡ÏÂ¾öÒª2
...
function get_addendar_3()
	...
	Say("B¹n x¸c nhËn muèn lÊy <color=yellow>"..strAddendar.."<color> chø? Xin x¸c nhËn cßn ®ñ søc lùc vµ kho¶ng trèng trong hµnh trang kÎo ®¸nh r¬i bİ kİp!",
		2,
		"LÊy bİ kİp!/confirm_get_addendar_3",
		"Ta kh«ng lÊy--Hñy bá/cancel_dialog")
end;
-- È·ÈÏÈ¡ÏÂ¾öÒª3
...
function get_addendar_4()
	...
	Say("B¹n x¸c nhËn muèn lÊy <color=yellow>"..strAddendar.."<color> chø? Xin x¸c nhËn cßn ®ñ søc lùc vµ kho¶ng trèng trong hµnh trang kÎo ®¸nh r¬i bİ kİp!",
		2,
		"LÊy bİ kİp!/confirm_get_addendar_4",
		"Ta kh«ng lÊy--Hñy bá/cancel_dialog")
end;
-- È·ÈÏÈ¡ÏÂ¾öÒª4

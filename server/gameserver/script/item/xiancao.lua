...
function ConfirmEat(nItem)
	if DelItem(tItemInfo[nItem][1], tItemInfo[nItem][2], tItemInfo[nItem][3], 1) == 1 then
		...
		if nRet == 1 then
			Msg2Player("B¹n nhËn ®­îc hiÖu qu¶ "..tItemInfo[nItem][6])
		end
	end
end
...
function TestEat(nIndex)
	...
	if nTime > 0 then
		...
		elseif tItemInfo[nIndex][5] == nSpeed then
			ConfirmEat(nIndex)
		elseif tItemInfo[nIndex][5] > nSpeed then
			Say("HiÖn b¹n b¹n cã "..tItemInfo[nType][6]..", muèn ®æi thµnh "..tItemInfo[nIndex][6].."?\n<color=red>Chó ý: Sau khi thay ®æi thêi gian hiÖu qu¶ ban ®Çu sÏ trë vÒ 0<color>", 2, "§óng/#ConfirmEat("..nIndex..")", "Kh«ng muèn/no_say")
		end
	else
		ConfirmEat(nIndex)
	end

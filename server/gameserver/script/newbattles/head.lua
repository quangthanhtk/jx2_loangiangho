...
MAIN_MAP_ID = 884;
PRIMARY_MAIN_MAP_ID = 900;		
--===================================================================================
SUB_BATTLE_MAX_PLAYER = 300        --¸±Õ½³¡µ¥·½ÈËÊýÉÏÏÞ
MAIN_BATTLE_MAX_PLAYER = 300    --Ö÷Õ½³¡µ¥·½ÈËÊýÉÏÏÞ
SUB_BATTLE_PLAYER_DIFF = 5        --¸±Õ½³¡×î´óÈËÊý²î
MAIN_BATTLE_PLAYER_DIFF = 5    --Ö÷Õ½³¡×î´óÈËÊý²î
SUB_BATTLE_SIGNUP_MAX_PLAYER = 320    --¸±Õ½³¡±¨Ãûµ¥·½ÈËÊýÉÏÏÞ
MAIN_BATTLE_SIGNUP_MAX_PLAYER = 320    --Ö÷Õ½³¡±¨Ãûµ¥·½ÈËÊýÉÏÏÞ
...
MAX_PRIMARY_JUNGONG = 50000;

ACTIVITY_ADJUSTOR = 0;            --Õ½³¡»îÔ¾ÏµÍ³µ÷Õû²ÎÊý£¨¼´Íæ¼ÒÊ²Ã´¶¼²»×öµÄÄ¬ÈÏ»îÔ¾ÏµÊý£©
...
tBTMSInfo = --Õ½³¡IDÓëMission IDºÍµØÍ¼IDµÄ¶ÔÓ¦¹ØÏµ
{
    ...
    [PRIMARY_MAINBATTLE_ID] = {PRIMARYMAIN_MISSION_ID,PRIMARY_MAIN_MAP_ID};
}
...
tBattleName = --Õ½³¡Ãû×Ö
{
    ...
    [PRIMARY_MAINBATTLE_ID] = "ChiÕn tr­êng Ngäc M«n Quan", -- ChiÕn tr­êng Ngäc M«n Quan
}
...
tBattleChannel = {                                --ÆµµÀÃû×ÖÓëID
    ...
    [PRIMARY_MAINBATTLE_ID] = {"ChiÕn tr­êng Ngäc M«n Quan",36},
}
tCampChannel = --Õ½³¡ÉÏË«·½¸÷×ÔµÄÁÄÌìÆµµÀ
{
    ...
    [9] = {"Ngäc M«n Quan phe Tèng",37},
    [10] = {"Ngäc M«n Quan phe Liªu",38},
}
...
tPKMed = --PKÒ©
{--         ÎïÆ·Ãû ´óÀà Ð¡Àà1 Ð¡Àà2 ËùÐè»ý·Ö Ò©Ð§ËµÃ÷        ËùÐè¾üÏÎ
    { "ChÝnh Nguyªn Håi T©m ®¬n", 1, 0, 50, 1500, "Lo¹i bá tÊt c¶ kü n¨ng CD", 0 },
    { "Ngò ChuyÓn Linh B¶o ®¬n", 1, 0, 53, 1000, "XuÊt chiªu+5", 0 },
    { "ThÇn Gi¸p ®¬n", 1, 0, 54, 1000, "Ngo¹i phßng 100%, duy tr× trong 5 gi©y", 0 },
    { "ThÇn Tr¸o ®¬n", 1, 0, 55, 1000, "Néi phßng 100%, duy tr× 5 gi©y", 0 },
    { "Th«ng C©n t¸n", 1, 0, 56, 1000, "MiÔn dÞch lµm chËm, duy tr× trong 10 gi©y", 0 },
    { "Ho¹t HuyÕt t¸n", 1, 0, 57, 1000, "MiÔn dÞch gi¶m tèc, duy tr× trong 10 gi©y", 0 },
    { "S­íng L¹c t¸n", 1, 0, 58, 1000, "MiÔn dÞch tª liÖt, duy tr× trong 10 gi©y", 0 },
    --	[8]  = {"ÈÎ¶½É¢",	 1,0,59,1000,	"ÃâÒß·âÕÐ£¬³ÖÐø10Ãë",	 0},
    { "Cù Th¹ch t¸n", 1, 0, 60, 1000, "MiÔn dÞch ®Èy lui, duy tr× trong 10 gi©y", 0 },
    { "Kim Cang t¸n", 1, 0, 61, 1000, "MiÔn dÞch ®¸nh ng·, duy tr× trong 10 gi©y", 0 },
    { "§Ò ThÇn t¸n", 1, 0, 62, 1000, "MiÔn dÞch mª man, duy tr× trong 10 gi©y", 0 },
    { "Ng­ng Hån t¸n", 1, 0, 63, 1000, "MiÔn dÞch hçn lo¹n, duy tr× trong 10 gi©y", 0 },
    { "ThÇn Phong t¸n", 1, 0, 64, 1000, "MiÔn dÞch ®Þnh th©n, duy tr× trong 10 gi©y", 0 },
    { "V« óy t¸n", 1, 0, 65, 1000, "MiÔn cho¸ng, duy tr× trong 10 gi©y", 0 },
    --	[15] = {"¶¨ÐÄÉ¢",	 1,0,66,1000,	"ÃâÒßÃÔ»ó£¬³ÖÐø10Ãë",	 0},
    --	[16] = {"ÁáççÕæÔªµ¤",1,0,67,1500,	"ÃâÒßËùÓÐ¸ºÃæ£¬³ÖÐø10Ãë",0},
    --	 {"ThÊt X¶o Linh ®¬n",	 1,0,68,2000,	"TiÓu v« ®Þch, duy tr× trong 3 gi©y",		 0},
    --	{"V« Cùc Tiªn ®¬n",	 1,0,69,5000,	"§¹i v« ®Þch, duy tr× trong 6 gi©y",		 0},
    { "ThÊt KhiÕu Nguyªn B¶o §¬n", 1, 0, 228, 6000, "XuÊt chiªu+10, duy tr× trong 15 gi©y", 4 },
    --	[20] = {"ÎÞÉÏÉñÔªµ¤",1,0,229,8000,	"ÃâÒßËùÓÐ¸ºÃæ£¬³ÖÐø20Ãë",4},
    --	 {"Th«ng Thiªn V« Cùc §¬n",1,0,230,10000,	"V« ®Þch, duy tr× trong 9 gi©y",		 4},
}
...
EXIT_POINT = --ÍË³öµã
{
    ...
    [VILLAGE_ID] = {{808,1748,3060},{808,1635,3192}},
    [RESOURCE_ID] = {{808,1748,3060},{808,1635,3192}},
    [EMPLACEMENT_ID] = {{808,1748,3060},{808,1635,3192}},
    [MAINBATTLE_ID] = {{808,1748,3060},{808,1635,3192}},
    [PRIMARY_MAINBATTLE_ID] = {{806,1748,3060},{806,1635,3192}},
}
--========================================================================================
...
PT_PIONEER_WEEK = 67;
PT_LIZHI_USED_2 = 68;
--PT_CHANGE_CAMP_COUNT_NEW = 69;
PT_QIMENZHENFU_USE_TIME = 70;
PT_DEATH_LOST_JUNGONG = 71;
PT_GET_SUB_AWARD_COUNT = 72;
PT_GET_MAIN_AWARD_COUNT = 73;
PT_GET_AWARD_WEEK = 74;
...
PTNC_SINGLE_LOSE = 90;			
PTNC_ENTER_USE_JUNGONGZHANG = 91--

...
--¸ù¾ÝÍæ¼ÒË÷Òý»ñµÃÍæ¼ÒÃû×Ö£¬Ä¬ÈÏÎªµ±Ç°Íæ¼ÒË÷Òý
...
--Ôö¼Ó¸öÈËÔÚÄ³³¡Õ½ÒÛÖÐµÄ¸öÈËµÃ·Ö£¨Õ½ÒÛ½áÊøºóÇå0£©
function BT_AddBattlePoint(nPoint)
    ...
    if nValue + nPoint < 0 then
        ...
        return nValue;    --·µ»Ø¿ÛÁË¶àÉÙ·Ö£¬±íÊ¾ÓÉÓÚ·ÖÊý²»¹»¿ÛÁË£¬Òª¿ÛÍêÊ£ÏÂµÄ»ý·Ö
    else
        BT_SetData(PTNC_BATTLEPOINT, nValue + nPoint);
        return nil;    --·µ»Ønil£¬±íÊ¾³É¹¦¿Û³ýÁË»ý·Ö
    end ;
end
...
--Ôö¼Ó¸öÈË»ý·Ö£¬µÚ¶þ¸ö²ÎÊý±íÊ¾ÊÇ·ñ²»ÏÔÊ¾ÐÅÏ¢£¬£°ÎªÏÔÊ¾£¬·Ç£°Îª²»ÏÔÊ¾¡£Ä¬ÈÏÊÇÏÔÊ¾¡£
--µÚÈý¸ö²ÎÊý±íÊ¾ÊÇ·ñ¿ÉÎª¶ÓÎé³ÉÔ±Ôö¼Ó»ý·Ö¡££°Îª²»Ôö¼Ó£¬·Ç£°ÎªÔö¼Ó¡£Ä¬ÈÏ²»Ôö¼Ó
function BT_AddPersonalPoint(nPoint, bNotShowMsg, bTeamPoint)
    ...
    if nTeamMemberCount == 0 or FORBIDENT_CREATE_TEAM == 1 or bTeamPoint == 0 or bTeamPoint == nil then
        if BT_GetData(PTNC_BATTLEPOINT) + nPoint >= POINT_LIMITED then
            ...
            Msg2Player("TrËn chiÕn nµy b¹n nhËn ®­îc ®iÓm tÝch lòy tèi ®a: " .. POINT_LIMITED);
        end ;
        if bNotShowMsg == nil or bNotShowMsg == 0 then
            Msg2Player("B¹n nhËn ®­îc ®iÓm tÝch lòy: " .. nPoint);
        end ;
...
--»ñÈ¡Ä³¸öÕ½³¡ÊÂ¼þËù¶ÔÓ¦µÄ·ÖÊý£¬µÚÒ»¸ö²ÎÊýÎªÊÂ¼þID£¬µÚ¶þ¸ö²ÎÊý±íÊ¾ÊÇ·ñÊÇ»ù´¡·ÖÊý£¬0Îª²»ÊÇ£¬·Ç0ÎªÊÇ£¬Ä¬ÈÏ²»ÊÇ
...
--¸ù¾ÝÕ½³¡ÊÂ¼þÀ´¸ø»ý·Ö¡£µÚÒ»¸ö²ÎÊýÎªÊÂ¼þID£¬µÚ¶þ¸ö²ÎÊý±íÊ¾ÊÇ·ñÏÔÊ¾ÌáÊ¾ÐÅÏ¢£¬£°ÎªÏÔÊ¾£¬·Ç£°Îª²»ÏÔÊ¾¡£Ä¬ÈÏÊÇÏÔÊ¾¡£
--µÚÈý¸ö²ÎÊý±íÊ¾ÊÇ·ñ¿ÉÎª¶ÓÎé³ÉÔ±Ôö¼Ó»ý·Ö¡££°Îª²»Ôö¼Ó£¬·Ç£°ÎªÔö¼Ó¡£Ä¬ÈÏ²»Ôö¼Ó
...
function BT_AddTeamPoint(nPoint, bNotShowMsg)
    ...
    local _, nPosX, nPosY = GetWorldPos();    --»ñµÃ´¥·¢ÊÂ¼þµÄÍæ¼ÒµÄ×ø±ê
    ...
    for i = 1, nTeamMemberCount do
        ...
        if BT_GetDistance(nPosX, nPosY) <= TEAM_MEMBER_DISTANCE then
            if bNotShowMsg == nil or bNotShowMsg == 0 then
                Msg2Player("B¹n nhËn ®­îc ®iÓm tÝch lòy ®éi ngò chiÕn tr­êng:" .. nTeamPoint);
            end ;
...
--¼õÉÙ¸öÈËÕ½³¡»ý·Ö£¬ÊÇ·ñÒª¼ÓÈë×é¶ÓµÄÇé¿öÄØ£¿
function BT_DecPersonalPoint(nPoint)
    ...
    if nRetValue ~= nil then
        --Èç¹û¸öÈË»ý·Ö¿ÛÍêÁË
        nPoint = nRetValue;
        BT_ShowDebugInfor("HiÖn ®ang trõ ®i ®iÓm tÝch lòy c¸ nh©n tõ tæng ®iÓm tÝch lòy" .. nRetValue .. " ®iÓm");
    end ;
    BT_AddPlayerCampPoint(-nPoint);
    BT_AddTotalPoint(-nPoint);
...
function BT_AddContribution(nPoint)
    --´¦Àí¼äµý×°ÊøÐ§¹û
    if GetTime() - BT_GetData(PT_SPYCLOTH_TIME) < ITEM_SPYCLOTH_TIME then
        --ÔÚ¼äµý×°ÊøÓÐÐ§Ê±¼äÄÚ
        return 0;
    end ;
...
--»ÊµÛ½±Àø;ÊäÈë²ÎÊýÊÇ»ñµÃ½±ÀøµÄ»úÂÊ£¬¿ÉÒÔÊÇ¸¡µãÊý£¬Èç1.45%ÔòÊäÈë1.45¼´¿É
function BT_EmperorAward(nProbability)
    ...
    if nRandomNum < nProbability * (10 ^ (nStrLen - nDotPos)) then
        Talk(1, "", "<color=yellow>Gi¸ th­ëng lÖnh: <color><enter>Do b¹n lËp ®­îc c«ng lín nªn nhËn ®­îc <color=yellow>" .. EMPEROR_AWARD_POINT .. " ®iÓm tÝch lòy, kinh nghiÖm " .. EMPEROR_AWARD_EXP .. " ®iÓm<color>.");
...
function BT_BattleStarAward()
    ...
    if nPlayerCount > 0 then
        ...
        for i = 1, nPlayerCount do
            --ÒòÎªÓÐ¿ÉÄÜÓÐ¶à¸ö×î¸ß·Ö£¬ËùÒÔ»¹ÊÇÒª±éÀúËùÓÐµÄÍæ¼ÒÒ»´Î
            PlayerIndex = IDTab[i];
            if BT_GetData(PTNC_BATTLEPOINT) == nFirstPoint then
                if IsPlayerDeath() == 0 then
                    ModifyExp(BATTLE_STAR_AWARD_EXP)
                    Msg2Player("B¹n nhËn ®­îc phÇn th­ëng Ng«i sao chiÕn ®Þa:" .. BATTLE_STAR_AWARD_EXP .. " ®iÓm kinh nghiÖm");
                end ;
    ...
    if nPlayerCount > 0 then
        Msg2MSAll(MISSION_ID, tBattleName[BT_GetData(PT_BATTLE_TYPE)] .. " sau khi kÕt thóc, vinh dù ng«i sao chiÕn tr­êng trao cho: " .. strName .. ", xem nh­ biÓu d­¬ng cho " .. sMul .. ", trong chiÕn dÞch nµy cã biÓu hiÖn kiÖt xuÊt!");
    end ;
    PlayerIndex = nOldPlayerIndex;
end;
...
function BT_SetPlayerState(nCamp)
    ...
    if nBattleType ~= RESOURCE_ID then
        ...
        SetCampToPlayer(tCampNameP[ALL_ID]);    --Èç¹ûÊÇ×ÊÔ´Õ½³¡ÔòËÎÁÉ¶¼ÊÇÖÐÁ¢ÕóÓª
    end ;
    ...
    SetMomentum(0); --LongDaiKa: fix l¹m dông trÊn ph¸i
    --LongDaiKa: fix t¾t hiÖu øng ngùa
    WTask:SetBit(WTASKID_PLAYERSTATE_BIT, WTASKID_PLAYERSTATE_BIT_DISABLE_EFFECT, 1);
    if IsHaveState(PLAYERSTATE_RIDING) == 1 then
        SetPlayerScript("\\script\\item\\yujian.lua");
        SendScript2VM("\\script\\item\\yujian.lua", format("OnEquip(%d)", GetPlayerEquipIndex(ITEMPART_HORSE)));
    end
end;
--Õ½³¡½áÊøÊ±Ëù×÷µÄ´¦Àí
function BT_ProcessBattleOver()
    --=============================¹«¹²Êý¾Ý
    BT_SetData(PTNC_BATTLEPOINT, 0);    --µ¥³¡»ý·ÖÇå¿Õ
    BT_ClearBattleActivity();    --»îÔ¾ÏµÊýÇå¿Õ
    ...
    BT_SetData(PTNC_KILL, 0);            --µ¥³¡É±ÈËÊýÇå¿Õ
    BT_SetData(PTNC_DEATH, 0);            --µ¥³¡ËÀÍöÊýÇå¿Õ
    local tMapping = {
        ...
        [PRIMARY_MAINBATTLE_ID] = { PT_ATTEND_MAIN, PT_MAIN_WIN },
    }
    ...
    local tType2Name = {
        [VILLAGE_ID] = "Th«n L¹c Tranh §o¹t ChiÕn",
        [RESOURCE_ID] = "Th¶o Cèc Tranh §o¹t ChiÕn",
        [EMPLACEMENT_ID] = "Ph¸o §µi Tranh §o¹t ChiÕn",
        [MAINBATTLE_ID] = "ChiÕn tr­êng chÝnh-Nh¹n M«n Quan",
        [PRIMARY_MAINBATTLE_ID] = "ChiÕn tr­êng chÝnh-Ngäc M«n Quan",
    }
    TriggerMisEvent("event_mission_joinonce", tType2Name[BATTLE_TYPE])
end;
--Àë¿ªÕ½³¡Ê±Ëù×÷µÄ´¦Àí
function BT_RestorePlayerState()
    ...
    --LongDaiKa: bËt l¹i hiÖu øng ngùa
    WTask:SetBit(WTASKID_PLAYERSTATE_BIT, WTASKID_PLAYERSTATE_BIT_DISABLE_EFFECT, 0);
    if IsHaveState(PLAYERSTATE_RIDING) == 1 then
        SetPlayerScript("\\script\\item\\yujian.lua");
        SendScript2VM("\\script\\item\\yujian.lua", format("OnEquip(%d)", GetPlayerEquipIndex(ITEMPART_HORSE)));
    end
...
----tao file
function _save_file_cttk()
    local szDate = date("%Y%m%d")
    local nCurTime = tostring(szDate);
    if strlower(strsub(getenv("os") or "",1,3)) == 'win' then
        fileName = "data\\chientruong\\logkillngay."..nCurTime..".txt";
    else
        fileName = "data/chientruong/logkillngay-"..nCurTime..".txt";
    end
    local nText = "Account\tRole\tKill\tDie\tPoint\tPhe\n"
    local pf = openfile(fileName, "a+");
    if not pf then return end
    write(pf, format("%s", nText))
    closefile(pf);
    return 1
end
-----------hµm l­u load
function _save_killct_count(nText)
    local szDate = date("%Y%m%d")
    local nCurTime = tostring(szDate);
    if strlower(strsub(getenv("os") or "",1,3)) == 'win' then
        fileName = "data\\chientruong\\logkillngay"..nCurTime..".txt";
...
function BT_ReportFinalResult()
    local killcountct=tonumber(BT_GetData(PTNC_KILL)) or 1;
    local diecounct=BT_GetData(PTNC_DEATH) or 0;
    local szRoleName = WPlayer:GetName();
    local szAccount=GetAccount();
    local nCamp = BT_GetCamp() or 0;
    local nPoint = BT_GetData(PTNC_BATTLEPOINT);
    local tName = {
        [0] = "Undentified",
        [1] = "Tong",
        [2] = "Lieu",
    }
    local savetext=szAccount.."\t"..szRoleName.."\t"..killcountct.."\t".. diecounct.."\t"..nPoint.."\t"..tName[nCamp];
    _save_killct_count(savetext);
    local nBattleType = BT_GetData(PT_BATTLE_TYPE);
    ...
    if IB_VERSION == 1 then
        --Èç¹ûÊÇIB°æ±¾
        sPersonalPoint = sStrResult .. "<enter>ChiÕn dÞch lÇn nµy b¹n nhËn ®­îc <color=yellow>" .. nBattlePoint .."<color> GiÕt ®­îc: <color=red>"..BT_GetData(PTNC_KILL).."<color> BÞ giÕt: <color=red>"..diecounct.." <color> ®iÓm tÝch lòy c¸ nh©n vµ <color=yellow>" .. nPointAward .. "<color> ®iÓm tÝch lòy, <color=yellow>" .. nJunGongAward .. "<color> ®iÓm qu©n c«ng, <color=yellow>" .. nExpAward .. "<color> ®iÓm kinh nghiÖm.";
    else
        sPersonalPoint = sStrResult .. "<enter>ChiÕn dÞch lÇn nµy b¹n nhËn ®­îc <color=yellow>" .. nBattlePoint .."<color> GiÕt ®­îc: <color=red>"..BT_GetData(PTNC_KILL).."<color> BÞ giÕt: <color=red>"..diecounct.." <color> ®iÓm tÝch lòy c¸ nh©n vµ <color=yellow>" .. nPointAward .. "<color> ®iÓm tÝch lòy, <color=yellow>" .. nJunGongAward .. "<color> ®iÓm qu©n c«ng, <color=yellow>" .. nExpAward .. "<color> ®iÓm kinh nghiÖm vµ <color=yellow>" .. nGoldenExpAward .. "<color> ®iÓm søc kháe chuyÓn thµnh kinh nghiÖm. Mêi ®Õn ®iÓm b¸o danh l·nh th­ëng!";
    end ;
    ...
    if nBattlePoint >= 200 and nBattleType == 4 then
        ...
        FireEvent("event_mission_affairs", "newbattle", "mainpoint200", nBattleType)
    end
    if nBattlePoint >= 100 and nBattleType == 3 then
        ...
        FireEvent("event_mission_affairs", "newbattle", "protectcitypoint100", nBattleType)
    end
end;
...
--Ôö¼ÓÕ½³¡»îÔ¾ÏµÊý¡£Õ½³¡»îÔ¾ÏµÊý=ÊÂ¼þ»ù´¡»îÔ¾ÏµÊý*ÃÅÅÉ»îÔ¾ÏµÊý
function BT_AddBattleActivity(nEvenID)
    ...
    if BT_GetData(PTNC_ACTIVITY) > 100 then
        --×î´óÖµ¾ÍÊÇ£±£°£°
...
--Ã¿µ±Íæ¼Ò½øÈëÐÂµÄÕ½³¡Ê±ÒªÇåµôµÄÊý¾Ý
function BT_NewBattleClear2()
    local nDate = tonumber(date("%y%m%d"));
    if BT_GetData(PT_BATTLE_DATE) ~= nDate then
        --Èç¹ûÉÏ´Î±¨ÃûÈÕÆÚÓëµ±Ç°ÈÕÆÚ²»Í¬£¨¼´ÐÂµÄÒ»Ìì£©
        BT_SetData(PT_BATTLE_DATE, 0);
...
function BT_NewBattleClear()
    ...
    if nDate * 1000 + nGlobalState - nSignUpTime > 1 then
        --Çø·ÖÍ¬³¡´ÎµÄ²»Í¬Õ½³¡µÄ±¨Ãû½×¶ÎÓë¿ªÕ½½×¶Î£¬×îºÃÏÈÁË½â±¨ÃûµÄÊµÏÖÔ­Àí
        BT_SetData(PT_BATTLE_DATE, 0);
        BT_SetData(PT_SIGN_UP, 0);
        BT_ClearPTNC();
    end ;
...
--Çå¿ÕËùÓÐÐÂÕ½³¡Ïà¹ØµÄ·þÎñÆ÷È«¾Ö±äÁ¿
...
--¸Ä±äÄ³ÕóÓªÀïÃæËùÓÐÍæ¼ÒµÄÄ³Ò»¸öÁÙÊ±±äÁ¿µÄÖµ
...
function BT_JoinBattle(nCamp)
    ...
    if NewWorld(ENTRANCE_POINT[nCampPlace][1], ENTRANCE_POINT[nCampPlace][2], ENTRANCE_POINT[nCampPlace][3]) == 1 then
        ...
        local nFaction = GetPlayerFaction();    --Í³¼Æ¸÷ÃÅÅÉÈËÊý
        local nFactionPlayerCount = BT_GetFactionPlayerCount(nFaction);
        BT_SetFactionPlayerCount(nFaction, nFactionPlayerCount + 1);    --ÃÅÅÉÈËÊý¼Ó£±
        AddMSPlayer(MISSION_ID, nCamp);
        if GetMissionV(MV_BATTLE_STATE) == MS_STATE_FIGHT then
            --Èç¹ûÊÇÔÚÕ½³¡¿ªÊ¼ºó¼ÓÈëµÄ
...
function BT_LeaveBattle()
    ...
    if DEBUG_VERSION == 1 then
        nCamp = 0;    --Èç¹ûÊÇµ÷ÊÔ°æ±¾£¬Ôò´«µ½ÈªÖÝÄÇÀï¡£Èç¹ûÏë²âÊÔ±¨ÃûÄ£¿éµÄ»°£¬ÄÇ»¹ÊÇ´«ËÍµ½ãê¾©ÄÇ°É
    end ;
    ...
    if NewWorld(EXIT_POINT[nCamp][1], EXIT_POINT[nCamp][2], EXIT_POINT[nCamp][3]) == 1 then
        ...
        local nFaction = GetPlayerFaction();    --Í³¼Æ¸÷ÃÅÅÉÈËÊý
        local nFactionPlayerCount = BT_GetFactionPlayerCount(nFaction);
        BT_SetFactionPlayerCount(nFaction, nFactionPlayerCount - 1);    --ÃÅÅÉÈËÊý¼õ£±		
        return 1;
    else
        return 0;
    end ;
end;
--Õë¶ÔÄ³¸öÈÎÎñ±äÁ¿µÄÖµÀ´½øÐÐÅÅÐò¡£µÚÒ»¸ö²ÎÊýÊÇÕ½³¡µÄÈÎÎñ±äÁ¿ID£¬µÚ¶þ¸ö²ÎÊýÎª1±íÊ¾ÉýÐò£¬0±íÊ¾½µÐò
--µÚÈý¸ö²ÎÊýÖ¸¶¨ÕóÓª£¬Ä¬ÈÏÖµÎª0¡£·µ»ØÖµÎªÒ»¸ö±£´æÅÅÍêÐòºóµÄÍæ¼ÒË÷Òýtable
...
--Õ½³¡½áÊøºóÒªÉèÖÃµÄÒ»Ð©×´Ì¬£¬ÒªÔÚEndMissionµÄ×îºóÖ´ÐÐ
function BT_EndBattle()
    ...
    local nCamp = 0;
    local nWhoWin = GetMissionV(MV_WINNER);
    BT_BattleStarAward();    --ÆÀÑ¡Õ½µØÖ®ÐÇ£¬ÔÚ¼ÆËã¹«¹²»ý·ÖºÍÊ¤¸º»ý·ÖÖ®Ç°
    BT_HideAllTimer()
    ...
    for i = 1, nPlayerCount do
        --´¦ÀíÃ¿¸öÈËµÄ¹«¹²»ý·ÖºÍÊ¤¸º»ý·Ö
        PlayerIndex = IDTab[i];
         if BATTLE_TYPE == RESOURCE_ID then
            FireEvent("event_mission_affairs", "newbattle", "resourcefinished", 0)
         end
        if PlayerIndex > 0 then
            nCamp = BT_GetCamp();
            if STATISTIC_OPEN == 1 then
            ...
            if nCamp == SONG_ID then
                ...
                nX = (nSongPlayerCount + nLiaoPlayerCount) / (2 * nSongPlayerCount);    --×ÜÈËÊý³ýÒÔ±¾·½2±¶ÈËÊý
            else
            ...
            local nExpX = nActivity * nX;    --»îÔ¾ÏµÊý¡ÁÆ½ºâÏµÊý
            local nU = 0;
            if IB_VERSION == 1 then
                nU = 80;
            else
            ...
            local tExpX = { 1, 1, 1.5, 2, 2, 3 };    --Óë¾üÏÎ¶ÔÓ¦µÄ¾­ÑéÏµÊý
            local nCurRank = abs(BT_GetData(PT_CURRANK));
            if nCurRank == 0 or nCurRank > getn(tExpX) then
                --Èç¹ûÃ»ÓÐ¾üÏÎ»ò¾üÏÎ³ö´í
                nCurRank = 1;    --ÖÃÎª1
            end ;
            ...
            --¼Ó³ÉÏµÊý
            nJunGongAward = 5 * nJunGongAward * (1 + BATTLE_JUNGONG_EXT_RATE)
            nExpAward = 10 * nExpAward * (1 + BATTLE_EXP_EXT_RATE) * (1 + GetVipCardParam())
            BT_SetTempData(PTT_RESULT_POINT, nResultPoint);
            BT_SetData(PT_EXP_AWARD, floor(nExpAward * tExpX[nCurRank]));    --¼ÇÂ¼¾­Ñé½±ÀøÊýÖµ
            ...
            --BT_SetData(PT_SPYCLOTH_TIME,0);	--¼äµý×°ÊøÐ§¹ûÇå³ý

            --LongDaiKa: tÝnh l¹i phÇn th­ëng chiÕn tr­êng
            ...
            if tbNewBattleConfigs[BATTLE_TYPE] then
                local tbAward = tbNewBattleConfigs[BATTLE_TYPE].tbDrawAward;
                if nWhoWin == ALL_ID then
                    tbAward = tbNewBattleConfigs[BATTLE_TYPE].tbDrawAward;
                elseif nCamp == nWhoWin then
                    tbAward = tbNewBattleConfigs[BATTLE_TYPE].tbWinAward;
                else
                    tbAward = tbNewBattleConfigs[BATTLE_TYPE].tbLoseAward;
                end
                BT_SetData(PT_EXP_AWARD, tbAward.nExp);
                BT_SetData(PT_POINT_AWARD, tbAward.nBattlePoint);
                BT_SetData(PT_JUNGONG_AWARD, tbAward.nHonor);
            end
        end
    end
    if BATTLE_TYPE == MAINBATTLE_ID or BATTLE_TYPE == PRIMARY_MAINBATTLE_ID then
        BT_ShowBattleRank(10, 1, 1);
    else
        BT_ShowBattleRank(10, 0, 1);
    end ;
    ...
    SetMissionV(MV_BATTLE_STATE, MS_STATE_PEACE);    --ÆäÊµÕâ¸öÔÚClear_MissionDataÖÐÒ²»á±»Çå£¬ÕâÀïÖ»ÊÇÐÎÊ½¶øÒÑ
    BT_ClearMissionData();
    ClearMapNpc(MAPID);
    ClearMapTrap(MAPID);
end;
...
--»ñµÃMissionÀïÃæËùÓÐ·ûºÏÄ³Ò»Ìõ¼þµÄÍæ¼ÒµÄÄ³Ò»ÏîÐÅÏ¢£¬ÒÔtableµÄÐÎÊ½·µ»Ø
--Ìõ¼þÓÉFuncº¯Êý¶¨ÖÆ
...
--Çå³ýRelay¹²ÏíÊý¾Ý¿âµÄÒ»Ð©ÐÅÏ¢£¬Ö÷ÒªÊÇÅÅÐÐ°ñÐÅÏ¢
function BT_ClearRelayData()
    ClearRelayShareData(RELAY_DATA_BATTLE, BATTLE_TYPE, 2, "", "");    --Çå³ý·þÎñÆ÷¶ËÉÏÒ»³¡µÄ¸öÈË»ý·ÖÅÅÃû
    --DelRelayShareDataCopy(RELAY_DATA_BATTLE,BATTLE_TYPE,2);
    if BATTLE_TYPE == EMPLACEMENT_ID or BATTLE_TYPE == MAINBATTLE_ID then
        --¼ÓÉÏÅÐ¶Ï£¬±ÜÃâÒ»Ð©²»±ØÒªµÄÇå¿Õ²Ù×÷
        ClearRelayShareData(RELAY_DATA_BATTLE, BATTLE_TYPE, 3, "", "");    --Çå³ý·þÎñÆ÷¶ËÉÏÒ»³¡µÄ¸öÈË»ý·ÖÅÅÃû
        --	DelRelayShareDataCopy(RELAY_DATA_BATTLE,BATTLE_TYPE,3);
        ClearRelayShareData(RELAY_DATA_BATTLE, BATTLE_TYPE, 4, "", "");    --Çå³ý·þÎñÆ÷¶ËÉÏÒ»³¡µÄ¸öÈË»ý·ÖÅÅÃû
        --	DelRelayShareDataCopy(RELAY_DATA_BATTLE,BATTLE_TYPE,4);
    end ;
end;
--ÏÔÊ¾ÅÅÃû£¬µÚ¶þ¸ö²ÎÊýÎª1Ê±±íÊ¾ÏòÈ«ÊÀ½çÍæ¼ÒÏÔÊ¾ÅÅÃû£¬·ñÔòÖ»ÊÇÏòÕ½³¡ÀïÃæµÄÍæ¼ÒÏÔÊ¾
function BT_ShowBattleRank(nPlayerCount, bShowWorld, bIsEndBattle)
    bShowWorld = bShowWorld or 0;
    bIsEndBattle = bIsEndBattle or 0;
    local tbPlayer = mf_SortMSPlayerByTaskValue(MISSION_ID, PTNC_BATTLEPOINT + BATTLE_OFFSET, 0);
    local nOldPlayerIdx = PlayerIndex;
    if bShowWorld == 1 then
        Msg2Global(tostring(tBattleName[BATTLE_TYPE]) .. " - TÝch lòy c¸ nh©n Top " .. nPlayerCount .. ":");
    else
        Msg2MSAll(MISSION_ID, tostring(tBattleName[BATTLE_TYPE]) .. " - TÝch lòy c¸ nh©n Top " .. nPlayerCount .. ":");
    end
    for i = 1, min(getn(tbPlayer), nPlayerCount) do
        PlayerIndex = tbPlayer[i];
        if bShowWorld == 1 then
            Msg2Global("H¹ng " .. i .. ": (" .. tCampNameZ[BT_GetCamp()] .. ") " .. GetName() .. " " .. BT_GetData(PTNC_BATTLEPOINT) .. " §iÓm");
        else
            Msg2MSAll(MISSION_ID, "H¹ng " .. i .. ": (" .. tCampNameZ[BT_GetCamp()] .. ") " .. GetName() .. " " .. BT_GetData(PTNC_BATTLEPOINT) .. " §iÓm");
        end
        if bIsEndBattle == 1 then
            WTask:SetByte(WTASKID_NEWBATTLE_LASTDATA_BYTE, WTASKID_NEWBATTLE_LASTDATA_BYTE_RANK, i);
        end
    end
...
function BT_KickSleeper()
    if IsSleeping() == 1 and GetFightState() == 0 then
        --ºóÓªË¯ÃßÕßÌß³öÕ½³¡
        Talk(1, "", "<color=yellow>Nguyªn so¸i:<color> Qu©n ®Þch tr­íc mÆt, cßn d¸m ngñ n÷a sao! Ng­êi ®©u! L«i h¾n ®i!");
        DelMSPlayer(MISSION_ID, BT_GetCamp());
        return 1;
    end ;
    return 0;
end;
--¸ù¾ÝÍæ¼Òµ±Ç°µÄ¾ü¹¦Õý¸º»ñÈ¡Íæ¼ÒµÄÕóÓª£¬¾ü¹¦Îª0Õßµ±ËÎ·½´¦Àí
function BT_GetCurCamp()
    local nRet = 0
    local nRank = BT_GetData(PT_CURRANK)
    if nRank >= 0 then
        nRet = SONG_ID
    elseif nRank < 0 then
        nRet = LIAO_ID
...
function BT_GetCurRank()
    ...
    if BT_GetData(PT_MARSHAL_WEEK) >= 15 then
        nEverRank = 6
    elseif BT_GetData(PT_GENERAL_WEEK) + BT_GetData(PT_MARSHAL_WEEK) >= 10 then
        nEverRank = 5
    elseif BT_GetData(PT_PIONEER_WEEK) + BT_GetData(PT_GENERAL_WEEK) + BT_GetData(PT_MARSHAL_WEEK) >= 10 then
        nEverRank = 4
    end
    local nCurRank = abs(BT_GetData(PT_CURRANK))
...
function summon_god_of_war()
    ...
    Say("<color=green>" .. sNpcName .. "<color>: ChiÕn ThÇn sÏ xuÊt hiÖn gióp phe ta ®ét ph¸ vßng v©y qu©n ®Þch trong vßng 2 phót. Mçi trËn chiÕn chØ ®­îc phÐp gäi ChiÕn ThÇn trî gióp tèi ®a 2 lÇn. <color=yellow>Nguyªn So¸i<color> ®ång ý chø?", getn(tSay), tSay)
end
...
function confirm_god_of_war()
    ...
    local fTaskTip = function(tArg)
        TaskTip("ChiÕn ThÇn phe ®Þch sÏ xuÊt hiÖn sau 15 gi©y.");
    end;
...
function battle_charge()
    ...
    if nNation == 66 and nDate <= 110709 then
        Talk(1, "", "HiÖn t¹i chøc n¨ng nµy ®ang ®ãng, kh«ng thÓ kÝch ho¹t!")
        return 0
    end
    ...
    Say("<color=green>" .. sNpcName .. "<color>: T­íng sÜ tham gia tæng tÊn c«ng theo lÖnh cña <color=yellow>Nguyªn So¸i<color> sÏ ®­îc <color=red>nh©n ®«i ®iÓm kinh nghiÖm<color>. <color=yellow>Nguyªn So¸i<color> ®ång ý chø?", getn(tSay), tSay)
end
...
function BT_Charge()
    ...
    SetGlbValue(1022, SetBit(GetGlbValue(1022), nChargeBit, 1)) -- nChargeBit quy ®Þnh riªng trong tõng file Chñ So¸i
...
function BT_ShowReadyTimer(nTimeLeft, nPlayerSong, nPlayerLiao)
    BT_OperateAllPlayer(function(tbArg)
        local nTime = tbArg[1]
        local nTongPlayer = tbArg[2]
        local nLieuPlayer = tbArg[3]
        StopTimeGuage(1)
        StopTimeGuage(2)
        StopTimeGuage(3)
        StartTimeGuage("ChuÈn bÞ chiÕn:", nTime, 0, 1)
        StartTimeGuage("Phe Tèng: " .. nTongPlayer .. " ng­êi", nTime, 0, 2)
        StartTimeGuage("Phe Liªu: " .. nLieuPlayer .. " ng­êi", nTime, 0, 3)
    end, { nTimeLeft, nPlayerSong, nPlayerLiao }, ALL_ID)
end

function BT_ShowFightTimer(nScaleSong, nScaleLiao, nTimeLeft)
    BT_OperateAllPlayer(function(tArg)
        local nTong = tArg[1]
        local nLieu = tArg[2]
        local nTimerCount = tArg[3]
        local nRefreshTime = tArg[4]
        StopTimeGuage(4)
        StopTimeGuage(5)
        StopTimeGuage(6)
        StartTimeGuage("ChiÕn ®Êu: ", nTimerCount, 0, 4)
        StartTimeGuage("§iÓm Tèng: " .. nTong, nRefreshTime, 0, 5)
        StartTimeGuage("§iÓm Liªu: " .. nLieu, nRefreshTime, 0, 6)
    end, { nScaleSong, nScaleLiao, nTimeLeft, FIGHT_TIME / FRAME_PER_SECOND }, ALL_ID)
end

function BT_ShowEndTimer()
    BT_OperateAllPlayer(function(nTimeWaitOut)
        for i = 1, 7 do
            StopTimeGuage(i)
        end
        StartTimeGuage("KÕt thóc cßn:", nTimeWaitOut, 0, 7)
    end, WAITOUT_TIME/ FRAME_PER_SECOND, ALL_ID)
end

function BT_HideAllTimer()
    for i = 1, 7 do
        StopTimeGuage(i)
    end
end

function BT_GetBattleMapID(nCurMapID, nBattleType)
    if nBattleType <= 0 or nBattleType > 5 then
        return 0;
    end ;
    local nMapID = tBTMSInfo[nBattleType][2]
    return nMapID;

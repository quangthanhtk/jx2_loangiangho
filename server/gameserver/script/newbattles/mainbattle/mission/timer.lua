...
function Process_Ready_Timer()
	...
	if nLoop == 0 then
		...
		BT_ShowReadyTimer(READY_TIME/18*nLoop, nPlayerSong, nPlayerLiao)
...
function Process_Fight_Timer()
	...
	if nLoop == 0 then
		...
		BT_ShowFightTimer(nPubPointSong, nPubPointLiao, FIGHT_TIME/18*nLoop)
...
function tell_end(tArg)
	...
	Say(tBattleName[BATTLE_TYPE].." ®· kÕt thóc, <color=yellow>tÝch lòy c«ng céng phe Tèng:"..nPubPointSong..", tÝch lòy c«ng céng phe Liªu:"..nPubPointLiao..", "..szResult.."<color>. C¸c vÞ ®· rÊt nç lùc, chuÈn bÞ ®ãn nhËn ban th­ëng cña hoµng th­îng.",0);
	SetFightState(0);

...
function OnDeath(npcIndex)
	...
	if npcIndex ~= tonumber(GetMissionV(MV_EMPLACEMENT_NPCIDX)) then
		RemoveNpc(npcIndex);
		WriteLog(format("[ERROR] [%s] [%s] %s",
				"NewBattles",
				"EmplacementBattle",
				format("OnDeath() Invalid Emplacement npcIndex, npcIndex=%d, NPCIDX=%d",
					npcIndex, tonumber(GetMissionV(MV_EMPLACEMENT_NPCIDX)) )))
		return
	end

	if Change_Emplacement_Camp(nCamp) == 1 then
		return
	end

	RemoveNpc(npcIndex);
	SetMissionV(MV_EMPLACEMENT_NPCIDX, 0);
	Create_Emplacement(nCamp);
end;

function OnNpcRevive(nNpcIndex, nMapIdx)
	On_Emplacement_Revive(nNpcIndex);
end
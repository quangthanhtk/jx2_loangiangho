...
function newres_general()
	...
	if gf_GetTransLevel() < 792 then
		Talk(1,"",szNpcName.."§¼ng cÊp chuyÓn sinh 7, level 92 trë lªn míi cã thÓ tham gia chiÕn dÞch!")
		return 0
	end	
	if abs(GetTask(701)) < 50000 then
		Talk(1,"",szNpcName.."§iÓm c«ng tr¹ng ph¶i ®¹t 50000 trë lªn míi cã thÓ tham gia chiÕn dÞch !!!!")
		return 0
	end
...
--		"N©ng cao tèc ®é (x3) thu thËp tµi nguyªn (tiªu hao 59 xu vËt phÈm)/#collect_res(2)",
		...
		"Hoµn thµnh nhanh nhiÖm vô Thu ThËp Tµi Nguyªn hµng ngµy(30 xu)/finish_fast_newres_task",
...
--	end
	...
	if  gf_GetTaskByte(TSK_NEWRES_FINISH,BYTE_NEWRES_STATE)  == 0 then
		...
		TaskTip("B¹n ®· nhËn 1 nhiÖm vô thu thËp tµi nguyªn. H·y ®i thu thËp ®ñ 3000 tµi nguyªn ®Ó l·nh th­ëng.")
...
function finish_newres_task()
	...
	--	Msg2Player('TSK_NEWRES_FINISH='..TSK_NEWRES_FINISH, 'BYTE_NEWRES_STATE='..BYTE_NEWRES_STATE);
	--	gf_SetTaskByte(2161,1,2); --lÖnh hoµn thµnh nhanh nv tµi nguyªn chiÕn
	--	gf_SetTaskByte(2161,2,0); --lÖnh reset sè l­ît tr¶ nhiÖm vô
	--end
...
--	end
	if nPetLevel < 1 then
		if gf_GetTaskByte(TSK_NEWRES_FINISH,BYTE_NEWRES_DONE) < 6 then
			Talk(1,"",szNpcName.."§¹i hiÖp ch­a nhËn th­ëng hÕt 6 lÇn hµng ngµy !!!!")
			return
		end
	else	
		if gf_GetTaskByte(TSK_NEWRES_FINISH,BYTE_NEWRES_DONE) < 7 then
			Talk(1,"",szNpcName.."§¹i hiÖp ch­a nhËn th­ëng hÕt 6 lÇn hµng ngµy !!!!")
			return
		end
--		if nIsPS >= 1 then
...
--		[6] = {150000000,16800},
--	}	
	local nExpVIPTNC = 150000000	--tAward_VIP[nPhongHam][1]* nDoubleExpFlag

	local nExpAward = nExpVIPTNC*nDoubleExpFlag;
	local nHonorAward = 22400;
	local nPointAward = 0;

	ModifyExp(nExpAward)
	if GetTask(701) >= 0 then
		SetTask(701,GetTask(701) + nHonorAward)	--tAward_VIP[nPhongHam][2])
	else
		SetTask(701,GetTask(701) - nHonorAward)	--tAward_VIP[nPhongHam][2])
	end
	Msg2Player("B¹n nhËn ®­îc "..nExpVIPTNC*nDoubleExpFlag.." ®iÓm kinh nghiÖm")
	Msg2Player("B¹n nhËn ®­îc 22400 ®iÓm c«ng tr¹ng")
...
--	end
	...
	-- KSG EDIT
	FireEvent("event_mission_task_award", "ksgsystem", "newresourcebattle", 5)
	-- END KSG

	WPlayerLog:WriteBattleLog(g_WCFG_NEWRESOURCE.szKey, "NhËn th­ëng VIP Tµi Nguyªn ChiÕn", 5, "Qu©n C«ng Vinh Dù", nExpBefore, nExpAward, nHonorBefore, nHonorAward, nPointBefore, nPointAward);
end
function use_jungong(nType)
	Say(szNpcName.."Ng­¬i muèn nhËn phÇn th­ëng <color=yellow>"..tJungongItem[nType][1].."<color>?\n<color=yellow>NÕu hoµn thµnh nhiÖm vô trong thêi gian Nguyªn So¸i kÝch ho¹t thu thËp tµi nguyªn sÏ ®­îc nh©n ®«i kinh nghiÖm.",2,format("§ång ý/#confirm_use_jungong(%d)",nType),"Ta ch­a muèn nhËn th­ëng/gf_DoNothing")
end
...
function confirm_use_jungong(nType)
	...
	local nDoubleExpFlag = 1;
	local nCurRank = abs(GetTask(704));
	local nExpAward, nJunGongAward = tAwardRank[nType][1], tAwardRank[nType][2];
	local nPointAward = 0;
...
--	end
	-------------		
	nExpAward = nExpAward * nDoubleExpFlag--floor(nExpAward * tJungongItem[nType][3]) * nDoubleExpFlag
...
--         end
--	end
	--VietSongLiao(nType) --LongDaiKa: ®ãng event th¸ng
...
--	end
	...
	WAward:TaiNguyenChien_HoanThanhNv(nType);
	-- KSG EDIT
	FireEvent("event_mission_task_award", "ksgsystem", "newresourcebattle", nType)
	-- END KSG

	WPlayerLog:WriteBattleLog(g_WCFG_NEWRESOURCE.szKey, "NhËn th­ëng Tµi Nguyªn ChiÕn", nType, tJungongItem[nType][1], nExpBefore, nExpAward, nHonorBefore, nJunGongAward, nPointBefore, nPointAward);
...
function finish_fast_newres_task()
	local tbConsume = {
		tbItems = {
	...
	if gf_GetTaskByte(TSK_NEWRES_FINISH,BYTE_NEWRES_DONE)>=4 then
		if DelItem(2,1,30230,30) == 1 then
			gf_SetTaskByte(2161,1,2); --lÖnh hoµn thµnh nhanh nv tµi nguyªn chiÕn
		else
			return Talk(1,"","B¹n cÇn 30 xu vËt phÈm ®Ó tr¶ nhanh");
		end

	else
		return Talk(1,"","Tõ lÇn thu thËp Tµi Nguyªn thø 4 míi ®­îc tr¶ nhanh");
	end

...
function main()
	...
	tinsert(tSay, "Ta ®ång ı nhËn nhiÖm vô!/accept_quest")
	tinsert(tSay, "MiÔn phİ nhËn th­ëng/done_quest")
	tinsert(tSay, "Sö dông 1 mÆt n¹ S¸t Thñ ®Ó l·nh th­ëng/done_quest1")
...
--			end
		...
		if nType2 ~= 0 then
			...
			Earn(tGoldAward[2])
			gf_AddItemEx2({2,1,30498,3}, "Hu©n ch­¬ng dòng sÜ", "Nhiem vu STD", "nhËn")
		--- §iÓm VËn May
--			local nPointVM = gf_GetTaskByte(TSK_LUCKY_DAOHUU_NEW_THANG4, BYTE_POINT_LUCKY)	
...
--			end
		...
		if nType3 ~= 0 then
			...
			Earn(tGoldAward[3])
			gf_AddItemEx2({2,1,30498,5}, "Hu©n ch­¬ng dòng sÜ", "Nhiem vu STD", "nhËn")
		--- §iÓm VËn May
--			local nPointVM = gf_GetTaskByte(TSK_LUCKY_DAOHUU_NEW_THANG4, BYTE_POINT_LUCKY)	
...
function done_quest1()
	...
	local nStatus = floor(mod(GetTask(KH_QUEST_TSK), 100)/10)
	if GetItemCount(2,1,1087) < 1 then
		Talk(1, "", szNpcName.."Tr¶ nhiÖm vô nµy cÇn cã <color=yellow>1<color> mÆt n¹ S¸t Thñ §­êng. C¸c h¹ h·y chuÈn bŞ ®Çy ®ñ!")
		return
	end
...
--			end
		...
		if nType2 ~= 0 then
			...
			Earn(tGoldAward[2])
			gf_AddItemEx2({2,1,30499,2}, "Hu©n ch­¬ng anh hïng", "Nhiem vu STD", "nhËn")
		--- §iÓm VËn May
--			local nPointVM = gf_GetTaskByte(TSK_LUCKY_DAOHUU_NEW_THANG4, BYTE_POINT_LUCKY)	
--			local nExpVM = (nPointVM - 100)/100*tExpAward[8]
--			ModifyExp(nExpVM)
...
--			end
		...
		if nType3 ~= 0 then
			...
			Earn(tGoldAward[3])
			gf_AddItemEx2({2,1,30499,3}, "Hu©n ch­¬ng anh hïng", "Nhiem vu STD", "nhËn")	
		--- §iÓm VËn May
--			local nPointVM = gf_GetTaskByte(TSK_LUCKY_DAOHUU_NEW_THANG4, BYTE_POINT_LUCKY)	
--			local nExpVM = (nPointVM - 100)/100*tExpAward[9]
--			ModifyExp(nExpVM)

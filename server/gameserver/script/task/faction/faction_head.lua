...
TB_ROUTE_IN_INFO = {
	...
	[31] = {
		...
		in_route_talk = "Tõ nay, ng­¬i lµ mét thµnh viªn cña NhËm HiÖp!",
		skill = {{17,"Chñy Thñ tÊn c«ng b×nh th­êng"},{1872,"MŞ nh T©m Ph¸p"},{1873,"Linh Lung Thİch"},{1874,"Linh Tª Truy MÖnh"}},
		book={0,112,197,"Hoa Tiªn T©m Kinh"},
		book_chip = {
			-- {2,95,16,"»¨ÏÉĞÄ¾­ÉÏ¾í²Ğ¾í"},
			-- {2,95,17,"»¨ÏÉĞÄ¾­ÖĞ¾í²Ğ¾í"},
			-- {2,95,18,"»¨ÏÉĞÄ¾­ÏÂ¾í²Ğ¾í"},
		},
		equip = 14,
	},
	[32] = {
		...
		in_route_talk = "Tõ nay, ng­¬i lµ mét thµnh viªn cña KiÕm T«n!",
		skill = {{4,"KiÕm tÊn c«ng b×nh th­êng"},{1885,"C«n L«n KiÕm T«n"},{1886,"Tµn V©n KiÕm Canh"},{1887,"Nh©n KiÕm Hîp NhÊt"}},
		book={0,112,197,"Hoa Tiªn T©m Kinh"},
		book_chip = {
			-- {2,95,16,"»¨ÏÉĞÄ¾­ÉÏ¾í²Ğ¾í"},
			-- {2,95,17,"»¨ÏÉĞÄ¾­ÖĞ¾í²Ğ¾í"},
...
--			tinsert(t_dia_show,t_dia_sel[3])
			...
			if route_seq == 32 then
				tinsert(t_dia_show,"NhËn ®­îc nguyªn liÖu luyÖn kiÕm/get_itemluyenkiem")
...
function in_route_dtm(faction_seq,route_seq)
	...
	if GetPlayerRoute() == TB_FACTION_ROUTE[faction_seq] and GetPlayerFaction() == faction_seq then
		...
		--¼¼ÄÜÑ§Ï°
		if TB_ROUTE_IN_INFO[route_seq].skill ~= nil then 
		...
		--Ê¦ÃÅÍâ×°Ôö¼Ó
		if route_seq ~= 31 then -- ÈÎÏÀ²»ÓÃÔÙ¸øÍâ×°ÁË
...
function learn_skill(route_seq)
	if GetPlayerRoute() ~= route_seq then	--Á÷ÅÉ¼ì²â
		...
		Talk(1,"","<color=green>"..TB_ROUTE_IN_INFO[route_seq].npc_name.."<color>: Gãc ph¶i biÓu t­îng vâ c«ng hiÖn # lµ vâ c«ng cã thÓ häc, hiÖn # lµ cã thÓ t¨ng cÊp, nhÊp chuét tr¸i vµo lµ ®­îc.")
		AllowLearnSkill()
	end
end
--===============ÕòÅÉÃØ¼®Ïà¹Ø===========
...
function get_itemluyenkiem()

	if GetTask(TSK_GET_THIETTINH) >= 1 then
		Talk(1,"","<color=green>Hå Man Thµnh<color>: §¹i hiÖp ®· nhËn nguyªn liÖu råi.");
		return 0;
	end
	...
	SetTask(TSK_GET_THIETTINH,1)
	AddItem(2,1,30533,200,4);
	AddItem(2,1,30534,100,4);
end
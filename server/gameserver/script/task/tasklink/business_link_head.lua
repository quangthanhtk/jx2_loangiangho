...
function accept_new_task()
    ...
    if check_accept_task_time() == 0 then
        Talk(1, "", "<color=green>¤ng chñ Th­¬ng Héi<color>: HiÖn ch­a ®Õn thêi gian b¾t ®Çu Th­¬ng Héi, triÒu ®×nh chØ cho phĞp th­¬ng héi ho¹t ®éng vµo 3 khung giê <color=red>tõ 13h00-14h30, 16h00-17h00 vµ 21h00-22h00<color> xin ®¹i hiÖp h·y quay l¹i sau!");
        return 0;
    end ;
    ...
    if GetLevel() < 73 or GetReputation() < nReput or GetTask(336) < nFaction or GetPlayerRoute() == 0 then
        Say("N¨ng lùc cña ng­¬i cßn kĞm l¾m, ®îi ®Õn <color=yellow>cÊp 73, gia nhËp hÖ ph¸i, cã ®ñ 3000 ®iÓm danh väng vµ 4000 s­ m«n <color>h·y quay l¹i cèng hiÕn søc lùc còng ch­a muén.", 0)
        return
    end
    local nDate = tonumber(date("%y%m%d"))
    ...
    if GetItemCount(2, 1, 30154) < tbCheck_Award[nType][1] and mod(GetTask(TSK_CHECK_BAOHAP), 100) == 0 then
        Talk(1, "", format("§¹i hiÖp kh«ng mang theo ®ñ sè l­îng N÷ Oa B¶o H¹p ®Ó nhËn nhiÖm vô !!!<enter>(CÇn %d N÷ Oa B¶o H¹p)", tbCheck_Award[nType][1]))
        return
    end

    local bCanAccept = can_accept_task()
    if (bCanAccept == 1) then
        -- ÅĞ¶ÏÊÇ·ñÄÜ¹»½ÓÈÎÎñ
        SelectRandomTask(11)
        -------------------------------------
...
function hand_up_task()
    ...
    if (nTaskFinish == 1) then
        ...
        local nDoneTime = GetTask(CURDAY_DONE_TIMES)
        FireEvent("event_mission_task_award", "ksgsystem", "thuonghoi", nDoneTime)
        if nDoneTime == 60 then
...
function refresh_last_week_end()
    ...
    elseif (nCurMonth == 4 or nCurMonth == 6 or
            nCurMonth == 9 or nCurMonth == 11) then
        -- 4¡¢6¡¢9¡¢11ÔÂµÄ´¦Àí£¬30ÌìµÄ´¦Àí
        nPassDay = 30
    end

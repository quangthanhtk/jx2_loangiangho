...
TASK_ID_TANGMEN_RENXIA = TASK_ID_TANGMEN 
TASK_ID_KUNLUN_JIANZUN = TASK_ID_KUNLUN_TIANSHI
...
function task_main_entrance(nFactionRouteNo)
	if GetAntiEnthrallmentStatus() == 2 then
		Talk(1,"",format("B¹n ®ang ë giai ®o¹n kh«ng lµnh m¹nh, h·y nhanh chãng rêi m¹ng nghØ ng¬i\n"));
		return
	end
...
--	end
	...
	if ((nCurTaskLinkID == TASK_ID_SHAOLIN_SUJIA + 2) or
		...
		(nCurTaskLinkID == TASK_ID_MINGJIAO_ZHENBING + 2) or
		(nCurTaskLinkID == TASK_ID_TANGMEN_RENXIA + 2) or
		(nCurTaskLinkID == TASK_ID_KUNLUN_JIANZUN + 2) 
	) then
	...
	if (nTaskFinish == 1) then
		...
		FireEvent("event_mission_task_award", "ksgsystem", "nvsumon", nCurDayTimes)
		...
		SetTask(TASKVALUE_CUR_DAY_TIMES_ID, nCurDayTimes)	-- ¸üĞÂ½ñÌìÒÑ¾­×öÁËµÄÈÎÎñ´ÎÊı
		    ...
		    (nCurTaskLinkID == TASK_ID_MINGJIAO_ZHENBING ) or
			(nCurTaskLinkID == TASK_ID_TANGMEN_RENXIA) or
			(nCurTaskLinkID == TASK_ID_KUNLUN_JIANZUN) ) then
		...
		elseif ((nCurTaskLinkID == TASK_ID_SHAOLIN_SUJIA + 1) or
		    ...
		    (nCurTaskLinkID == TASK_ID_MINGJIAO_ZHENBING+1 ) or
			(nCurTaskLinkID == TASK_ID_TANGMEN_RENXIA + 1) or
			(nCurTaskLinkID == TASK_ID_KUNLUN_JIANZUN + 1) ) then
		    ...
		    (nCurTaskLinkID == TASK_ID_MINGJIAO_ZHENBING+2 ) or
			(nCurTaskLinkID == TASK_ID_TANGMEN_RENXIA + 2) or
			(nCurTaskLinkID == TASK_ID_KUNLUN_JIANZUN + 2) ) then
...
function get_advanced_faction_book()
	local tGaojiMiji = {
				...
				[31] = {{"Mª Tung HiÖp ¶nh lôc",0,107,230}},
				[32] = {{"Thiªn Phong §Şnh ¶nh quyÕt",0,107,234}},
			};
	local nFactionRoute = GetPlayerRoute()	-- »ñµÃÍæ¼ÒµÄÁ÷ÅÉÊı¾İ

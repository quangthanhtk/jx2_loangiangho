...
tGtTask = {
	...
	GT_TASK_CANCLE_TIME = 3389, --¼ÇÂ¼ÈÎÎñÈ¡ÏûµÄÊ±¼ä
};
	...
	for i = 3,TABLE_GT_MAIN:getRow() do
		if TABLE_GT_MAIN:getCell(i,4) == sNpcName or TABLE_GT_MAIN:getCell(i,5) == sNpcName then
			...
			if tGtNpcInfo[nTaskID] == nil then
				...
				tGtNpcInfo[nTaskID].data[11] = TABLE_GT_MAIN:getCell(i,8); --µÚ11ÏîÎª¸øµÄÈÎÎñµÀ¾ß£¬ÎªÁËÉ¾³ıÎïÆ·
				tGtNpcInfo[nTaskID].data[12] = tonumber(TABLE_GT_MAIN:getCell(i,14)) or 0; --µÚ12ÏîÌø¹ıÈÎÎñÊÇ·ñĞèÒªÇ¿ÖÆÏûºÄÌõ¼şÖĞµÄµÀ¾ß
				tGtNpcInfo[nTaskID].data[13] = tonumber(TABLE_GT_MAIN:getCell(i,15)) or 0; --µÚ13Ïî Èç¹ûÎª1 ±íÊ¾ÔÚ12ÏîÎª0µÄÊ±ºò£¬Ìø¹ıÈÎÎñ²»¿Û³ıµÀ¾ß
...
function tGtNpcMain:LevelGtask(npcId)
	SetPlayerScript("\\settings\\static_script\\isolate\\base\\script_data_center.lua")--×ªÒÆµ½ÊÂ¼ş½Å±¾È¥¶Ô»°
	FireEvent("event_npc_talk", npcId)
...
function tGtNpcMain:npc_main(sNpcName,sNpcTalk)
	...
	if getn(strtab) ~= 0 then
		...
		tinsert(strtab,"T¹i h¹ chØ xem qua th«i/nothing");
...
--		end
	...
	if GetTime() - GetTask(tGtTask.GT_TASK_CANCLE_TIME) < 60 then
...
function tGtNpcMain:cancel_task_confirm(nTaskID,nMoney)
	...
	if Pay(nMoney) == 1 or nMoney == 0 then
		...
		tGtPrize:del_item(tGtNpcInfo[nTaskID]["data"][11])
	
		SetTask(tGtTask.GT_TASK_CANCLE_TIME,GetTime());
...
function tGtNpcMain:skip_task_talk(nTaskID, value)
	...
	if getn(tRet) == 1 then
		...
		if tGtNpcInfo[nTaskID].data[12] == 1 then
			strMoney = strMoney .. " Vµ vËt phÈm cÇn theo yªu cÇu nhiÖm vô"
		end
	...
	if getn(tRet) == 4 then
		...
		szItemName = format("%s*%d",szItemName, num)
		if tGtNpcInfo[nTaskID].data[12] == 1 then
			szItemName = szItemName .. " Vµ vËt phÈm cÇn theo yªu cÇu nhiÖm vô"
		end
		Say(format("Bá qua nhiÖm vô nµy cÇn tèn <color=yellow>%s<color>, x¸c ®Şnh bá qua?", szItemName),
			2,
...
--¼ì²éºÍÏûºÄÈÎÎñÌõ¼şÖĞµÄµÀ¾ß
function tGtNpcMain:skip_task_check_and_cunsume_condition_item(nTaskID)
	local nRet = 1
	if tGtNpcInfo[nTaskID].data[12] == 1 then
		if tGtCondition:del_item_confirm(tGtNpcInfo[nTaskID]["data"][10]) == nil then
			Talk(1, "", "CÇn thu thËp ®ñ vËt phÈm theo yªu cÇu nhiÖm vô míi ®­îc bá qua nhiÖm vô nµy")
			nRet = 0
		end
	else
	end
...
function tGtNpcMain:skip_task_confirm(nTaskID,nMoney)
	...
	if tGtNpcMain:skip_task_check_and_cunsume_condition_item(nTaskID) ~= 1 then return 0 end
	
	if tGtTask:_CheckRoomWeight(tGtNpcInfo[nTaskID]["data"][7]) ~= 1 then
		return 0
	end
	if Pay(nMoney) == 1 then
		FireEvent("event_jump_gtask", nTaskID)
...
function tGtNpcMain:skip_task_confirm2(nTaskID, nG, nD, nP, num)
	...
	--Í³¼ÆÌø¹ıÈÎÎñÏûºÄµÀ¾ß
	gt_add_runtime_stat("skip_task", nTaskID, PackItemId(nG, nD, nP), num)
	FireEvent("event_ib_cost", nG, nD, nP, num)
	FireEvent("event_jump_gtask", nTaskID)
	tGtNpcMain:finish_task_force(nTaskID);
end
...
function tGtNpcMain:finish_task_force(nTaskID)
	...
	--½áÊøÇ°¶¯×÷£º²ß»®Éè¶¨£¬É¾³ı£¬¶Ô»°
	if tGtNpcInfo[nTaskID]["data"][12] ~= 1 and tGtNpcInfo[nTaskID].data[13] ~= 1 then
		tGtCondition:del_item_force(tGtNpcInfo[nTaskID]["data"][10])
	end
	tGtPrize:del_item(tGtNpcInfo[nTaskID]["data"][11])
	...
	--Íê³ÉÈÎÎñÍ³¼Æ
	gt_add_runtime_stat("finish_task", nTaskID, 0, 1)
...
function tGtNpcMain:ShowAward(nTaskID)
	...
	while 1 do
		...
		elseif	g_GtPrize[nConditionID][1] == "ShiMenWeiWang" then
			msg = msg..format("Danh Väng S­ M«n: %s", g_GtPrize[nConditionID][2]).."\n"
		end
...
function tGtCondition:check_tv(nValue1,nValue2,nValue3)
	...
	local nTaskValue = GetTask(nValue1);
	return ((1 == nValue3 and nTaskValue == nValue2) or
	        (2 == nValue3 and nTaskValue >  nValue2) or
	        (3 == nValue3 and nTaskValue >= nValue2) or
	        (4 == nValue3 and nTaskValue <  nValue2 and nTaskValue ~= 0) or
	        (5 == nValue3 and nTaskValue <= nValue2 and nTaskValue ~= 0) or
	        (6 == nValue3 and nTaskValue ~= nValue2));
end
...
function tGtCondition:check_item(nID1,nID2,nID3,nNums,sName)
	...
	if nNums > 0 and GetItemCount(nID1,nID2,nID3) < nNums then
		return nil;
	else
		return 1;
	end
...
function tGtCondition:check_del_item(nConditionID)
	if g_GtCondition[nConditionID] ~= nil then
		if g_GtCondition[nConditionID][1] == "Item" then
			local g,d,p,n = tonumber(g_GtCondition[nConditionID][2]),tonumber(g_GtCondition[nConditionID][3]),tonumber(g_GtCondition[nConditionID][4]),tonumber(g_GtCondition[nConditionID][5])
			if n < 0 then--n<0 ±íÊ¾ÓĞ¶àÉÙ¿Û¶àÉÙ
				local nMax = -n
				n = BigGetItemCount(g,d,p)
				n = min(n, nMax)
				if BigDelItem(g,d,p,n) == 1 then
					return 1;
				else
					return nil;
				end
...
--¸øÓëÊ¦ÃÅÍşÍû
function tGtPrize:give_shimenweiwang(nValue)
	if nValue == "" then
		return nil;
	end
--	if tonumber(nValue) > 1000 then
--		print("WARN: \\script\\task\\global_task\\gtask_head.lua tGtPrize:give_zhenqi nValue > 1000")
--	end
	gf_Modify("ShiMenWeiWang",(tonumber(nValue)));
end
--¸øÓèÎïÆ·
...
--É¾³ıÎïÆ·
function tGtPrize:del_item(sExpression)
	--print(sExpression)
	if sExpression == "" or sExpression == nil then
		return 1;
	end
	sExpression = gsub(sExpression,"&"," ; ");
	sExpression = gsub(sExpression,"|"," ; ");
	sExpression = gsub(sExpression,"(%d+)","tGtPrize:force_del_item(%1)");
	--print(sExpression)
	dostring(sExpression);
end
function tGtPrize:force_del_item(nPrizeID)
	self:check(nPrizeID);
	if g_GtPrize[nPrizeID] ~= nil then
		--gf_PrintTable(g_GtPrize[nPrizeID])
		if g_GtPrize[nPrizeID][1] == "Item" then
			if BigDelItem(unpack(gf_StrSplit(g_GtPrize[nPrizeID][2],","))) == 1 then
				return 1;
			else
				return 2;
			end
...
tGtPrize.tPrizeType = {
	...
	ShiMenWeiWang = tGtPrize.give_shimenweiwang,
};
...
function tGtPrize:result(sExpression)
	sExpression = tGtCondition:get_sub(sExpression);
	dostring("nRet ="..sExpression);
	return nRet;
end

function tGtPrize:check(nPrizeID)
	g_GtPrize = g_GtPrize or {}
	if not g_GtPrize[nPrizeID] then
		g_GtPrize[nPrizeID] = {};
		local nIDRow = TABLE_GT_PRIZE:selectRowNum(1,nPrizeID);
		g_GtPrize[nPrizeID][1] = TABLE_GT_PRIZE:getCell(nIDRow,2);
		for i = 3,8 do
			g_GtPrize[nPrizeID][i-1] = TABLE_GT_PRIZE:getCell(nIDRow,i);
		end
		if tGtPrize.tPrizeType[g_GtPrize[nPrizeID][1]] == nil then
			return nil;
		end
	end
end

...
--	fix_bug_20160601()
	local szTitle = "Danh phËn §¹i HiÖp"
...
function vc_Activating()
	...
	local szTitle = format("<color=green>%s:<color>", GetVipCardName(VC_CURRENT_ID))..format("Ng­¬i ch­a kÝch ho¹t %s.Ng­¬i ch¬i cÊp 80 trë lªn cã thÓ mua thÎ VIP vâ l©m vµ tiÕn hµnh kÝch ho¹t.", szCardName)
...
function vc_ActivatingNow(bTag)
	...
	if GetLevel() < 59 and gf_GetPlayerRebornCount() <= 0 then
		Talk(1,"",format("§¼ng cÊp nh©n vËt kh«ng ®ñ cÊp %d, kh«ng thÓ kÝch ho¹t %s", 59, szCardName));
		return 0;
	end
	if is_in_valid_date(1) ~= 1 then
		return 0
	...
	Msg2Player(msg);
	local msg = format("%s ®· kÝch ho¹t thµnh c«ng", szCardName);
...
function vc_JoinFunction()
	...
	local tSay = {
		...
		format("NhËn ®iÓm tinh lùc (%s)/vc_DailyEnergy", msg),
		"PhÇn th­ëng ®é cèng hiÕn/vc_GetValueAward",
		"NhËn §ång TÕ Chi NguyÖn/vc_GetItemTjzy",
		"NhËn th­ëng tiªu hao Thiªn Kiªu LÖnh/ib_award_talk_main",
		"\n rót lui/nothing"
	}
	...
	Say(format("<color=green>%s:<color> ", GetVipCardName(GetVipCardIndex()))..format("§· kÝch ho¹t thµnh c«ng %s, h¹n sö dông %s ®Õn %s", GetVipCardName(GetVipCardIndex()), nStartTime, nEndTime), getn(tSay), tSay)
end
...
function vc_GetItemTjzy()
	...
	if task ~= 0 then
		Talk(1,"","TuÇn nµy c¸c h¹ ®· nhËn, hÑn gÆp l¹i tuÇn sau!");
		return 0;
	end
	SetTask(TASKID_VC_RECORD_WEEK, 1, TASK_ACCESS_CODE_VIP_CARD);
	gf_AddItemEx2({2, 1, 30913, 7}, "§ång TÕ Chi NguyÖn", GetVipCardName(GetVipCardIndex()), "NhËn §ång TÕ Chi NguyÖn", 0, 1)
end
...
function vc_Rand_10_6_Gem(nNum)
	nNum = nNum or 1;
	local tAward = {
		{1, 18, "HuyÕtTrÝchTh¹ch Lv3", {2, 22, 103, 1, 4}, 0},
		{1, 18, "NguyÖtB¹chTh¹ch Lv3", {2, 22, 203, 1, 4}, 0},
		{1, 18, "Hæ Ph¸ch Th¹ch CÊp 3", {2, 22, 303, 1, 4}, 0},
		{1, 18, "H¾c DiÖu Th¹ch CÊp 3", {2, 22, 403, 1, 4}, 0},
		{1, 6,  "HuyÕtTrÝchTh¹ch Lv4", {2, 22, 104, 1, 4}, 1},
		{1, 6,  "NguyÖtB¹chTh¹ch Lv4", {2, 22, 204, 1, 4}, 1},
		{1, 6,  "Hæ Ph¸ch Th¹ch CÊp 4", {2, 22, 304, 1, 4}, 1},
		{1, 6,  "H¾c DiÖu Th¹ch CÊp 4", {2, 22, 404, 1, 4}, 1},
		{1, 1,  "HuyÕtTrÝchTh¹ch Lv5", {2, 22, 105, 1, 4}, 1},
		{1, 1,  "NguyÖtB¹chTh¹ch Lv5", {2, 22, 205, 1, 4}, 1},
		{1, 1,  "Hæ Ph¸ch Th¹ch CÊp 5", {2, 22, 305, 1, 4}, 1},
		{1, 1,  "H¾c DiÖu Th¹ch CÊp 5", {2, 22, 405, 1, 4}, 1},
	}
	local nIndex = gf_GetRandItemByTable(tAward, gf_SumRandBase(tAward), 1);
	gf_AddItemEx2({tAward[nIndex][4][1], tAward[nIndex][4][2], tAward[nIndex][4][3], tonumber(nNum), tAward[nIndex][4][5]}, tAward[nIndex][3], "VipCard", "§¸ quý cÊp 3-5", 0, 1);
	if tAward[nIndex][5] == 1 then
		Msg2Global(format("[%s] trong ThÎ VIP Vâ L©m ®æi ®iÓm cèng hiÕn nhËn ®­îc phÇn th­ëng %s*%d", GetName(), tAward[nIndex][3], tonumber(nNum)))
	end
end
...
function vc_Rand_10_8_Gem(nNum)
	...
	local nIndex = gf_GetRandItemByTable(tAward, gf_SumRandBase(tAward), 1);
	gf_AddItemEx2({tAward[nIndex][4][1], tAward[nIndex][4][2], tAward[nIndex][4][3], tonumber(nNum), tAward[nIndex][4][5] or 1, -1,-1,-1,-1,-1,-1}, tAward[nIndex][3], "VipCard", "UÈn Linh cÊp 5 (NgÉu nhiªn)", 0, 1);
end

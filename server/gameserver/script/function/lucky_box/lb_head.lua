...
--	Observer:onEvent(SYSEVENT_GLOBAL_COST_IB, LB_FREE_COST_ITEM)
	...
	--¼ÇÂ¼Á¬Ðø³é½±Ê£Óà´ÎÊý
	gf_SetTaskByte(TASKID_LUCKYBOX_LOTTERY, 3, 0, TASK_ACCESS_CODE_LUCKY_BOX);
	------------------------Cµi ®Æt task cho viÖc nhËn tÝch lòy quay th­ëng
	local nCountLifeLuckyBox = GetTask(TASKID_LUCKYBOX_COUNT);
	SetTask(TASKID_LUCKYBOX_COUNT, nCountLifeLuckyBox+1);
	--KsgTask:Set(TASKID_LUCKYBOX_COUNT, nCountLifeLuckyBox+1, TASK_ACCESS_CODE_LUCKY_BOX);
...
----------------------------------------Quay th­ëng
function QuayThuong_GetAccumulateAward(nIndex)
	local g_szQuayThuongNpcName = "<color=green>Hoµn Nhan Hy Y<color>: "
	local nQuayThuongCount = WTask:Get(TASKID_LUCKYBOX_COUNT);
	local szQuayThuongCount = format("<enter>(Sè lÇn rót th­ëng may m¾n cña ®¹i hiÖp hiÖn t¹i lµ <color=green>%d<color> lÇn)", nQuayThuongCount)
	if nIndex then

		local nStatus = WTask:GetBit(TASKID_LUCKYBOX_AWARD, nIndex)
		--local nStatus = gf_SetTaskByte(TASKID_LUCKYBOX_LOTTERY, 3, nLotteryCount, TASK_ACCESS_CODE_LUCKY_BOX);
		if not g_tbQuayThuong_AccumulateAward[nIndex] then
			return KsgNpc:FormalTalk(g_szQuayThuongNpcName, "H×nh nh­ chän sai råi, phÇn th­ëng nµy hiÖn kh«ng cã ë chç ta")
		end

		local tbAccumulate = g_tbQuayThuong_AccumulateAward[nIndex]
		if nQuayThuongCount < tbAccumulate.nRequireNum then
			return KsgNpc:FormalTalk(g_szQuayThuongNpcName, format("ch­a ®¹t ®ñ %d lÇn tÝch lòy rót th­ëng, kh«ng thÓ nhËn phÇn th­ëng mèc nµy %s", tbAccumulate.nRequireNum, szQuayThuongCount))
		end
		if nStatus == 1 then
			return KsgNpc:FormalTalk(g_szQuayThuongNpcName, "®· nhËn phÇn th­ëng nµy råi.")
		end
		if not KsgLib:HasEnoughBagRoom(3, 10, g_szQuayThuongNpcName) then
			return
		end

		WTask:SetBit(TASKID_LUCKYBOX_AWARD, nIndex, 1,TASK_ACCESS_CODE_LUCKY_BOX)
		KsgAward:Give(tbAccumulate.tbAward, "NhËn th­ëng tÝch lòy quuay th­ëng " .. nIndex)
		----------------------------------------------------------------------
		if tbAccumulate.nRequireNum == EVENT_QUAYTHUONG_MAX_LOTTERY_NUM then
			WTask:Set(TASKID_LUCKYBOX_AWARD, 0,27);
			SetTask(TASKID_LUCKYBOX_COUNT, 0);
			--KsgTask:Set(TASKID_LUCKYBOX_LOTTERY, 0, TASK_ACCESS_CODE_LUCKY_BOX)
			WNpc:Talk("Sè lÇn rót th­ëng ®· ®­îc ®Æt l¹i. §¹i hiÖp cã thÓ tiÕp tôc tÝch lòy rót th­ëng.",1)
		end
		------------------
	end
	-----------------------------------------
	local szTitle = g_szQuayThuongNpcName .. "Rót th­ëng may m¾n ®¹t sè lÇn nhÊt ®Þnh th× ®¹i hiÖp sÏ nhËn ®­îc phÇn th­ëng thªm"
	local tbStatus = { "Ch­a nhËn", "§· nhËn" }
	local tSay = {
		szTitle .. szQuayThuongCount
	}
	for nAwardIndex, tbAccumulate in g_tbQuayThuong_AccumulateAward do
		--local nStatus = GetBit(TASKID_LUCKYBOX_AWARD, 2 + nAwardIndex) + 1
		local nStatus = WTask:GetBit(TASKID_LUCKYBOX_AWARD,nAwardIndex) + 1
		local szStatus = tbStatus[nStatus]
		tinsert(tSay, format("TÝch lòy rót th­ëng %d lÇn nhËn ®­îc %s (%s)/#QuayThuong_GetAccumulateAward(%d)", tbAccumulate.nRequireNum, tbAccumulate.szName, szStatus, nAwardIndex))
	end

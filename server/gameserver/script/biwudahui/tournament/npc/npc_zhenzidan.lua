...
function main_bwdh()
	if g_WCFG_BIWUDAHUI_1VS1:IsDateOpen(1) ~= WEnv.TRUE then
...
--		{"Ç§Àï×··ç(ºì)","gf_AddItemEx({0,105,107,1,1,-1,-1,-1,-1,-1,-1},'Ç§Àï×··ç(ºì)')"},
		{"Ngo¹i trang l­ng: Tinh Kú (Kh«i Thñ)","season_add_guajian(1)"},
...
--		{"Ç§Àï×··ç(ºì)","gf_AddItemEx({0,105,107,1,1,-1,-1,-1,-1,-1,-1},'Ç§Àï×··ç(ºì)')"},
		{"Ngo¹i trang l­ng: Tinh Kú (KiÒu Së)","season_add_guajian(2)"},
--		{"ĞÂ±ÈÎä´ó»áÈü¼¾°ü40¸ö","gf_AddItemEx({2,95,820,40,1},'ĞÂ±ÈÎä´ó»áÈü¼¾°ü')"},
--		{"³ÆºÅ£º¾ÅÖİÎäÊ¥£¨Òş²ØÓĞĞ§£©","season_add_title(5,3,'¾ÅÖİÎäÊ¥')"},
	},
	{
		...
		{"Ngo¹i trang l­ng: Tinh Kú (Anh KiÖt)","season_add_guajian(3)"},
--		{"ĞÂ±ÈÎä´ó»áÈü¼¾°ü40¸ö","gf_AddItemEx({2,95,820,40,1},'ĞÂ±ÈÎä´ó»áÈü¼¾°ü')"},
...
function GLB_BW_Goto_Field(bConfirm)
	...
	if WPlayer:GetBigLevel() < g_WCFG_BIWUDAHUI_1VS1.nBigLevelReq then --LongDaiKa: mod yªu cÇu tham gia 1v1
		WNpc:Talk(format("§¼ng cÊp cña ng­¬i kh«ng ®ñ %s, h·y cè g¾ng luyÖn tËp thªm.", WLib:Level2String(g_WCFG_BIWUDAHUI_1VS1.nBigLevelReq)), WEnv.TRUE);
		return 0;
	end

	if BWT_CheckBiWuTime() ~= 1 then
		Talk(1,"",g_szInfoHead..format("HiÖn nay kh«ng ph¶i thêi gian cuéc dù tuyÓn cña ®¹i héi tû vâ toµn quèc, xin ®¹i hiÖp mçi ngµy (trõ chñ nhËt) trong %s ®Õn ®©y.", "18:00-19:00"));
		return 0;
	end
--	if GetXYB() <= 0 then

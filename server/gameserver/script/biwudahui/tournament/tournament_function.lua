...
function BWT_ReportResult(nWinnerIdx,nLoserIdx,nResultType)
	...
	Msg2MSAll(MISSION_ID,"Ng­êi ch¬i "..BWT_GetName(nWinnerIdx).." g©y ra: "..tbDamageInfo2[2]..", tû lÖ s¸t th­¬ng:"..tbDamageInfo2[1].."%");
	Msg2MSAll(MISSION_ID,"Ng­êi ch¬i "..BWT_GetName(nLoserIdx).." g©y ra: "..tbDamageInfo1[2]..", tû lÖ s¸t th­¬ng:"..tbDamageInfo1[1].."%");
	local nOldPIdx = PlayerIndex
	if nResultType == 1 then	--´òÆ½ÁË
		...
		BWT_AddPoint(-1);
		merit_1V1(-1);
		FireEvent("event_mission_award", "biwudahui", nCurGs, 4, 0)
		PlayerIndex = nLoserIdx;
		Say("Hai bªn hßa, trõ <color=yellow>1<color> ®iÓm so tµi.",0);
		Msg2Player("B¹n bŞ trõ 1 ®iÓm so tµi");
		BWT_AddPoint(-1);
		...
		FireEvent("event_mission_award", "biwudahui", nCurGs, 2, 0)
		PlayerIndex = nLoserIdx;
		local nLostMerit = _merit_GetMerit();
		local nLostLevel = GetLevel() + gf_GetPlayerRebornCount() * 10;
		FireEvent("event_mission_award", "biwudahui", nCurGs, 3, 0)
		--¸ø½±Àø
		PlayerIndex = nWinnerIdx;
		...
		if nCurGs == 1 then
			--ÔÚ¿ç·şÇø
			SendScript2VM("\\script\\misc\\fair\\fair.lua", format("ggs_do_fair_task([[%s]])", "1v1_battle"));
		end
		BWT_AddPoint(nPoint);
...
function BWT_GetPairGestPlayer(nOrder)
	...
	local nMaxFitIndex = 0 --×îÆ¥ÅäµÄÍæ¼ÒĞÅÏ¢
	local nMaxFitPointRangeLv = 99999
	...
	for i = 1, nMaxQueueIdx do
		...
		if 1 == bPass then
			...
			if nTempPointRangeLv < nMaxFitPointRangeLv then
				nMaxFitIndex = nTempPIdx
				nMaxFitPointRangeLv = nTempPointRangeLv
			end
		end
	end

	if 0 == nPIdx2 then
		if 0 == nMaxFitIndex then
...
--		GLB_BW_SetTask(0,3);--Í¬²½åĞÒ£Óñ
	...
	WriteLog(format("[1v1] [Role:%s Acc:%s level:%d score:%d route:%d result:win balanceflag:%d cnt:%d/%d]", GetName(), GetAccount(), GetLevel(),GetTask(TSK_CURREALRESULT),GetPlayerRoute(),nResultType, GetTask(TSK_CURWEEKMATCH), REQUIRE_MATCH_TIME));
	PlayerIndex = nLoserIdx;
...
--		GLB_BW_SetTask(0,3);
	...
	WriteLog(format("[1v1] [Role:%s Acc:%s level:%d score:%d route:%d result:lose balanceflag:%d cnt:%d/%d]", GetName(), GetAccount(), GetLevel(),GetTask(TSK_CURREALRESULT),GetPlayerRoute(),nResultType, GetTask(TSK_CURWEEKMATCH), REQUIRE_MATCH_TIME));
	PlayerIndex = nOldPIdx;
end;
--»ñµÃÖÜÊı
...
function BWT_WeeklyClear()
	...
	if nBodyWeek - nWeekNum >= 1 then	--ºÏ·şºónWeekNum»á´Ó0¿ªÊ¼
		WriteLog(format("[1v1] BWT_WeeklyClear [Role:%s Acc:%s Level:%d BodyWeek=%d WeekNum=%d repair BodyWeek ]", GetName(), GetAccount(), GetLevel(),nBodyWeek, nWeekNum))
...
--		"ÁìÈ¡ÉÏÖÜÅÅÃû½±Àø/BWT_GetRankAward",
...
function get_win_rate()
	local nMatch = GetTask(TSK_CURWEEKMATCH)
	local nRate = 0
	if nMatch >= WIN_AWARD_MIN_MATCH then
		local nWin = GetTask(TSK_CURWEEKWIN)
		nRate = floor(nWin * 100 / nMatch+0.5)
	end
	return nRate
end

function BWT_GetWeekAward(confirm)
--	local strtab = {
--		"ÁìÈ¡Ê¤¸º³¡ÊıµÄ½±Àø/BWT_GetWeekAward_Win",
...
----	end
--	Say(g_szInfoHead.."ÄúÒªÁìÈ¡ÄÄÖÖ½±Àø£¿",
--		getn(strtab),
--		strtab)
	if not confirm then
		local szTitle = format("Mçi tuÇn tham gia thi ®Êu ®ñ %d trËn cã thÓ nhËn phÇn th­ëng phong phó, tham gia thi ®Êu ®¹t %d/%d/%d trËn sÏ nhËn thªm phÇn th­ëng trang søc ngo¹i trang.",
			REQUIRE_MATCH_TIME, tTotalAward[3][1], tTotalAward[2][1], tTotalAward[1][1])
		szTitle = format("%s\n tham gia thi ®Êu ®¹t %d trËn, x¸c suÊt th¾ng (Sè trËn th¾ng/Sè trËn tham gia) ®¹t %d%%/%d%%/%d%% trËn sÏ nhËn thªm phÇn quµ Danh HiÖu hÊp dÉn.", 
			szTitle, WIN_AWARD_MIN_MATCH, tWinAward[3][1], tWinAward[2][1], tWinAward[1][1])
		szTitle = format("%s\n HiÖp SÜ tuÇn nµy th¾ng/sè trËn tham gia lµ <color=yellow>%d/%d<color> x¸c suÊt th¾ng lµ <color=yellow>%d%%<color>", szTitle, GetTask(TSK_CURWEEKWIN), GetTask(TSK_CURWEEKMATCH), get_win_rate())
		
		local tSay = {
			"X¸c nhËn nhËn th­ëng/#BWT_GetWeekAward(1)",
			"KÕt thóc ®èi tho¹i/nothing",
			}
	...
	if gf_GetTaskByte(TASK_BIWU_GET_AWARD_10,1) ~= 0 then
		Talk(1,"",g_szInfoHead.."PhÇn th­ëng tuÇn ®· nhËn");
		return 0;
	end
	...
	local nExp = 200000000
	gf_Modify("Exp",nExp);
	--gf_AddItemEx({2,1,30499,5},"Hu©n ch­¬ng anh hïng");
	--gf_AddItemEx({2,1,30692,15},"R­¬ng §¹i Héi Tû Vâ");
	gf_AddItemEx({2,1,31174,5,4},"PhiÕu §iÓm Liªn §Êu");
	AddRuntimeStat(20, 1, 0, 1)
	AddRuntimeStat(20, 4, 0, 10)
	getWeekTotalMatchAward()
	getWeekWinMatchAward()
	gf_SetLogCaption("");
end

tTotalAward = {
	{60,10000000000, {
		--{ tbProp = { 2, 1, 40016 }, nStack = 5, nStatus = 4 }
		{ tbProp = { 2, 1, 40048 }, nStack = 100, nStatus = 4 }, -- M¶nh Bİ §iÓn 27
		{ tbProp = { 2, 1, 31174 }, nStack = 10, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
	{50,8000000000, {
		--{ tbProp = { 2, 1, 40016 }, nStack = 3, nStatus = 4 }
		{ tbProp = { 2, 1, 40048 }, nStack = 50, nStatus = 4 }, -- M¶nh Bİ §iÓn 27
		{ tbProp = { 2, 1, 31174 }, nStack = 10, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
	{40,5000000000, {
		--{tbProp = { 2, 1, 40016 }, nStack = 1, nStatus = 4}
		{ tbProp = { 2, 1, 40048 }, nStack = 30, nStatus = 4 }, -- M¶nh Bİ §iÓn 27
		{ tbProp = { 2, 1, 31174 }, nStack = 10, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
}
function getAwardIdx(t, v)
	local r = 0
	for i = 1,getn(t) do
		if v >= t[i][1] then
			r = i
...
function getWeekTotalMatchAward()
	local nAwardId = getAwardIdx(tTotalAward, GetTask(TSK_CURWEEKMATCH))
	if nAwardId <= 0 then return end

	local nExp = tTotalAward[nAwardId][2]
	local tbAward = tTotalAward[nAwardId][3]
	if tbAward then
		KsgAward:Give(tbAward, "L«i ®µi LSV 1 vs 1 - Th­ëng §ñ trËn")
	end
	gf_Modify("Exp",nExp);
	season_add_guajian(nAwardId)
end
...
function season_award_title(nRank)
	...
	if IsTitleExist(nG,nDetal) <= 0 then
		if AddTitle(nG,nDetal) > 0 then
			SetTitleTime(nG,nDetal, GetTime() + 7*24*3600);
...
tWinAward = {
	{70,3,10, {
		{ tbProp = { 2, 1, 31174 }, nStack = 20, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
	{50,2,6, {
		{ tbProp = { 2, 1, 31174 }, nStack = 10, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
	{30,1,3, {
		{ tbProp = { 2, 1, 31174 }, nStack = 5, nStatus = 4 }, -- PhiÕu §iÓm Liªn §Êu
	}},
}
function getWeekWinMatchAward()
	local nAwardId = getAwardIdx(tWinAward, get_win_rate())
	if nAwardId <= 0 then return end
	
	local nItem1, nItem2 = tWinAward[nAwardId][2], tWinAward[nAwardId][3]
	season_award_title(nAwardId)
	--gf_AddItemEx({2,1,30693,nItem1,}, "R­¬ng S­ M«n Gia T­ëng")
	--gf_AddItemEx({2,1,31196,nItem2,}, "Gãi quµ QuyÓn tİch ®iÓm Liªn §Êu")
	WAward:Give(tWinAward[nAwardId][4], "L«i ®µi LSV 1 vs 1 - Th­ëng tØ lÖ th¾ng ".. get_win_rate());
end

--function BWT_GetWeekAward_Gold()
--	if GetTask(TSK_CURWEEKWIN) < 10 then
--		Talk(1,"",g_szInfoHead.."Äú±¾ÖÜ±ÈÎä´ó»áÊ¤³¡²»×ã10³¡£¬ÎŞ·¨ÁìÈ¡½±Àø¡£");
--		return 0;
--	end
--	if gf_GetTaskByte(TASK_BIWU_GET_AWARD_10,2) ~= 0 then
--		Talk(1,"",g_szInfoHead.."ÄãÒÑ¾­ÁìÈ¡ÁË±¾ÖÜµÄ½£ÏÀ±ÒµÄ½±Àø¡£");
--		return 0;
--	end
--	gf_SetTaskByte(TASK_BIWU_GET_AWARD_10,2,1, TASK_ACCESS_CODE_BIWUDAHUI);
--	gf_SetLogCaption("±ÈÎä´ó»á½£ÏÀ±Ò½±Àø");
...
--	if GetTask(TSK_CURWEEKMATCH) < REQUIRE_MATCH_TIME then
--		Talk(1,"",g_szInfoHead.."±¾ÖÜÄã²Î¼Ó½ÏÒÕµÄ³¡´ÎĞ¡ÓÚ<color=yellow>"..REQUIRE_MATCH_TIME.."<color>³¡£¬²»ÄÜÁìÈ¡½±Àø¡£");
--		return 0;
--	end
--	if gf_GetTaskByte(TASK_BIWU_GET_AWARD_10,1) ~= 0 then
--		Talk(1,"",g_szInfoHead.."ÄãÒÑ¾­ÁìÈ¡¹ı±¾ÖÜ½±Àø¡£");
--		return 0;
--	end
--	
...
--		gf_SetLogCaption("±ÈÎä´ó»áÊ¤¸º³¡Êı½±Àø")
...
--		--gf_Modify("Dzjy",2400);
--		gf_Modify("Exp",nExp);
--		gf_AddItemEx({2,1,30499,2},"Ó¢ĞÛÑ«ÕÂ");
--		gf_AddItemEx({2,1,30692,10},"±ÈÎä´ó»á±¦Ïä");
--		AddRuntimeStat(20, 1, 0, 1)
--		AddRuntimeStat(20, 4, 0, 10)
--		gf_SetLogCaption("");
...
--		szString1 = "<color=red>ÒÑÁìÈ¡<color>";
--		szString2 = "<color=red>ÒÑÁìÈ¡<color>";
--		szString3 = "<color=red>ÒÑÁìÈ¡<color>";
--	elseif nGetXinDeState == 20 then
--		szString1 = "<color=red>ÒÑÁìÈ¡<color>";
--		szString2 = "<color=red>ÒÑÁìÈ¡<color>";
--		szString3 = "<color=yellow>Î´ÁìÈ¡<color>";
--	elseif nGetXinDeState == 10 then
--		szString1 = "<color=red>ÒÑÁìÈ¡<color>";
--		szString2 = "<color=yellow>Î´ÁìÈ¡<color>";
--		szString3 = "<color=yellow>Î´ÁìÈ¡<color>";
--	else
--		szString1 = "<color=yellow>Î´ÁìÈ¡<color>";
--		szString2 = "<color=yellow>Î´ÁìÈ¡<color>";
--		szString3 = "<color=yellow>Î´ÁìÈ¡<color>";
--	end;
--
--	if nCount > 0 then
--		gf_AddItemEx({2,1,1101,nCount},"ÊµÕ½ĞÄµÃ");
--		Talk(1,"",g_szInfoHead.."ÄúÄ¿Ç°µÄÁìÈ¡Çé¿öÈçÏÂ£º\n»ı·Ö120·Ö¿ÉÒÔµÃµ½10±¾ÊµÕ½ĞÄµÃ       "..szString1.."\n»ı·Ö150·Ö¿ÉÒÔ¶àµÃµ½10±¾ÊµÕ½ĞÄµÃ     "..szString2.."\n»ı·Ö200·Ö¿ÉÒÔÔÙµÃµ½10±¾ÊµÕ½ĞÄµÃ     "..szString3);
--	else
--		Talk(1,"",g_szInfoHead.."ÄúÄ¿Ç°µÄÁìÈ¡Çé¿öÈçÏÂ£º\n»ı·Ö120·Ö¿ÉÒÔµÃµ½10±¾ÊµÕ½ĞÄµÃ       "..szString1.."\n»ı·Ö150·Ö¿ÉÒÔ¶àµÃµ½10±¾ÊµÕ½ĞÄµÃ     "..szString2.."\n»ı·Ö200·Ö¿ÉÒÔÔÙµÃµ½10±¾ÊµÕ½ĞÄµÃ     "..szString3);
--	end;
--end
...
--		Talk(1,"",g_szInfoHead.."ÄãÉÏÖÜµÄ»ı·ÖÅÅÃû²¢Ã»ÓĞ½øÈëÇ°10£¬ËùÒÔÃ»ÓĞ×Ê¸ñÁìÈ¡½±Àø¡£");
--		return 0;
--	end;
--	local selTab = {};
...
----	end;
--	tinsert(selTab,format("ÅÅÃû½±Àø/#BWT_GetRankAward_Exp(%d)", nBWRank));
--	tinsert(selTab,"Ôİ²»ÁìÈ¡½±Àø/nothing");
--	Say(g_szInfoHead.."ÉÏÖÜÄãµÄ»ı·ÖÅÅÃûÎªµÚ<color=yellow>"..nBWRank.."<color>Ãû£¬ÄãÒªÁìÈ¡ÄÄÖÖ½±Àø£¿",getn(selTab),selTab);
--	BWT_PointAttenuation();
--end
--
...
--		Talk(1,"",g_szInfoHead.."ÄãÒÑ¾­ÁìÈ¡¹ı½±ÀøÁË¡£");
--		return 0;
--	end;
--	if nBWRank >= 1 and nBWRank <= 10 then
--		local nExp = 100000000--(11 - nBWRank)*10000*10000
--		gf_SetLogCaption("±ÈÎä´ó»áÃ¿ÖÜÅÅÃû½±Àø")
--		BWDH_SetTask(TSK_GET_AWARD_WEEK,nWeekNum);
--		gf_Modify("Exp",nExp);
--		gf_AddItemEx({2,1,30499,6},"Ó¢ĞÛÑ«ÕÂ");
--		gf_AddItemEx({2,1,30692,30},"±ÈÎä´ó»á±¦Ïä");
...
--		Talk(1,"",g_szInfoHead.."Äú»¹ÊÇÏÈ×°±¸°Ñ³ÃÊÖµÄ±øÆ÷ÔÙÀ´ÕÒÎÒ°É£¡")
--		return 0;
--	end
--	local nType = 0;
...
--	end;
--	local selTab = {
--				"È·¶¨/#BWT_AddWeaponEffect("..nType..")",
--				"È¡Ïû/nothing",
--				}
--	Say(g_szInfoHead.."ÁìÈ¡±ÈÎä´ó»á³ÆºÅ½«»á<color=yellow>¸²¸ÇÄãµ±Ç°Í¬ÀàµÄ±ÈÎä´ó»á³ÆºÅ»òÊ¦ÃÅ³ÆºÅ<color>£¬ÄãÈ·¶¨ÏÖÔÚ¾ÍÒªÁìÈ¡±ÈÎä´ó»áµÄ³ÆºÅ½±ÀøÂğ£¿",getn(selTab),selTab);
--end;
--
...
--		return 0;
--	end
--	local tbWeaponEffect = {"ÆÆ¾ü","ĞÇŞÈ"};
...
--	Msg2Player("ÄãÁìÈ¡ÁËÎäÆ÷ÌØĞ§£º"..szWeaponEffectName.."£¬ÌØĞ§³ÖĞøÊ±¼äÎª7Ìì¡£");
--	WriteLog("["..LOG_HEAD.."]:"..GetName().."ÁìÈ¡ÁËÎäÆ÷ÌØĞ§½±Àø:"..szWeaponEffectName);
--end;
...
--	end;
--	local selTab = {
--				"È·¶¨/#BWT_AddBiWuTitle("..nTitleGenre..","..nRoutePos..")",
--				"È¡Ïû/nothing",
--				}
--	Say(g_szInfoHead.."ÁìÈ¡±ÈÎä´ó»á³ÆºÅ½«»á<color=yellow>¸²¸ÇÄãµ±Ç°Í¬ÀàµÄ±ÈÎä´ó»á³ÆºÅ»òÊ¦ÃÅ³ÆºÅ<color>£¬ÄãÈ·¶¨ÏÖÔÚ¾ÍÒªÁìÈ¡±ÈÎä´ó»áµÄ³ÆºÅ½±ÀøÂğ£¿",getn(selTab),selTab);
--end;

...
function CallBackStepInq(szKey, nKey1, nKey2, nCount)
	local tong_ID,tong_level,tong_num,tong_state,tong_level_max, tong_version, data_count = GetRelayShareDataByKey(szKey, nKey1, nKey2,"0")
	DelRelayShareDataCopy(szKey,nKey1,nKey2)
	--Êı¾İÕûÀíÅĞ¶Ï--
	if tong_level_max == nil or tong_level_max <= 0 or not data_count or tong_version == nil or tong_version < g_nVersion then
	...
	local dialog_date_now = "12:00 ®ªm CN tuÇn nµy"					--±¾ÂÖÈÎÎñ½áÊøÊ±¼ä
	local dialog_date_next =""					--ÏÂÂÖÈÎÎñ¿ªÊ¼Ê±¼ä
--	if Zgc_conf_task_dist_date() ==  1 then
--		dialog_date_now = "½ñÍí24µã"
--		dialog_date_next = "Ã÷Íí24µã"
--	else
--		dialog_date_now = "Ã÷Íí24µã"
--		dialog_date_next = "ºóÌìÍíÉÏ24µã"
--	end
...
function Zgc_conf_task_accept(task_level,level_max)
	...
	AddRuntimeStat(85, 1, 0, 1)
	AddRelayShareData(GetTongName(), 0, 0, File_name, "cb", 0,"0", "dddddd", tong_ID,tong_level,tong_num,tong_state,tong_level_max, g_nVersion)
...
function CallBackGoodsin(szKey, nKey1, nKey2, nCount)
	...
	if tong_state ~= 1 then																--¸Ã°ï»áÒÑ¾­ÁìÈ¡ÈÎÎñ£¬µ«ÊÇÊÕ¼¯ÎïÆ·ÊıÁ¿²»×ã
		local cent_remain = 0
		...
		if cent_remain > 0 then
			...
			if  goods_num > 0 and goods_cent > 0 then										
				if goods_cent >= cent_remain then													--ÎïÆ·ÊÕ¼¯ÊÇ·ñµ½´ï400ÉÏÏŞ
					...
					local dialog_date = "TuÇn Sau"
--					if Zgc_conf_task_dist_date() ==  1 then
--						dialog_date = "Ã÷Ìì"
--					else
--						dialog_date = "ºóÌì"
--					end
					AddRuntimeStat(89, 1, 0, 1)
...
function CallBackConfDeal(szKey, nKey1, nKey2, nCount)
	...
	local dialog_date = "12:00 ®ªm CN tuÇn nµy"
--	if Zgc_conf_task_dist_date() ==  1 then
--		dialog_date = "½ñÍí24µã"
--	else
--		dialog_date = "Ã÷Íí24µã"
--	end
	local need_num = ""
	...
	if player_goods_num >= (Zgc_conf_task_goos_num[tong_level][1] - tong_num ) and tong_state == 2 then
		...
		AddRuntimeStat(86, tong_level, 0, 1)
		if tong_level_max < getn(Zgc_conf_task_goos_num) then
			tong_level_max = tong_level_max + 1
			SendTongMessage("Quı bang ®· hoµn thµnh nhÖm vô Bé Phi Yªn, cã thÓ nhËn"..tong_level_max.."!!")
		end
		tong_num = tong_num + player_goods_num
		AddRelayShareData(GetTongName(), 0, 0, File_name, "cb", 0,"0", "dddddd", tong_ID,tong_level,Zgc_conf_task_goos_num[tong_level][1],3,tong_level_max,g_nVersion)
	else
		tong_num = tong_num + player_goods_num
		AddRelayShareData(GetTongName(), 0, 0, File_name, "cb", 0,"0", "dddddd", tong_ID,tong_level,tong_num,tong_state,tong_level_max, g_nVersion)
	end
...
function CallBackDataRepair(szKey, nKey1, nKey2, nCount)
	...
	AddRelayShareData(GetTongName(), 0, 0, File_name, "cb", 0,"0", "dddddd", tong_ID,tong_level,tong_num,tong_state,tong_level_max, tong_version)
...
function Zgc_conf_task_time_return()
	local time_return = tf_GetWeekSequence(1,0); --¶¨Î»µ½ÖÜÒ»Áè³¿
	return time_return
end
--**************************ÊıÁ¿ºÍÈÕÆÚ·Ö×°º¯Êı*************************
...
function Zgc_conf_task_dist_date()
	local date_tomorrow = floor((GetTime() + 28800)/(604800))

...
function g_t3v3Data:resetDate(tData)
	...
	if ndateMatchStart and (ndateMatchStart < 20161114 or ndateMatchStart > minStartDate:tonumber())  then --ÕýÊ½¿ªÈüÖ®Ç°Ç¿ÖÆÇåÀíÈü³Ì
		DebugOutput("C­ìng chÕ reset lÞch thi ®Êu bÊt th­êng",today, ndateMatchStart, ndateFinal, PreLiminaryFinish, FinalsFinish, nMatchRound)
...
function update3v3MatchRank_Msg(nCount, s)
	...
	if bAwarded == 1 then
		local msg = "§¹i héi tØ vâ 3 vs 3 ®· ®i ®Õn quý ®Êu míi!"
		msg = msg.."Ng­êi ch¬i cã thÓ ®i bµn ®å BiÖn Kinh ®æi míi xÕp h¹ng 3 vs 3 quý tr­íc vµ nhËn th­ëng."
		Msg2Global(msg);
		AddGlobalNews(msg, 1);
		local localSDB = SDB(SDBKEY_3V3, 2, 0);
...
function get_sort_value(pidx)
	...
	if type(pidx) == "table" then
		...
		PlayerIndex = pidx or PlayerIndex
		nVal = get_lv_stage() * 10000*10000 + GetTask(TASKID_3V3_WIN_ROUND_DAILY)*100 + get_rand_value(100) --×îºó¼ÓÒ»¸öËæ»úÖµ£¬Ê¤³¡Ò»ÑùµÄË³ÐòËæ»ú£¬·ÀÖ¹×îºóÒ»¸öÈËÊ¼ÖÕÆ¥Åä²»ÉÏ
	end
	PlayerIndex = nOldPlayerIndex
...
--ÊÇ·ñÎª¿ªÈüÈÕ 0 ·Ç±ÈÈüÈÕ 1ÈÕÈü 2ÌÔÌ­Èü 3¾öÈü

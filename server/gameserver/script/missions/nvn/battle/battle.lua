...
function checkTeamBattleResult()
	--ÓÅÏÈ±ÈÈËÍ·
	if GetMissionV(MV_TEAM_ALIVE_1) > GetMissionV(MV_TEAM_ALIVE_2) then
		return 1
	elseif GetMissionV(MV_TEAM_ALIVE_1) < GetMissionV(MV_TEAM_ALIVE_2) then
		return 2
	end
...
function missionAward_3v3(result)
	...
	for i = 1,getn(nWinTb) do
		...
		if nCurGs == 1 then
			--ÔÚ¿ç·þÇø »¯¾³ÈÎÎñ
			SendScript2VM("\\script\\misc\\fair\\fair.lua", format("ggs_do_fair_task([[%s]])", "3v3_battle"));
		end
		...
		strWinRoute = strWinRoute.."<enter/>"..format(msgFmt, GetName(), gf_GetRouteName(), gf_GetTransLevel(),
	...
	for i = 1,getn(nLoseTb) do
		...
		strLoseRoute = strLoseRoute.."<enter/>"..format(msgFmt, GetName(), gf_GetRouteName(), gf_GetTransLevel(),
			GetTask(TASKID_3V3_BATTLE_POWER),
			GetTaskTemp(TEMP_TASKID_3V3_KILLED), GetTaskTemp(TEMP_TASKID_3V3_IS_DEAD),
			GetTask(TASKID_3V3_VIC_RATE)/100);
		GDS_writeTask(5, 0, "3v3", g_tSyncTaskList);

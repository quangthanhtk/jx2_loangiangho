...
function OnBossDeath(npcIdx)
	...
	gf_TeamOperateEX(ht_GetTongChip, 25, 1);
	
	--通关奖励
	if GetMissionV(MV_GAME_OVER_TAG) >= 4 then
		gf_TeamOperateEX(function()
			SendScript2VM("\\script\\online_activites\\head\\activity_normal_head.lua", "vaf_award_api('tLiangShanFinish')");
		end)
	end

	-- --发布事件 CRT中已经做了
	-- FireEventTeam("event_kill_npc", GetNpcName(npcIdx), GetNpcWorldPos(npcIdx), npcIdx)
end

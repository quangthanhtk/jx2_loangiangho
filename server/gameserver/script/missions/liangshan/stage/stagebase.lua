...
--	if nStage < 7 then
--		--Observer:onEvent(OE_LS_CREATE_NPC,{m, x, y}); --pvpµØÍ¼´«ÈëmapIDºÍÕóÓª£¬pve´«ÈëmapIDºÍ×ø±ê
--	end
end
...
function tStageBase:giveAward(nStage)
	local fawrd = function(nStage)
		...
		local ExpAddRate = (1 + nBongLaiAddRate + GetVipCardParam() + T_ROUTE_CNT_EXP_ADD_RATE[GetMissionV(MV_LS_ROUTE_COUNT)])*(1+LS_EXP_ADD_RATE)
		if not nStage then
			nStage = GetMissionV(MV_STAGE_LEVEL)
    		if nStage >= HIDEMISSION_BEGIN then	-- Òþ²Ø¹Ø¿¨µÄÏÂÒ»¹Ø
...
--		end
		...
		--gf_Modify("Exp", floor(nExp)); --LongDaiKa: mod ®­a phÇn th­ëng vÒ 1 hµm WAward:LuongSonBac_KillBoss
		WAward:LuongSonBac_KillBoss(nStage);
	end
...
function tStageBase:OnBossDeath(npcIdx)
	...
	self:BossDownAddWXXS(npcIdx)--²»ÔÙµôÂäÎåÐÐÐþÊ¯
	StopMissionTimer(MISSION_ID, TIMER_ID);
...
--==:´å³¤ 5-25-2011Ôö¼Ó»÷°ÜBOSS»ñµÃÎåÐÐÐþÊ¯½±Àø
function tStageBase:BossDownAddWXXS(npcIdx)
	FireEvent("event_mission_affairs", "liangshan", "killboss", npcIdx)

...
function preStart()
    ...
    if state ~= SS_WAITING_2 and state ~= SS_WAITING then
        Talk(1, "","§· th«ng quan,nhanh chèng rêi khái!");
        return
    end
    if g_WCFG_LUONGSONBAC.bDisableDropBox == WEnv.TRUE then --LongDaiKa: mod bá drop r­¬ng
    else -- gèc
    ...
    if g_WCFG_LUONGSONBAC.bConsumeToFree == WEnv.TRUE then --LongDaiKa: mod bá tÝnh n¨ng gäi boss tiªu hao Thiªn Kiªu LÖnh
        ...
        if DelItem(T_TIANJIAOLING[1],T_TIANJIAOLING[2],T_TIANJIAOLING[3],N_CALL_BOSS_TIANJIAOLING_NEED) == 1 then
            ...
            _stat_when_ib_consume(N_CALL_BOSS_TIANJIAOLING_NEED)--IBÏûºÄÍ³¼Æ
...
function preClose()
    ...
    if state == SS_WAITING_2 or state == SS_WAITING or state == SS_PASSING then
        _stat_when_all_pass()--Êý¾ÝÍ³¼Æ
        _op_on_team(_stat_on_player_when_all_pass)--Êý¾ÝÍ³¼Æ
...
function tStage_6:OnBossDeath(npcIdx)
    ...
    if nBossCount >= self.nMaxBossCount then
        ...
        FireEvent("event_mission_stage_finish", "liangshan", 8)
        return
    end
    self:giveAward();
    SetMissionV(MV_BOSS_COUNT, nBossCount);
    SetMissionV(MV_STAGE_STATE,SS_WAITING_2);
    tMission:dropBox(npcIdx, 1);--ºÃººÏä

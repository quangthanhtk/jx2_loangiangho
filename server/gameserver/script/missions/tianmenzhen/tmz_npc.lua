...
function tmz_check_time(bNotify)
	...
	if (not ((1100 <= nTime and nTime < 1300) or (1600 <= nTime and nTime < 1800))) then
		if bNotify and 1==bNotify then
			Talk(1,"",g_NpcName..format("DiÔn vâ vÉn ch­a chuÈn bŞ xong, thiÕu hiÖp h·y ®Õn tham gia vµo lóc %s.", "11:00~13:00, 16:00~18:00"))
		end
		return 0
	end
	return 1
...
function tmz_single_join()
	...
	if nLv < PLAY_NEED_LV then
		if nCount_CS <= 0 and nPhucSinh <= 0 then
			Talk(1,"",g_NpcName.."LÇn diÔn vâ nµy kh«ng ph¶i tÇm th­êng, thiÕu hiÖp trÎ ng­êi non da khã tr¸nh khái th­¬ng vong. C¸c h¹ ®¹t ®Õn cÊp <color=yellow>90<color> h·y ®Õn tham gia.")
			return 0
		end
	end
	--ÈëÁ÷ÅÉ²Å¸ø½ø
	...
	if gf_Check55MaxSkill() == 0 then
		Talk(1,"","B¹n ch­a häc tèi ®a kü n¨ng 55 nªn kh«ng thÓ tham gia chiÕn tr­êng")
		return
	end	
	if GetTask(336) < 4000 then
		Talk(1,"","B¹n kh«ng ®ñ 4000 ®iÓm s­ m«n ®Ó tham gia chiÕn tr­êng!")
		return
	end	
	if GetReputation() < 3000 then
		Talk(1,"","B¹n kh«ng ®ñ 3000 ®iÓm danh väng ®Ó tham gia chiÕn tr­êng!")
		return
	end	
	if abs(GetTask(701)) < 50000 then
			Talk(1,"",szNpcName.."B¹n kh«ng cã ®ñ 50000 ®iÓm qu©n c«ng ®Ó tham gia chiÕn tr­êng.")
			return 
	end		
	if GetCash() < 80000 then
		Talk(1,"","§¹i hiÖp kh«ng cã 8 vµng")
		return
	end
...
function tmz_team_join()
	...
	if tmz_LvCheck() == 0 then
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn ch­a ®¹t cÊp 90.");
		return 0;
	end
	if tmz_SkillCheck() == 0 then
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn ch­a häc tèi ®a kü n¨ng 55.");
		return 0;
	end	
	if tmz_SuMonCheck() == 0 then
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn kh«ng ®ñ 4000 ®iÓm S­ m«n.");
		return 0;
	end	
	if tmz_DanhVongCheck() == 0 then
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn kh«ng ®ñ 3000 ®iÓm Danh väng.");
		return 0;
	end	
	if tmz_QuanCongCheck() == 0 then
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn kh«ng ®ñ 50000 ®iÓm Qu©n c«ng.");
		return 0;
	end				
...
function tmz_LvCheck()
	local nCount_CS = GetTranslifeCount()
	local nPhucSinh = GetPlayerRebornParam(0)		
	if nCount_CS > 0 or nPhucSinh > 0 then
		return 1
	end
	local nTeamSize = 0;
	local nOldIndex = PlayerIndex;
...
function tmz_SkillCheck()
	local nTeamSize = 0;
	local nOldIndex = PlayerIndex;
	for i=1, GetTeamSize() do
		PlayerIndex = GetTeamMember(i)
		local nSkill = gf_Check55MaxSkill();
		if nSkill == 0 then
			PlayerIndex = nOldIndex;
			return 0;
		end
...
function tmz_DanhVongCheck()
	local nTeamSize = 0;
	local nOldIndex = PlayerIndex;
	for i=1, GetTeamSize() do
		PlayerIndex = GetTeamMember(i)
		local nDanhVong = GetReputation();
		if nDanhVong < 3000 then
			PlayerIndex = nOldIndex;
			return 0;
		end
	end
...
function tmz_QuanCongCheck()
	local nTeamSize = 0;
	local nOldIndex = PlayerIndex;
	for i=1, GetTeamSize() do
		PlayerIndex = GetTeamMember(i)
		local nQuanCong = abs(GetTask(701));
		if nQuanCong < 50000 then
			PlayerIndex = nOldIndex;
			return 0;
		end
	end
...
function  tmz_TeamCashCheck()
 ...
 function tmz_TeamTalk(g_NpcName)
	...
	for i=1, GetTeamSize() do
		PlayerIndex = GetTeamMember(i)
		Talk(1,"",g_NpcName.."Trong ®éi ngò cã thµnh viªn ch­a cã 8 vµng");
	end
	PlayerIndex = nOldIndex;
 end
...
function tmz_confirm_award(nType)
	...
	if type(tbNeedItem[1]) == "string" then
		...
		FireEvent("event_ib_cost",tbNeedItem[2],tbNeedItem[3],tbNeedItem[4],tbNeedItem[5]);
	end
...
--	end
	...
	if bIsWin == 1 then
		...
		--NhËn qu©n c«ng cho server Thien Hæ
		if GetGlbValue(GLB_TSK_SERVER_ID) == 100 and nDate <= 20150719 then
...
--	AwardZhenQi_TianMenZhen(nActiveLevel, bIsWin)--ÌìÃÅÕó»ñÊ¤½±ÀøÕæÆø
	...
	FireEvent("event_mission_award", "tianmenzhen", nType, bIsWin, 0)
end
...
function tmz_tong_award_1_2(nType)
	...
	if nType ~= 0 then
		...
		FireEvent("event_ib_cost", tAwardItem[nType][2],tAwardItem[nType][3],tAwardItem[nType][4],nBagCount2);
	end

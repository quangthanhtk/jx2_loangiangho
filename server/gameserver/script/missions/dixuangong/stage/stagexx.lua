...
function tStageXX:giveAward(nNpcIdx, nBossCount)
    ...
    if nBossCount == 4 then
        ...
        --cdkey»î¶¯
        g_theMS.msCamp[1]:turnPlayer(function()
            SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(1203)");
        end);
    else
        for i = 1, 4 do
    ...
    g_theMS.msCamp[1]:turnPlayer(giveStageExpAward)
    ...
    TriggerTeamMisEvent("event_mission_passstage", "§Þa HuyÒn Cung", 3 + nBossCount);
end
...
function phasexx_1:onTalk(npcIdx)
    ...
    if g_WCFG_DIAHUYENCUNG.bConsumeToFree == WEnv.TRUE then --LongDaiKa: mod bá tÝnh n¨ng gäi boss tiªu hao thiªn kiªu lÖnh, s­a l¹i ®èi tho¹i
        tSel[1] = "Truy t×m KÎ Chñ M­u/perStart";
        Say("§¸m Minh Gi¸o Th¸m Tö: §¹i ©n bÊt ng«n t¹, sù tÝch c¸c vÞ ®¸nh tan §Þa HuyÒn Cung sÏ ®­îc d©n gian truyÒn tông! T×m gÆp thªm cao thñ ®Ó biÕt ®­îc ai lµ chñ m­u!", getn(tSel), tSel);
    else
...
function perStart()
    if g_WCFG_DIAHUYENCUNG.bDisableDropBox == WEnv.TRUE then --LongDaiKa: mod t¾t tÝnh n¨ng më r­¬ng
    else -- gèc
	...
	if GetMissionV(MV_PHASE) == 1 then
        if g_WCFG_DIAHUYENCUNG.bConsumeToFree == WEnv.TRUE then --LongDaiKa: mod bá tÝnh n¨ng gäi boss tiªu hao thiªn kiªu lÖnh
...
function phasexx_2:onNpcDeath(npcIdx)
	...
	FireEvent("event_mission_affairs", "dixuangong", "killboss", npcIdx)
end
...
function phasexx_3:onInit()
    g_theMS.msTimer:setInterval(self.stepInterval);
    g_theMS:Msg2MSAll("Qua nç lùc hÕt m×nh, c¸c ng­¬i ®· v­ît qua ®­îc §Þa HuyÒn Cung, ")

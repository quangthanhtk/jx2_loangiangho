...
Import("\\script\\missions\\tong_mission\\main_head.lua");
Import("\\script\\online_activites\\task_values.lua");--ÈÎÎñ±äÁ¿Í·ÎÄ¼ş
...
--¸ù¾İµØÍ¼ID£¬ÇøÓòË÷Òı£¬³¡µØË÷ÒıÈıÕß»ñÈ¡³¡µØµÄµØÍ¼ID
...
--¼ì²âÕıÑ¡Íæ¼ÒµÄ×é³É½á¹¹£ºÄ³¸öÁ÷ÅÉÕıÑ¡ÈËÊıÊÇ·ñ³¬¹ı3¡£·ûºÏ¹æÔò·µ»Ø1£¬²»·ûºÏ¹æÔò·µ»Ø0
function TM_CheckNormalPlayer(nRoute)
	local tbNormalPlayer = TM_GetPlayerTable();
	local nOldPlayerIdx = PlayerIndex;
	local tbRoutCount = {};	
	for i=0,32 do	--³õÊ¼»¯tbRoutCount
		tbRoutCount[i] = 0;
	end;
...
--ÉèÖÃÄ³¸ö³¡µØµÄĞÅÏ¢£º±ê¼ÇÕâ¸ö³¡±»Ä³¸ö°ï»áÕ¼ÓĞ
...
function TM_CreateBox(nStage)
	local nMapID = SubWorldIdx2ID(SubWorld);
	local nNpcIdx = CreateNpc("R­¬ng tiÒn","R­¬ng ¶i "..nStage,nMapID,1569,3175);
...
--ĞŞ¸ÄNPCÑªÁ¿£¬²ÎÊı1£ºNPCË÷Òı£¬²ÎÊı2£ºÑªÁ¿Öµ£¬²ÎÊı3£ºÊÇ·ñÎª°Ù·Ö±È£¬1±íÊ¾ÊÇ£¬0±íÊ¾²»ÊÇ£¨Ä¬ÈÏÊÇ1£©
...
--»ñÈ¡NPC×î´óÑªÁ¿ºÍµ±Ç°ÑªÁ¿£¬²ÎÊı¶ş±íÊ¾µ±Ç°ÑªÁ¿ÊÇ·ñÎª°Ù·Ö±È,1±íÊ¾ÊÇ£¬0±íÊ¾²»ÊÇ£¬Ä¬ÈÏÊÇ1
...
function TM_AddPoint(nPoint,nCurStage,tbPlayer)
	if tbPlayer == nil then --Ä¬ÈÏÊÇËùÓĞÍæ¼Ò
	...
	if GetMissionV(MV_KILL_SUB_NPC) >= MIN_KILL_SUB_NPC_COUNT then
		...
		Msg2MSAll(MISSION_ID,"§· giÕt vÖ binh ngo¹i ®­êng ë ¶i nµy v­ît qu¸ "..MIN_KILL_SUB_NPC_COUNT..", tÊt c¶ mäi ng­êi sÏ ®­îc th­ëng thªm "..KILL_SUB_NPC_POINT.."®iÓm phÇn th­ëng tİch lòy");
	end;
...
--		ĞÂÔö½±Àø¶Ø»ÍĞş¾§£¬°Ù·ÖÖ®5¸ÅÂÊ¸øÓè¶ÓÎéÖĞµÄËæ»úÒ»¸öÈË
	--KSGVN xoa Don Hoang Huyen Tinh
--	local funAddItem = function(nArg)
--		local nSuc = 0;
--		if PlayerIndex == nArg then
--			nSuc = AddItem(2,1,31120,1)
--			if nSuc == 1 then
--				Msg2Global("Chóc mõng "..GetName().." nhËn ®­îc 1 §«n Hoµng HuyÒn Tinh ë Tö Quang C¸c.")
--				Msg2All("Chóc mõng "..GetName().." nhËn ®­îc 1 §«n Hoµng HuyÒn Tinh ë Tö Quang C¸c.")
--			end
--		end
--	end
--	local nAwardflag = random(1,100)
--	local nPlayerflag = -1;
--	if nAwardflag <= 5 then 
--		nPlayerflag = random(1,getn(tbPlayer))
--	end
--	if nPlayerflag == -1 then return 0 end;
--	local nAwardIndex = tbPlayer[nPlayerflag]
--	gf_OperatePlayers(tbPlayer,funAddItem,nAwardIndex);
end;
--»ñµÃ¹ÖµÄÃÅÅÉ
	...
	if GetTask(TSK_WEEK) < nCurWeek then
		...
		SetTask(TSK_ATTEND_STAGE_6,0);
                if tonumber(date("%w")) ~= 6 and tonumber(date("%w")) ~= 0 then
                    SetTask(2122, 0);--Éè¶¨µ½ÁËĞÇÆÚÁù¿ÉÒÔÖØÖÃ
                end
        else

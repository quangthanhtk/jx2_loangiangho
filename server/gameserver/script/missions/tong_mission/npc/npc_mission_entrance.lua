...
Import("\\script\\missions\\tong_mission\\main_function.lua");
...
function get_award()
	local selTab = {
				"§Æc s¶n Tö Quang C¸c (dïng ®Ó nép cèng hiÕn Bang Héi)/get_tong_materials",
...
function get_award_use_point()
	local selTab = {
--				"? §Æc s¶n Tö Quang C¸c/get_tong_materials",
...
function get_tong_materials()  -- TSK_POINT = 652
	local selTab = {
				"Tö Quang Kim Ti (Tèn 150 ®iÓm/c¸i)/#get_tong_materials_confirm(1)",
				"Tö Quang L©n Vò (Tèn 150 ®iÓm/c¸i)/#get_tong_materials_confirm(2)",
				"Kú L©n HuyÕt (Tèn 150 ®iÓm/c¸i)/#get_tong_materials_confirm(3)",
				"M¶nh Hµnh Thó (Tèn 150 ®iÓm/c¸i)/#get_tong_materials_confirm(4)",
				"Tö Quang DiÖm CÈm (Tèn 150 ®iÓm/c¸i)/#get_tong_materials_confirm(5)",
				"trë l¹i/get_award_use_point"
				}
	Say(g_szInfoHeader.."Ng­¬i ®ang cã <color=yellow>"..GetTask(TSK_POINT).."<color>NhÊp ¶i tÝch ®iÓm, ng­¬i muèn ®æi lÊy vËt phÈm nµo d­íi ®©y?",getn(selTab),selTab);
end

function get_tong_materials_confirm(nType)
	local szFunc = format("get_tong_materials_confirm_CB%d",nType);
	local nMax = floor(GetTask(TSK_POINT)/150)
	nMax = min(nMax , 99)
	AskClientForNumber(szFunc, 1, nMax, "§æi bao nhiªu c¸i?");
end
function get_tong_materials_confirm_CB(nType,nNum)
	local tItem = {
		{PackItemId(2,0,737),"Tö Quang Kim ThiÒn Ti"},
		{PackItemId(2,0,736),"Tö Quang Kú L©n Vò"},
		{PackItemId(2,0,739),"Th­îng Cæ Kú L©n HuyÕt"},
		{PackItemId(2,0,738),"M¶nh TiÖt Hoµnh Thó"},
		{PackItemId(2,0,735),"Tö Quang LiÖt DiÖm CÈm"},
	}
	...
	local nCost = nNum * 150
	if nCurpoint < nCost then
		Talk(1,"","§æi thÊt b¹i")
		return
	end
	SetTask(TSK_POINT,nCurpoint-nCost)
	local g,d,p = UnPackItemId(tItem[nType][1])
	gf_AddItemEx({g,d,p,nNum}, tItem[nType][2]);
end

function get_tong_materials_confirm_CB1(nNum)get_tong_materials_confirm_CB(1,nNum)end
function get_tong_materials_confirm_CB2(nNum)get_tong_materials_confirm_CB(2,nNum)end
function get_tong_materials_confirm_CB3(nNum)get_tong_materials_confirm_CB(3,nNum)end
function get_tong_materials_confirm_CB4(nNum)get_tong_materials_confirm_CB(4,nNum)end
function get_tong_materials_confirm_CB5(nNum)get_tong_materials_confirm_CB(5,nNum)end
...
function get_special_equip_affirm(nEquipType,nEquipIdx)
	local selTab = {
				format("§ång ý ®æi ./#get_special_equip_confirm(%d,%d)",nEquipType,nEquipIdx),

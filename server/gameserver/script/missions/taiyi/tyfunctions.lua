...
function TY_GetExpAward(nMSType, nDifType, nStageId)
	...
	local nExp = TYT_EXP_AWARD[nMSType][nDifType][nStageId] * (1 + nBongLaiAddRate +  nRAdd + GetVipCardParam() + VNG_CFG_EXTRA_TYT_ADD_RATE); 
...
function TY_StagePassed(tStage, tPos)
	...
	if tMainLevelID[tStage.nStageId] then
		...
		if nStageId == 6 then --Í¨¹Ø
			...
			--cdkey»î¶¯
			this.msCamp:turnPlayer(0, function() 
				SendScript2VM("\\script\\function\\cdkey\\ck_head.lua", "ck_global_set_task_finish(1204)");
			end);
		end
		...
		if GetMissionV(this.mv_02) == eDifType.NORMAL then
			...
			FireEvent("event_mission_stage_finish", "taiyi", tMainLevelID[tStage.nStageId])
			TriggerTeamMisEvent("event_mission_passstage", "Th¸i NhÊt Th¸p", tMainLevelID[tStage.nStageId]);
		else
			...
			FireEvent("event_mission_stage_finish", "taiyihero", tMainLevelID[tStage.nStageId])
			TriggerTeamMisEvent("event_mission_passstage", "Th¸i NhÊt Th¸p_Anh Hïng", tMainLevelID[tStage.nStageId]);
		end
...
function TY_CheckAndCostItem(szNpcName, nMSType, nDifType, bOnlyCheck)
	...
	FireEvent("event_ib_cost", tCostItem[1][1], tCostItem[1][2], tCostItem[1][3], tCostItem[1][4])
	return 1;
end
...
function TY_EnterCondCheck(szNpcName, nMSType, nDifType)
	...
	if tResInfo and getn(tResInfo) > 0 then
		if eDifType.NORMAL == nDifType then
			Talk(1, "", format(szNpcName.."Ph¶i tæ ®éi míi cã thÓ vµo %s. Thµnh viªn trong ®éi ph¶i <color=yellow>80 cÊp trë lªn, sè thµnh viªn h¬n 1 ng­êi, vµ ®· gia nhËp l­u ph¸i<color>.", tMSName[nMSType]));
		else
			Talk(1, "", format(szNpcName.."Ph¶i tæ ®éi míi cã thÓ vµo %s. Thµnh viªn trong ®éi ph¶i <color=yellow>80 cÊp trë lªn, ph¶i h¬n 4 lo¹i l­u ph¸i, sè thµnh viªn h¬n 4 ng­êi, vµ ®Òu ®· gia nhËp l­u ph¸i<color>.", tMSName[nMSType]));
		end

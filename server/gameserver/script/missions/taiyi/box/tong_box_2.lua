----Í­±¦Ïä2
--Import("\\script\\lib\\globalfunctions.lua")
--Include("\\script\\missions\\taiyi\\tyfunctions.lua")
--Include("\\script\\online_activites\\head\\activity_normal_head.lua")
--
--function main()
--	local tAward = {
--		{5, {"ËÄÁé¹éÔªµ¤",{2, 1, 30352, 1, 4}}},
--		{10, {"ÅàÔªµ¤",{2,	1, 30351, 1, 4}}},
--		{10, {"´«¹¦µ¤",{2,	1, 30313, 1, 4}}},
--		{5, {"ÐÞÕæÒª¾÷²ÐÒ³",{2,	1, 30315, 1, 4}}},
--		{20, {"1¼¶Ìú¾§",{2,	1, 30533, 1, 4}}},
--		{15, {"2¼¶Ìú¾§",{2,	1, 30534, 1, 4}}},
--		{15, {"3¼¶Ìú¾§",{2,	1, 30535, 1, 4}}},
--		{5, {"Ñ×ÁéË¿",{2,	1,	30671, 1}}},
--		{5, {"Ä§ÀÇÆ¤",{2,	1,	30672, 1}}},
--		{5, {"µØÁúÆ¤",{2,	1,	30673, 1}}},
--		{5, {"²ÐÆÆµÄµ¶ÇÊ", {2,	1,	30694, 1}}},
--	}
--	local tItem = gf_RandRate(tAward)[2];
--	gf_ThrowDice(tItem[2], nil, nil, nil, nil, GetTargetNpc());
--	SetNpcLifeTime(GetTargetNpc(), 0);
--	SetNpcScript(GetTargetNpc(), "");	
--	if tItem[1] == "Ñ×ÁéË¿" then
--		AddRuntimeStat(18, 44, 0, 1);
--	elseif tItem[1] == "Ä§ÀÇÆ¤" then
--		AddRuntimeStat(18, 45, 0, 1);
--	elseif tItem[1] == "µØÁúÆ¤" then
--		AddRuntimeStat(18, 46, 0, 1);
--	end
--	
--	gf_TeamOperateEX(vaf_award_api, "tTYTTongBox")
--end
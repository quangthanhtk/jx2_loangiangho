...
function main()
	local tSay = {}
	if GetMissionV(KF_MV_BATTLE_STATE) == KF_MS_STATE_FIGHT then
	...
	Say("<color=green>Tèng Nguyªn So¸i: <color>"..
		...
		format("C¶nh M«n (%s), DiÖt M«n (%s), Sinh M«n (%s), Tö M«n (%s), §¹i TrËn Kú (%s).", tTemp[GetMissionV(KF_MV_BATTLE_FLAG_JM)], 
		tTemp[GetMissionV(KF_MV_BATTLE_FLAG_MM)], tTemp[GetMissionV(KF_MV_BATTLE_FLAG_SM)], tTemp[GetMissionV(KF_MV_BATTLE_FLAG_SI)], tTemp[GetMissionV(KF_MV_BATTLE_FLAG_DA)]),
		getn(tSay), tSay)
end

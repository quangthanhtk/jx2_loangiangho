...
KF_NPC_NAME = "<color=green>ChiÕn tr­êng liªn ®Êu: <color>"
KF_BATTLE_NEED_TRANS = 8
KF_BATTLE_NEED_LEVEL = 92
KF_BATTLE_NEED_JUNGONG = 100000

function main()
	if g_WCFG_CHIENTRUONG_LSV:IsDateOpen(1) ~= WEnv.TRUE then
		return WEnv.RETCODE_NOTOPEN;
	end

	local szTitle = KF_NPC_NAME..format("H·y chän chiÕn tr­êng muèn vµo!")
	local tbSay = {}
	if IsKfNewBattleOpen() ~= 0 then
		tinsert(tbSay, format("Ph¸o §µi ChiÕn Liªn Server/#main_kf_newbattle(%d)", EMPLACEMENT_ID) )
		tinsert(tbSay, format("Tham gia Nh¹n M«n Quan Liªn Server-ChiÕn Tr­êng ChÝnh/#main_kf_newbattle(%d)", MAINBATTLE_ID) )
	end
	tinsert(tbSay, "Thiªn M«n TrËn Liªn Server/main_kf_tmz")
	tinsert(tbSay, "C¸o Tõ/nothing")
	Say(szTitle, getn(tbSay), tbSay)
end

function main_kf_tmz()
	local tSay = {};
	local nJoinTimes = KF_GetJoinTimes();
	tSay.msg = KF_NPC_NAME..format("§¹i hiÖp muèn tham gia chiÕn tr­êng liªn server kh«ng? Trong chiÕn tr­êng liªn server, b¹n cã thÓ chiÕn ®Êu mét c¸ch s¶ng kho¸i víi hµo kiÖt cña toµn vâ l©m, kiÕn c«ng lËp nghiÖp vµ nhËn ®­îc phÇn th­ëng quý hiÕm. HiÖn t¹i chØ më cho nh©n vËt chuyÓn sinh %d cÊp %d trë lªn, ®· gia nhËp l­u ph¸i vµ ®· häc tÊt c¶ kü n¨ng cÊp %d, ®ång thêi mçi ng­êi chØ cã thÓ tham gia %d lÇn/ngµy. §¹i hiÖp h«m nay ®· tham gia <color=green>%d<color> lÇn.", KF_BATTLE_NEED_TRANS, KF_BATTLE_NEED_LEVEL, 55, KF_JOIN_TIMES_MAX, nJoinTimes);
...
function jyr_check_time()
	--Ê±¼äÏÞÖÆ
	local nTime = tonumber(date("%H%M"));
	if (not ( 1630 <= nTime and nTime < 1730) ) then
		Talk(1,"",KF_NPC_NAME..format("ChiÕn tr­êng liªn server ch­a më, ®¹i hiÖp h·y b¸o danh vµo lóc %s hoÆc %s ®Ó tham gia.", "16:30", ""));
		return 0;
	end
...
function jyr_single_join()
	if jyr_check_time() ~= 1 then
		return 0;
	end

	if gf_GetTransLevel() < KF_BATTLE_NEED_TRANS*100+KF_BATTLE_NEED_LEVEL then
		Talk(1,"",format("CÊp nh©n vËt ch­a ®¹t ®Õn %d chuyÓn cÊp %d.", KF_BATTLE_NEED_TRANS, KF_BATTLE_NEED_LEVEL));
		return 0;
	end

	--¾ü¹¦ÒªÇó
	if  abs(GetTask(701)) < KF_BATTLE_NEED_JUNGONG then
		Talk(1,"",format("Qu©n c«ng ch­a ®¹t %d.", KF_BATTLE_NEED_JUNGONG));
		return 0;
	end
	...
	if nCurMapId ~= 0 then
		if mf_GetPlayerCount(KF_MISSION_ID, KF_CampOne_ID, nCurMapId) <= mf_GetPlayerCount(KF_MISSION_ID, KF_CampTwo_ID, nCurMapId) then
			mf_JoinMission(KF_MISSION_ID, KF_CampOne_ID, nCurMapId);
		else
			mf_JoinMission(KF_MISSION_ID, KF_CampTwo_ID, nCurMapId);
		end
	else
		local nMissionCount = GetMapTaskTemp(KF_MAP_TASK_TEMP_ID, KF_CURR_OPEN_COUNT);
		if nMissionCount >= KF_MISSION_COUTN_MAX then
		...
		if SubWorldID2Idx(nLastMapID) ~= -1 then
			local nLoadTag = mf_GetMissionV(KF_MISSION_ID, KF_MV_BATTLE_LOAD, nLastMapID);
			local nMissionState = mf_GetMissionV(KF_MISSION_ID, KF_MV_BATTLE_STATE, nLastMapID);
			if nLoadTag == 1 then --ÉÏÒ»³¡ÒÑ¿ªÆô
				if nMissionState == KF_MS_STATE_PEACE or nMissionState == KF_MS_STATE_READY then --ÉÏÒ»³¡¿ªÆôµÄ½×¶Î»¹Ã»½øÈëÕ½¶·½×¶Î
		...
		if nMapID == 0 and nMapIdx == 0 then
			Talk(1,"","XuÊt hiÖn lçi ch­a x¸c ®Þnh, chiÕn tr­êng kh«ng thÓ më");
			return 0;
		end
		if mf_OpenMission(KF_MISSION_ID, nMapID) == 1 then
			...
			mf_SetMissionV(KF_MISSION_ID, KF_MV_BATTLE_LOAD, 1, nMapID);	
			mf_SetMissionV(KF_MISSION_ID, KF_MV_BATTLE_MAP_ID, nMapID, nMapID);
			mf_SetMissionV(KF_MISSION_ID, KF_MV_BATTLE_MAP_INDEX, nMapIdx, nMapID);
			mf_JoinMission(KF_MISSION_ID, KF_CampOne_ID, nMapID);	
...
function jyr_team_join()
	...
	msg = msg.."<color>\nNh©n vËt ch­a ®ñ qu©n c«ng gåm:\n<color=red>"
	for i = 1, getn(tResult[5]) do
		msg = msg..tResult[5][i].."   ";
		bTag = 1;
	end
	msg = msg.."<color>"
...
function jyr_get_last_award()
	...
	local strTemp = format("ChiÕn tr­êng lÇn tr­íc cña b¹n <color=green>%s<color>, ®é tÝch cùc lµ <color=green>%d<color>#", tTempString[nRet], KF_GetActivity(), tResult[1], tResult[2]);
	tSay.sel = {};
	if nRet > 0 and nActivity >= 300 then
		if GetTask(TASKID_KF_POZHENKING) == 1 then
			strTemp = strTemp.."NhËn th­ëng <color=yellow>Vua Ph¸ TrËn<color>, th­ëng c«ng tr¹ng trËn nµy +20%."
		end
		...
		strTemp = strTemp.."RÊt tiÕc, ®é tÝch cùc cña ngµi ch­a ®¹t ®ñ 300, chØ ®­îc nhËn th­ëng EXP."
		tinsert(tSay.sel, {"NhËn th­ëng EXP", "kf_expaward"})
	end
...
function jyr_battle_introduce()
	local tSay = {};
	local string1 = format("ChiÕn tr­êng liªn server më h»ng ngµy vµo %s, %s giê, hiÖn t¹i chØ më cho nh©n vËt chuyÓn sinh %d cÊp %d trë lªn, ®· gia nhËp l­u ph¸i, häc tÊt c¶ kü n¨ng cÊp 55, ®ång thêi mçi ng­êi chØ ®­îc tham gia %d lÇn/ngµy.\n", "16:30", "", KF_BATTLE_NEED_TRANS, KF_BATTLE_NEED_LEVEL, KF_JOIN_TIMES_MAX);
	local string2 = "ChiÕn tr­êng th¾ng lîi: muèn nhËn ®­îc th¾ng lîi cÇn ph¶i nhËn ®­îc ®Çy ®ñ ®iÓm tÝch lòy chiÕn tr­êng. §iÓm tÝch lòy chiÕn tr­êng nÕu bªn nµo ®¹t ®Õn sè ®iÓm giíi h¹n tr­íc sÏ giµnh th¾ng lîi. Giíi h¹n ®iÓm tÝch lòy th«ng qua nh©n sè khi chiÕn tr­êng b¾t ®Çu ®Ó quyÕt ®Þnh, nh©n sè cµng nhiÒu th× chiÕn tr­êng cµng kÞch liÖt ®iÓm tÝch lòy yªu cÇu sÏ cµng nhiÒu. §iÓm tÝch lòy chiÕn tr­êng cã thÓ th«ng qua tiªu diÖt ng­êi ch¬i ®èi ph­¬ng, binh lÝnh ®èi ph­¬ng, t­íng lÜnh, ®o¹t kú ®Ó nhËn ®­îc. \n"
	local string3 = "PhÇn th­ëng chiÕn tr­êng: Muèn nhËn ®­îc phÇn th­ëng tèt sau khi chiÕn ®Êu, cÇn ph¶i nç lùc giµnh ®­îc th¾ng lîi. PhÇn th­ëng bªn th¾ng sÏ tèt h¬n phÇn th­ëng bªn thua. Ngoµi ra trong chiÕn tr­êng cã thÓ th«ng qua viÖc ®¸nh b¹i ng­êi ch¬i ®èi ph­¬ng, binh lÝnh, t­íng lÜnh, qu¸i, ®o¹t kú vµ nhiÒu h×nh thøc ®Ó nhËn ®­îc ®iÓm n¨ng ®éng. §iÓm n¨ng ®éng cµng cao th× sÏ nhËn ®­îc mét sè phÇn th­ëng v­ît møc. \n"
	local string4 = "Lîi Ých chiÕn tr­êng: trong chiÕn tr­êng cã rÊt nhiÒu qu¸i vËt trung lËp, ®¸nh b¹i nã th× toµn ®éi cã thÓ nhËn ®­îc hiÖu qu¶ lîi Ých. Trong chiÕn tr­êng cßn cã 2 Boss, ®¸nh b¹i nã kh«ng nh÷ng nhËn ®­îc trang bÞ tèt, mµ c¶ ®éi cßn nhËn ®­îc hiÖu qu¶ lîi Ých. \n"
	--local string3 = "Ã¿ÌìµÚÒ»´Î»ñµÃ¸ß¼¶½±ÀøÊ±»¹ÄÜ»ñµÃ¶îÍâµÄÓÂÊ¿Ñ«ÕÂ¡£½±ÀøÖÐµÄÓÂÊ¿Ñ«ÕÂ£¬Ó¢ÐÛÑ«ÕÂ¿ÉÒÔÓÃÀ´ºÍÎÒ¶Ò»»ð©ÔÂ£¬¾ÅÖÝ×°±¸¡£\n";
...
function jyr_jinshe_shop()
	local szTitle = format("NÕu chän sö dông ph­¬ng thøc n©ng cÊp ngò hµnh, xin chó ý sÏ<color=yellow> gi¶m ®é c­êng hãa<color>:\n")
	szTitle = format("%s 1. ChØ trang bÞ cã <color=yellow>®é c­êng hãa +13 vµ trë lªn<color> míi ®­îc tham gia n©ng cÊp\n", szTitle)
	szTitle = format("%s 2. N©ng cÊp trang bÞ xong <color=yellow> +13 ®æi thµnh +0, +14 +15 ®æi thµnh +7<color>\n", szTitle)
	szTitle = format("%s tÊt c¶ <color=yellow>hiÖu qu¶ ®Þnh hån kh«ng b¶o l­u<color>\n ", szTitle)
...
function jyr_junzhuang_shop()
	local szTitle = format("NÕu chän sö dông trang bÞ Th­¬ng ¦ng n©ng cÊp ®Õn trang bÞ L«i Hæ, xin chó ý sÏ <color=yellow>gi¶m ®é c­êng hãa<color>:\n")
	szTitle = format("%s 1. ChØ trang bÞ cã <color=yellow>®é c­êng hãa +14,+15 <color> míi ®­îc tham gia n©ng cÊp\n", szTitle)
...
function jyr_ys_shop()
	local szTitle = format("NÕu chän sö dông trang bÞ H¹o NguyÖt n©ng cÊp ®Õn trang bÞ DiÖu D­¬ng, xin chó ý <color=yellow>®é c­êng hãa gi¶m nöa<color>:\n")

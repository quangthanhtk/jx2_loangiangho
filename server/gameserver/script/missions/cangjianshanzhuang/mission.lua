...
function onPlayerDeath(nkiller)
	...
	if tCurPhase and tCurPhase.onPlayerDeath then
		...
		_stat_when_player_death(GetMissionV(tbJZMY.mv_stage))
	end
end
...
function stageFight1:onNpcDeath(nNpcIDX)
	local strNpcName = GetNpcName(nNpcIDX);
	local npcs = get_npcs()
	if strNpcName == npcs.boss1[2] then
	...
	elseif strNpcName == npcs.fish1_1[2] or strNpcName == npcs.fish1_2[2] or strNpcName == npcs.fish1_3[2] then
...
function stageFight1.callBoss()
	...
	if GetMissionV(this.mv5) == 0 then
		...
		if nCurFish == 3 then
			if nCurNum <= 0 then
				if nCurTime <= 0 then
					local nNpcIDX = ttd_createSomeNpc(get_npcs().boss1,tPos.boss1);
					if nNpcIDX ~= 0 then
		...
		if nCurNum <= 0 then
			if nCurTime <= 0 then
				...
				if nPreStageStatus ~= 0 then
    				...
    				for i = 1,self.fishNum[nCurFish][1][2] do
    					local nRand = random(1,getn(self.fishPos))
    					ttd_createSomeNpc(npcs.fish1_1,self.fishPos[nRand],{{1689,3154}});
    				end
    				for j = 1,self.fishNum[nCurFish][2][2] do
    					local nRand = random(1,getn(self.fishPos))
    					ttd_createSomeNpc(npcs.fish1_2,self.fishPos[nRand],{{1689,3154}});
    				end
    				for k = 1,self.fishNum[nCurFish][3][2] do
    					local nRand = random(1,getn(self.fishPos))
    					ttd_createSomeNpc(npcs.fish1_3,self.fishPos[nRand],{{1689,3154}});
    				end			
    				SetMissionV(this.mv2,nCurFish);
    				SetMissionV(this.mv3,nFishNum);
    				...
    				this.msCamp:turnPlayer(0, HeadMsg2Player, szMsg);
    				if 1 == nCurFish and  get_open_type() ~= tbJZMY.open_type_hard then
    					local nJTYLidx = ttd_findNpc(this.szNpc_jiangbiehe);
						local nLFidx = ttd_findNpc(this.szNpc_renwoxing);
						local nQRSidx = ttd_findNpc(this.szNpc_renyingying);
						--3¸öNPC±äÕ½¶·NPC
...
function stageFight1.callFish2()
	self = stageFight1;
	local nBossIdx = ttd_findNpc(get_npcs().boss1[2]);
	if nBossIdx ~= 0 and IsNpcDeath(nBossIdx) ~= 1 then
		...
		if (nNum == 0 and nPercent <= 75) or (nNum == 1 and nPercent <= 50) or (nNum == 2 and nPercent <= 25) then
			local nNewBoss = ttd_createSomeNpc(get_npcs().boss1_1,{{x,y}});
			if nNewBoss ~= 0 then
...
function stageFight2:onNpcDeath(nNpcIDX)
	local strNpcName = GetNpcName(nNpcIDX);
	local npcs = get_npcs()
	if strNpcName ==npcs.boss2[2] then
		ttd_passStage(2,strNpcName);
		local szMsg = format("V­ît ¶i thµnh c«ng")
    	this.msCamp:turnPlayer(0, HeadMsg2Player, szMsg);
	elseif strNpcName == npcs.fish2_1[2] or strNpcName == npcs.fish2_2[2] or strNpcName == npcs.fish2_3[2] then
...
function stageFight2.callBoss()
	...
	if GetMissionV(this.mv9) == 0 then
		...
		if nCurFish == 3 then
			if nCurNum <= 0 then
				if nCurTime <= 0 then
					local nNpcIDX = ttd_createSomeNpc(get_npcs().boss2,tPos.boss2);
					if nNpcIDX ~= 0 then
				...
				if nPreStageStatus ~= 0 then
					...
					for x = 1, 3 do
						...
						if t then
							...
							for i = 1,nCurNum do
    							local nRand = random(1,getn(self.fishPos))
    							local tt = get_npcs()[t[1]]
    							ttd_createSomeNpc(tt,self.fishPos[nRand],{{1689,3154}});
    						end
						end
...
function stageFight2.callFish2()
	self = stageFight2;
	local nBossIdx = ttd_findNpc(get_npcs().boss2[2]);
	if nBossIdx ~= 0 and IsNpcDeath(nBossIdx) ~= 1 then
		...
		local m,x,y = GetNpcWorldPos(nBossIdx);
		if (nNum == 0 and nPercent <= 75) or (nNum == 1 and nPercent <= 50) or (nNum == 2 and nPercent <= 25) then
			local nNewBoss = ttd_createSomeNpc(get_npcs().fish3,{{x+1,y+1},{x-1,y-1}});
			if nNewBoss ~= 0 then
...
function stageFight3:onNpcDeath(nNpcIDX)
	local strNpcName = GetNpcName(nNpcIDX);
	local npcs = get_npcs()
	if strNpcName ==npcs.boss3[2] or strNpcName == npcs.boss4[2] or strNpcName == npcs.boss5[2] then
		ttd_removeNpc(nNpcIDX);
...
function stageFight3.callBoss()
	self = stageFight3;
	local npcs = get_npcs()
	local nBossIdx1 = ttd_findNpc(npcs.boss3[2]);
	local nBossIdx2 = ttd_findNpc(npcs.boss4[2]);
	local nBossIdx3 = ttd_findNpc(npcs.boss5[2]);
	local nPreStageStatus = GetMissionV(this.mv20);
	if nPreStageStatus == 0 then
		if GetMissionV(this.mv12) == 0 then
			if nBossIdx1 == 0 then
    			...
    			if nCurTime <= 0 then
    				local nBossIdx = ttd_createSomeNpc(get_npcs().boss3,tPos.boss3);
		...
		elseif GetMissionV(this.mv12) == 1 then
    		if nBossIdx2 == 0 then
    			local nCurTime = GetMissionV(this.mv13);	
    			if nCurTime <= 0 then
    				local nBossIdx = ttd_createSomeNpc(get_npcs().boss4,tPos.boss4);
    				SetCurrentNpcSFX(nBossIdx, 905, 0, 0); --ÌØÐ§
    				g_NpcAI:setAI(nBossIdx, AT_SM_ATTACK);
    				g_NpcAI:setWayPoint(nBossIdx, {{1686,3151}});
    	...
    	elseif GetMissionV(this.mv12) == 2 then
    		if nBossIdx3 == 0 then
    			local nCurTime = GetMissionV(this.mv13);	
    			if nCurTime <= 0 then
    				local nBossIdx = ttd_createSomeNpc(get_npcs().boss5,tPos.boss5);
    				SetCurrentNpcSFX(nBossIdx, 905, 0, 0); --ÌØÐ§
    				g_NpcAI:setAI(nBossIdx, AT_SM_ATTACK);
    				g_NpcAI:setWayPoint(nBossIdx, {{1686,3151}});
    	...
    	elseif GetMissionV(this.mv12) == 3 then
			...
			if nCurTime >= 8 then
				if nBossIdx1 == 0 then
        			...
        			local nCurTime = GetMissionV(this.mv13);	
        			if nCurTime <= 0 then
        				local idx1 = ttd_createSomeNpc(get_npcs().boss3,{1686,3151});
        				local idx2 = ttd_createSomeNpc(get_npcs().boss4,{1686,3151});
        				local idx3 = ttd_createSomeNpc(get_npcs().boss5,{1686,3151});
...
function stageFight3.doSkill()
	...
	if GetMissionV(this.mv12) >= 3 then
		local npcs = get_npcs()
		local nBossIdx3 = ttd_findNpc(npcs.boss3[2]);
		local nBossIdx4 = ttd_findNpc(npcs.boss4[2]);
		local nBossIdx5 = ttd_findNpc(npcs.boss5[2]);
...
function stageFight4:onNpcDeath(nNpcIDX)
	local strNpcName = GetNpcName(nNpcIDX);
	if strNpcName == get_npcs().boss7[2] then
...
function stageFight4.callFish()
	...
	if nPreStageStatus == 0 then
    	local nBossIdx = ttd_findNpc(get_npcs().boss7[2]);--"Ä§½£Å«"
    	if nBossIdx ~= 0 and IsNpcDeath(nBossIdx) ~= 1 then
...
--							SetMissionV(this.mv21, 0);
                        	...
                        	local nNewIdx = ttd_createSomeNpc(get_npcs().boss7,tPos.boss7); --Ä§½£Å«
                        	ttd_ChangeNpc2Talk(nNewIdx);
        ...
        --                	g_NpcAI:setWayPoint(nNewIdx, {{1698,3160}});
					...
					[nTalkB+29]  = {this.szNpc_mjn, "¡n ®­îc ch­a? T¹i h¹ ®ãi råi.",
						function()
							...
							local nNpcIdxMJN = ttd_findNpc(get_npcs().boss7[2]);
                        	...
                        	if get_open_type() ~= tbJZMY.open_type_hard then
                        		...
                        		NpcChat(nIdxRYY, "Ta bÞ nhèt kh«ng thÓ cö ®éng n÷a, c¸c vÞ h·y nhanh chãng tiªu diÖt KiÕm Ma N«!");
                        	end
                        	local nIdxJbh = ttd_findNpc(tNpcs.boss6[2]);--ÖØÖÃÒ»ÏÂ½­±ðº×AI ÃâµÃÕâ»õ²»´òÈÎÎÒÐÐ
                        	ttd_ChangeNpc2Fight(nIdxJbh);
						end
							},
				}
				if t_talk[nCurTime] then
					if t_talk[nCurTime][1] then
						local szNpcName = t_talk[nCurTime][1]
    					if szNpcName == this.szNpc_mjn then
    						local npcs = get_npcs()
    						szNpcName = npcs.boss7[2]
    					end
						local nNpcIdx = ttd_findNpc(szNpcName)
...
--	ttd_ChangeNpc2Talk(nNpcIDX);
--	ttd_createJTYL();
	local nNpcIDX = ttd_createSomeNpc(get_npcs().qrs,tPos.qrs);
	ttd_ChangeNpc2Talk(nNpcIDX);
	ttd_createWRX();
end

	...
	if nRemainCount == 0 then
		...
		if PlayerIndex > 0 then
			...
			if nCurRing == nRingCount then
				...
				FireEvent("event_mission_stage_finish", "taixu", nCurRing)
				TriggerTeamMisEvent("event_mission_passstage", "Th¸i H­ HuyÔn C¶nh", nCurRing);
				gf_TeamOperateEX(oly_AddShengHuo, 1, 3) --²ú³öÒ»¸ö»ÆÉ«Ê¥»ğÊ¯
				gf_TeamOperateEX(function()
					SendScript2VM("\\script\\online_activites\\head\\activity_normal_head.lua", "vaf_award_api('tTaiXuFinish')");
				end)
				return
			end

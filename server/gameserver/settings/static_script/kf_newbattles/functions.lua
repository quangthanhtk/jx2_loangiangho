...
function BT_AddPersonalPoint(nPoint,bNotShowMsg,bTeamPoint)
	...
	if nTeamMemberCount == 0 or FORBIDENT_CREATE_TEAM == 1 or bTeamPoint == 0 or bTeamPoint == nil then
		if BT_GetData(PTNC_BATTLEPOINT_KF) + nPoint >= POINT_LIMITED then
			nPoint = POINT_LIMITED - BT_GetData(PTNC_BATTLEPOINT_KF);
...
function BT_BattleStarAward()
	local IDTab = BT_SortMSPlayerByTaskValue(PTNC_BATTLEPOINT_KF,0);
	...
	if nPlayerCount > 0 then
		PlayerIndex = IDTab[1];
		local nFirstPoint = BT_GetData(PTNC_BATTLEPOINT_KF);
		for i=1,nPlayerCount do	--ÒòÎªÓĞ¿ÉÄÜÓĞ¶à¸ö×î¸ß·Ö£¬ËùÒÔ»¹ÊÇÒª±éÀúËùÓĞµÄÍæ¼ÒÒ»´Î
			PlayerIndex = IDTab[i];
			if BT_GetData(PTNC_BATTLEPOINT_KF) == nFirstPoint then
	...
	if nPlayerCount > 0 then
		Msg2MSAll(MISSION_ID,tBattleName[BT_GetData(PT_BATTLE_TYPE)].." sau khi kÕt thóc, vinh dù ng«i sao chiÕn tr­êng trao cho: "..strName..", xem nh­ biÓu d­¬ng cho "..sMul..", trong chiÕn dŞch nµy cã biÓu hiÖn kiÖt xuÊt!");
	end;
	PlayerIndex = nOldPlayerIndex;
end;
...
function BT_ProcessBattleOver()
	--=============================¹«¹²Êı¾İ
	BT_SetData(PTNC_BATTLEPOINT_KF,0);	--µ¥³¡»ı·ÖÇå¿Õ
...
function BT_RestorePlayerState()
	...
	SetCampToPlayer("");
	KillFollower();	--É¾³ı¸ú°à
	gf_ForbidFactionSkill(0);	--È¡ÏûÆÁ±ÎÕòÅÉ
...
function BT_ReportFinalResult()
	...
	sPersonalPoint = sStrResult.."<enter>ChiÕn dŞch lÇn nµy, §é Tİch Cùc cña ®¹i hiÖp ®¹t <color=yellow>"..BT_GetData(PTNC_ACTIVITY_KF)
	if GetTask(T_TSK_KF_PERSONAL_RANK[nBattleType]) > 0 then
		sPersonalPoint = format("%s<color>, tİnh trong phe ta ®¹t h¹ng <color=yellow>%d",sPersonalPoint, GetTask(T_TSK_KF_PERSONAL_RANK[nBattleType]))
	end	
	sPersonalPoint = sPersonalPoint.."<color>. Mêi c¸c b¹n ®Õn chç Ng­êi NhËn Th­ëng Liªn Server ë c¸c thµnh chİnh ®Ó nhËn phÇn th­ëng chiÕn tr­êng.";
...
function BT_AddBattleActivity(nEvenID)
	local nRoute = GetPlayerRoute();
	local nValue = floor(tBAEven[nEvenID]*tBAFactionAdjustment[nEvenID][nRoute])
	local nBefore = BT_GetData(PTNC_ACTIVITY_KF)
	BT_SetData(PTNC_ACTIVITY_KF,nBefore+nValue)
	if BT_GetData(PTNC_ACTIVITY_KF) > 10000000 then  -- ÏÖÔÚ²»ÒªÉÏÏŞÁË£¬Éè³ÉÒ»¸öÓÀÔ¶ÎŞ·¨´ïµ½µÄÖµ
		BT_SetData(PTNC_ACTIVITY_KF,10000000);
	end;
	Msg2Player(format("§¹i hiÖp nhËn ®­îc %d §é Tİch Cùc, ®iÓm hiÖn t¹i %d", nValue, BT_GetData(PTNC_ACTIVITY_KF)))
end;

--Çå³ıÕ½³¡»îÔ¾ÏµÊı
function BT_ClearBattleActivity()
	BT_SetData(PTNC_ACTIVITY_KF,0);
end;
...
function BT_NewBattleClear(nBattleType)	
	local nDate = tonumber(date("%y%m%d"));
...
function BT_EndBattle()
	local IDTab = BT_SortMSPlayerByTaskValue(PTNC_BATTLEPOINT_KF,0,ALL_ID);
	local nPlayerCount = getn(IDTab);
	...
	for i=1,nPlayerCount do	--´¦ÀíÃ¿¸öÈËµÄ¹«¹²»ı·ÖºÍÊ¤¸º»ı·Ö
		...
		if PlayerIndex > 0 then
			nCamp = BT_GetCamp();
			if STATISTIC_OPEN == 1 then
				AddRelayShareData(RELAY_DATA_BATTLE,BATTLE_TYPE,2,"","",0,GetName(),"dddd",BT_GetData(PTNC_BATTLEPOINT_KF),GetPlayerRoute(),GetLevel(),nCamp);
			...
			local nActivity = BT_GetData(PTNC_ACTIVITY_KF)/100;
			...
			if nPointAward + BT_GetData(PTNC_BATTLEPOINT_KF) > POINT_LIMITED then
				nPointAward = POINT_LIMITED - BT_GetData(PTNC_BATTLEPOINT_KF);
			end;
			...
			BT_SetData(PT_POINT_AWARD_KF,nPointAward);
			BT_SetData(PT_JUNGONG_AWARD,nJunGongAward);
			BT_SetData(PT_LAST_CAMP,nCamp);
			BT_SetData(PT_LAST_BATTLE,BATTLE_TYPE);
			local nRetTaskID = T_TSK_KF_BATTLE_RESULT[BATTLE_TYPE]
			if nWhoWin == ALL_ID then
				SetTask(nRetTaskID, LOSE)
			elseif nWhoWin == nCamp then
				SetTask(nRetTaskID, WIN)
			else
				SetTask(nRetTaskID, DEUCE)
			end
...
function BT_ShowBattleRank(nPlayerCount,bShowWorld)
	local tbPlayer = mf_SortMSPlayerByTaskValue(MISSION_ID,PTNC_BATTLEPOINT_KF+BATTLE_OFFSET,0);
	local nOldPlayerIdx = PlayerIndex;
	if bShowWorld == 1 then
		Msg2Global(tostring(tBattleName[BATTLE_TYPE]).." - Tİch lòy c¸ nh©n Top "..nPlayerCount..":");
	else
		Msg2MSAll(MISSION_ID,tostring(tBattleName[BATTLE_TYPE]).." - Tİch lòy c¸ nh©n Top "..nPlayerCount..":");
	end;
	for i=1,min(getn(tbPlayer),nPlayerCount) do
		PlayerIndex = tbPlayer[i];
		if bShowWorld == 1 then
			Msg2Global("H¹ng "..i..": ("..tCampNameZ[BT_GetCamp()]..") "..GetName().." "..BT_GetData(PTNC_BATTLEPOINT_KF).." §iÓm");
		else
			Msg2MSAll(MISSION_ID,"H¹ng "..i..": ("..tCampNameZ[BT_GetCamp()]..") "..GetName().." "..BT_GetData(PTNC_BATTLEPOINT_KF).." §iÓm");
		end;
	end;

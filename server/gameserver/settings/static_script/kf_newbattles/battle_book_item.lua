...
function resource_info()
	if BT_GetData(PT_BATTLE_TYPE) == RESOURCE_ID then
		local nCamp = BT_GetCamp();
		local nEnemyCamp = 3-nCamp;	
		Talk(1,"battle_info","§iÓm tÝch lòy c¸ nh©n: <color=yellow>"..BT_GetData(PTNC_BATTLEPOINT_KF).."<color><enter>Sè l­¬ng th¶o phe m×nh: <color=yellow>"..GetMissionV(MV_RESOURCE_SONG+nCamp-1).."<color>Sè l­¬ng th¶o phe ®Þch: <color=yellow>"..GetMissionV(MV_RESOURCE_SONG+nEnemyCamp-1).."<color>");
	else
...
function see_secret()
	Talk(1,"other_info","HÖ sè ho¹t ®éng hiÖn t¹i:"..BT_GetData(PTNC_ACTIVITY_KF).." Tin tøc b¸o danh:"..BT_GetData(PT_SIGN_UP).." Thêi gian b¸o danh:"..BT_GetData(PT_BATTLE_DATE).."<enter>HiÖn ë chiÕn tr­êng:"..BT_GetData(PT_BATTLE_TYPE).." Tr¹ng th¸i chiÕn tr­êng hiÖn t¹i:"..GetMissionV(MV_BATTLE_STATE).." TÝch lòy:"..BT_GetData(PTNC_BATTLEPOINT_KF).."");
end;

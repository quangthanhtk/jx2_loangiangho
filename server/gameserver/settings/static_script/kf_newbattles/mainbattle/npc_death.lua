...
function process_camp_fight_camp(nPlayerCamp,sNpcName,nNpcIndex)
	...
	elseif sNpcTypeName == "Tiªn phong" then
		...
		if GetMissionV(MV_KILL_SONG_PIONEER_COUNT+nEnemyCamp-1) == PIONEER_COUNT then
			if GetMissionV(MV_KILL_SONG_GENERAL_COUNT+nEnemyCamp-1) == GENERAL_COUNT then
				...
				--Try_To_Create_FightNPC("ÔªË§",nEnemyCamp);	--¶Ô·½´ó½«ºÍÏÈ·æ¶¼±»Ãð£¬Ôò¶Ô·½ÔªË§³ö
			end;
			if GetMissionV(MV_KILL_SONG_CAPTAIN_COUNT+nEnemyCamp-1) == CAPTAIN_COUNT then
				if nEnemyNpcType == 1 then
					if nEnemyCamp == SONG_ID then
	...
	elseif sNpcTypeName == "T­íng qu©n" then
		...
		-- 		if nEnemyNpcType == 2 then
		-- 			if nEnemyCamp == SONG_ID then
		-- 				Msg2MSAll(MISSION_ID,"ËÎ·½µÄ´ó½«ÏÈ·æ±»ÍÀÂ¾´ù¾¡£¬ËÎ·½µÄÔªË§³öÏÖÁË¡£");
		-- 			else
		-- 				Msg2MSAll(MISSION_ID,"ÁÉ·½µÄ´ó½«ÏÈ·æ±»ÍÀÂ¾´ù¾¡£¬ÁÉ·½µÄÔªË§³öÏÖÁË¡£");
		-- 			end;
		-- 		end;
		-- 		Try_To_Create_FightNPC("ÔªË§",nEnemyCamp);	--¶Ô·½´ó½«ÏÈ·æ±»ÍÀÂ¾´ù¾¡£¬¶Ô·½ÔªË§³ö
		-- 	end;
		-- end;
...
function tell_end(tArg)
	Say(tBattleName[BATTLE_TYPE].."®· kÕt thóc, sù nç lùc cña c¸c vÞ sÏ ®­îc ban th­ëng xøng ®¸ng.",0);
	SetFightState(0);
end;

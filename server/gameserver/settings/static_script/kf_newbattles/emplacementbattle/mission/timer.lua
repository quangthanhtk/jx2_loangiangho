...
function BT_EmplacementDefendAward()
	local ex,ey = EMPLACEMENT_POS[1],EMPLACEMENT_POS[2]
	local m,mx,my = GetWorldPos()
	local sqareRadius = (mx - ex)*(mx - ex) + (my - ey)*(my - ey)
	if sqareRadius <= 765 and GetLife() > 1 then
		BT_AddBattleActivity(BA_EMPL_DEFEND_15S);
	end
end
...
function Process_Fight_Timer()
	...
	if nLoop == 0 then
		...
		SetMissionV(MV_TIMER_LOOP,nLoop-1)
		BT_OperateAllPlayer(BT_EmplacementDefendAward,{},ALL_ID);  -- ÏÖÔÚË«·½¶¼»ñµÃ»îÔ¾¶È
		...
		-- 	Msg2MSAll(MISSION_ID, format("BOSS%sÒÑË¢ÐÂ£¡", "ÀäÏãÁâ"));
		-- end
...
function tell_end(param)
	...
	local szMsg = tBattleName[BATTLE_TYPE].." kÕt thóc!"
	SetTask(T_TSK_KF_ACTIVITY_TO_SOURCE[BATTLE_TYPE], BT_GetData(PTNC_ACTIVITY_KF))
	local szRank = format("Top %d §é TÝch Cùc phe ta:", T_TSK_KF_RANK_AWARD_NEED_RANK[BATTLE_TYPE])
	for i = 1, getn(tRank) do
		if i > T_TSK_KF_RANK_AWARD_NEED_RANK[BATTLE_TYPE] then break end
		szRank = format("%s\n    H¹ng %d: <color=yellow>%s<color>  TÝch Cùc: <color=yellow>%d<color>", szRank, i, tRank[i][1], tRank[i][2])
	end
	Say(szMsg..szRank,0);
	SetFightState(0);
end;
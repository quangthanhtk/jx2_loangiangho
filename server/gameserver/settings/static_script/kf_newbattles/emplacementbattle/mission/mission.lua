...
function InitMission()
	...
	SetNpcScript(npcIndex,"\\settings\\static_script\\kf_newbattles\\emplacementbattle\\maingeneral_npc.lua");
	-- npcIndex = CreateNpc("ÖĞÔ­´¢Îï¹ñ",sCampName1.."¾üĞè¹Ù",MAPID,1582,3366);
	-- SetNpcScript(npcIndex,"\\settings\\static_script\\kf_newbattles\\quartermaster_npc.lua");
	-- npcIndex = CreateNpc("ÖĞÔ­´¢Îï¹ñ",sCampName2.."¾üĞè¹Ù",MAPID,1875,3365);
	-- SetNpcScript(npcIndex,"\\settings\\static_script\\kf_newbattles\\quartermaster_npc.lua");
...
-- ¿ªÆôÅÚÌ¨Õ½Ê±Ç©¸öÃû£¬ÓĞ±¾³¡Ç©ÃûµÄÍæ¼Ò¿ÉÒÔ»Øµ½Õ½³¡
function emplacement_sign_in()
	-- ¼ÇÏÂÊ±¼ä´Áµ±Ç©Ãû
	SetTask(TSK_EMPLACEMENT_SIGN, GetTime())
end

function RunMission()
	Msg2MSAll(MISSION_ID,"Th¸m tö håi b¸o, ®Şch ph­¬ng ®· hµnh ®éng, t­íng sÜ phe ta nhanh chãng vµo trong chuÈn bŞ t¸c chiÕn.");

	BT_OperateAllPlayer(emplacement_sign_in,{},ALL_ID);
...
function JoinMission(RoleIndex,nCamp)
	...
	SetTask(TSK_LAST_JOIN_CAMP, date*10 + nCamp)
	PlayerIndex = RoleIndex;
end;

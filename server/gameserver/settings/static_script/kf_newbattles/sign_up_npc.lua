...
function join_battle(nBattleType)
	...
	SendScript2VM("\\script\\function\\cdkey\\ck_head.lua",  format("ck_global_set_task_finish(%d)", 1305 + BT_GetBattleType() - 1) );
...
function get_battle_award()
	...
	local nPointAward = BT_GetData(PT_POINT_AWARD_KF);
...
function get_all_award(nType)
	...
	local nLastCamp = BT_GetData(PT_LAST_CAMP);
	local nLastBattle = BT_GetData(PT_LAST_BATTLE);
	local nLastResult = BT_GetData(PT_LAST_RESULT_KF);

...
function gemNpcTalkMain()
	...
	local szTitle = format("<color=green>%s<color>: %s", ING_SZ_NPC_NAME, "Chç cña ta cã thÓ hoµn thµnh c¸c thao t¸c liªn quan vÒ ®¸ quý")
	local tbSay = {}
	...
	tinsert(tbSay, format("%s/_gemOp_RemoveGem", "Th¸o ®¸ quý"))
	--tinsert(tbSay, format("%s/_open_gem_shop", "±¦Ê¯ÉÌµê"))
	tinsert(tbSay, format("%s/_open_gem_lucky_box", "R­¬ng May M¾n §¸ Quý")); --LongDaiKa:

...
Import("\\script\\ksgvn\\callback\\spend_coin_callback.lua")
...
function vfs_main_callback(nCost, nExpValue, nJinJuan)
	...
	-- KSGVN
	_ksg_SpendCoinCallback(tonumber(nCost))
	-- END KSGVN
...
function vfs_open_JJ_shop()
	...
	Say("§¹i hiÖp ng­êi cÇn g× kh«ng?", getn(tSay), tSay);
end

function vfs_open_shop(nIndex)
	SendScript2Client(format("Open([[EquipShop]], %d, [[%s]]);", tonumber(nIndex), "TiÖm Kim PhiÕu")); --show_equip_shop(tonumber(nIndex));
end

function vfs_open_YJ_shop()
	SendScript2Client(format("Open([[EquipShop]], %d, [[%s]]);", 3033, "TiÖm Ng©n PhiÕu")); --show_equip_shop(3033);
end

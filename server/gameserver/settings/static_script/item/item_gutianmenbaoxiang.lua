...
m_szLogTitle ="Cæ Thiªn M«n B¶o R­¬ng"
...
t_normal_award=
{
	...
	{{"Cöu ChuyÓn Hoµn Hån §an",{1,0,32,1},nil,nil, {1, 100}},200},
...
function _ConfirmUse(nItemIdx)
	...
	if 1 == _MissionAward_CheckCost(t_require, 1) then
		if 1 == DelItemByIndex(nItemIdx, 1) then
			...
			local g,d,p,n = MissionAward_UpdateSpecialAward(t_special_award, m_szLogTitle, nAdd, 1)--±Ø³öÌìÃÅ½ğÁî
			...
			while not n or n<=0 or _IsTianMenJinLing(g,d,p,n)==1 do
				...
				if nTryCnt > N_MAX_TRY_CNT then
					WriteLog(format("[%s] open [gutianmenbaoxiang][%s], for [%d/%d] times rand, get none item, specitial counter=[%d/%d] ", GetName(), m_szLogTitle, nTryCnt, N_MAX_TRY_CNT, nCnt,nMax))
					return 0,0,0,-1
				end
				
				g,d,p,n = _MissionAward_RandAward(t_normal_award, m_szLogTitle)
				...
				if _IsTianMenJinLing(g,d,p,n)==1 then
					...
					if nCnt and nCnt > N_NEVER_X_AWARD_LIMIT then
    					return MA_SpecialAward(t_special_award, m_szLogTitle)--Ëæµ½ÁËÌìÃÅ½ğÁî
    				end
				end
			end
			return g,d,p,n

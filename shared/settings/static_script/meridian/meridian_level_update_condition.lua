...
function meridianlevel_check_require(nNewLevel, bNotify)
	...
	if MeridianGetLevel() >= nNewLevel then
		Talk(1,"",format("Ng­¬i ®· kÝch ho¹t %s c¶nh giíi!", tCfg.sz_name))
		return 0
	end
	...
	if t then
		local playerLv = gf_GetTransCount()*100 + GetLevel()
		local limitLv = t.trans_count*100 + t.level
		if playerLv < limitLv then
			if 1 == bNotify then
    			Talk(1,"",format("Ch­a ®¹t %d chuyÓn %d cÊp kh«ng thÓ kÝch ho¹t C¶nh giíi %s!", t.trans_count,t.level,tCfg.sz_name))
    		end
    		return 0
		end
		...
		-- 		if 1 == bNotify then
  --       			Talk(1,"",format("µÈ¼¶Î´´ïµ½%d¼¶²»ÄÜ¼¤»î%s¾³½ç!", t.level, tCfg.sz_name))
  --       		end
  --       		return 0
		-- 	end
		-- end
